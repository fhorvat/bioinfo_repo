...
# save table
readr::write_csv(ltr_sample_tb, file.path(outpath, "LTRs.random_200_per_repName.200730.csv"))
...
purrr::map(names(ltr_sample_seq_list), function(ltr_name){
  ...
  ltr_sample_seq_list[[ltr_name]] %T>% 
    Biostrings::writeXStringSet(., file.path(outpath, "fasta_files", str_c("LTRs.random_200_per_repName", ltr_name, "200730.single.fasta", sep = ".")))
