...
#PBS -m n
#PBS -N pbs.02c.filter_bams.19to32
...
BASE=${FILE#${INPUT_DIR}/}
BASE=${BASE%.bam}.19to32nt

# ----------------Commands------------------- #
# remove reads with deletions and insertions
# get reads between 19-32 nt long
samtools view -h ${FILE} | \
awk -F '\t' '($1 ~ /^@/ || $6 !~/I/) && ($1 ~ /^@/ || $6 !~/D/) && ($1 ~ /^@/ || $6 ~/19M|20M|21M|22M|23M|24M|25M|26M|27M|28M|29M|30M|31M|32M/)' | \
