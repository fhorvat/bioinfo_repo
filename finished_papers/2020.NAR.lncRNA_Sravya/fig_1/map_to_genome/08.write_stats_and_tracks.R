...
######################################################## WORKING DIRECTORY
setwd("/common/WORK/fhorvat/Projekti/Svoboda/lncRNA_KO/datasets/Lnc1_KO.2018_Dec/Data/Mapped/STAR_mm10")
...
tracks_tbl <-
  ...
  dplyr::mutate(sample_id = str_extract(URL, pattern = str_c(stats_tbl$sample_id, collapse = "|")),
                ...
                bw_name = ifelse(test = (scaled == "RPM_scaled"), 
                                 yes = str_c(experiment, ".bw.", str_replace(sample_id, "s_", ""), ".scaled"), 
                                 no = str_c(experiment, ".bw.", str_replace(sample_id, "s_", ""), ".raw")), 
                bam_name = str_c(experiment, ".bam.", str_replace(sample_id, "s_", "")), 
