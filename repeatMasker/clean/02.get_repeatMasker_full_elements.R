...
rmsk_all <- 
  ...
  readr::write_delim(., file = file.path(genome_dir, basename(rmsk_path) %>% str_replace(., "clean.fa.out.gz", "joined_rmsk_id.fa.out.gz")), delim = "\t")



# #### test
# rmsk_all_gr <- GRanges(rmsk_all %>% dplyr::mutate(strand = "*"))
# 
# rmsk_query <- 3786534
# test <- rmsk_all[rmsk_all$rmsk_id %in% rmsk_query, ]
# test_gr <- GRanges(test %>% dplyr::mutate(strand = "*"))
# test_overlaps <- findOverlaps(test_gr, rmsk_tidy, ignore.strand = T)
# rmsk_tidy[subjectHits(test_overlaps)]
# 
# 
