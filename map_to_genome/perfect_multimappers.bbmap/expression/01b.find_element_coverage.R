...
### INFO: finds coverage of LINE1 elements from list and outputs total coverage ratio and longest coverage stretch in each element
...
line1_coverage_list <- purrr::map(names(coverage_list), function(sample_id){
  ...
  line1_coverage_sum <- 
    ...
    set_colnames(., c("rmsk_id", str_c(sample_id, c("coverage_ratio", "longest_stretch"), sep = ".")))
