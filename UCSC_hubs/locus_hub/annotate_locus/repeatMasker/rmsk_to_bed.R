...
######################################################## WORKING DIRECTORY
setwd("/common/WORK/fhorvat/Projekti/Svoboda/ovomucin_KO/Analysis/2021_paper/ovomucin_locus/AcoCah/annotate_locus/repeatMasker")
...
rmsk_files <- list.files(inpath, pattern = "*.\\.fa\\.out\\.gz", recursive = T, full.names = T)
...
purrr::map(rmsk_files, function(rmsk_path){
  ...
  rmsk_tb <-
    ...
    dplyr::filter(seqnames == scaffold) %>% 
  ...
  trackDb_all <- purrr::map(names(rmsk_bed), function(rep_class){
