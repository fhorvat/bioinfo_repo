...
# blastn query input
INPUT_DIR=.
IN_SEQ=($(find ${INPUT_DIR} -maxdepth 1 -name "*.genomic.fasta"))
FILE=${IN_SEQ[0]}
BASE=${FILE#${INPUT_DIR}/}
BASE=${BASE%.fasta}
...
# ----------------Commands------------------- #
# blast
blastn -query ${FILE} -db ${IN_DB} -out ${BASE}.blastn.txt -num_threads ${THREADS} -outfmt 6
