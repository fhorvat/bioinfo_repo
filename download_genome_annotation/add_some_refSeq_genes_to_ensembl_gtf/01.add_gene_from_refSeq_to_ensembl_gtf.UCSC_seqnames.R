...
######################################################## WORKING DIRECTORY
setwd("/common/DB/genome_reference/rat/rn6.Rnor_6.0.GCA_000001895.4/gtf_with_some_refSeq_genes")
...
refSeqToEnsemblStyleGtf <- function(gene_name){
  ...
  if(length(refseq_gtf_filt) > 0){
    ...
    # change seqnames
    new_seqname <- scaffold_names[scaffold_names$refSeq_accn == as.character(seqnames(refseq_gtf_filt)[1]), ]$UCSC_style_name
...
refSeqToGeneInfo <- function(gene_name){
  ...
  if(length(refseq_gtf_filt) > 0){
    ...
    refseq_geneInfo <- tibble(gene_id = mcols(refseq_gtf_gene)$gene, 
                              seqnames = scaffold_names[scaffold_names$refSeq_accn == as.character(seqnames(refseq_gtf_gene)), ]$UCSC_style_name,
                              ...
                              gene_description = str_c(mcols(refseq_gtf_gene)$db_xref, ", added from RefSeq .gtf"))
...
# ensembl gtf path
ensembl_gtf_path <- file.path(inpath, "..", "ensembl.99.Rnor_6.0.20200415.UCSCseqnames.gtf.gz")

# refSeq gtf path
refseq_gtf_path <- file.path(inpath, "../GCF_000001895.5_Rnor_6.0_genomic.gtf.gz")

# gene info path
gene_info_path <- file.path(inpath, "..", "ensembl.99.Rnor_6.0.20200415.UCSCseqnames.geneInfo.csv")

# scaffold names path
scaffold_names_path <- file.path(inpath, "..", "GCF_000001895.5_Rnor_6.0_assembly_report.txt")
...
# read scaffold names
scaffold_names <- readr::read_delim(scaffold_names_path, delim = "\t", comment = "#", 
