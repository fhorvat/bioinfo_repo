...
# pseudocount <- 0.0001
# 
# # rank sort tiles based on WT expression
# rpm_tb_sorted <- 
#   rpm_tb %>% 
#   set_colnames(., str_remove(colnames(.), "Mov10l_")) %>% 
#   dplyr::select(coordinates, WT_13dpp, KO_13dpp) %>% 
#   dplyr::arrange(desc(WT_13dpp)) %>% 
#   dplyr::mutate(log2FC = log2((KO_13dpp + pseudocount) / (WT_13dpp + pseudocount))) %>% 
#   dplyr::filter(WT_13dpp >= 0.1)
# 
# # write tiles rank sorted
# readr::write_csv(rpm_tb_sorted, file.path(outpath, str_c(table_name, "rank_sorted", "0.1_rpkm_cutoff", "csv", sep = ".")))
