...
### directory paths
# set experiment path
experiment_path <- "/common/WORK/fhorvat/Projekti/Svoboda/CNOT6L"
...
fpkm_df <-
  assay(se) %>%
  as.data.frame(.) %>% 
  set_colnames(., str_remove(colnames(.), ".Aligned.sortedByCoord.out.bam|.PE.total.bam")) %>%
  ...
  .[, list(fpkm = round(mean(fpkm), 3)), by = .(gene_id, stage, genotype)] %>%
  .[, sample_id := str_c(stage, "_", genotype)] %>% 
  .[, -c("stage", "genotype")] %T>% 
