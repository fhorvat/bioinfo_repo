...
### find expression of Sirena1 and neighbour genes (Spty2d1, Uevld)
# Fugaku GV 
fpkm_fugaku <- 
  fpkm_long_tb %>%
  dplyr::filter(gene_id %in% c("ENSMUSG00000110001", "ENSMUSG00000049516", "ENSMUSG00000043262"), 
                sample_id == "s_GV.WE.PE") %>% 
  dplyr::left_join(., genes_info, by = "gene_id") %>% 
  dplyr::select(gene_name, gene_id, stage = sample_id, fpkm) %>% 
  dplyr::mutate(fpkm = round(fpkm, 2)) %>% 
  dplyr::arrange(gene_name) %T>% 
  readr::write_csv(., path = file.path(outpath, "Sirena1_Spty2d1_Uevld.FPKM.Fugaku.csv"))

# CNOT6L GV
fpkm_cnot6l <- 
  readr::read_csv("/common/WORK/fhorvat/Projekti/Svoboda/CNOT6L/Analysis/expression/old/ensembl.93.GRCm38.p6.20180919.UCSCseqnames.CNOT6L.avg.FPKM.csv") %>% 
  dplyr::filter(gene_id %in% c("ENSMUSG00000110001", "ENSMUSG00000049516", "ENSMUSG00000043262"), 
                stage == "s_GV_WT") %>% 
  dplyr::left_join(., genes_info, by = "gene_id") %>% 
  dplyr::select(gene_name, gene_id, stage, avg_fpkm) %>% 
  dplyr::mutate(avg_fpkm = round(avg_fpkm, 2)) %>% 
  dplyr::arrange(gene_name) %T>% 
  readr::write_csv(., path = file.path(outpath, "Sirena1_Spty2d1_Uevld.FPKM.CNOT6L.csv"))
