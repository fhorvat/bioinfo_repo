...
######################################################## WORKING DIRECTORY
# set data/reference
reference <- "Zhang_2021_NatCellBiol_GSE169528"
...
rename_file <- 
  ...
  dplyr::mutate(stage = str_extract_all(tissue, "GV|secondary follicle|primary follicle|1C|2C|4C|MII|[0-9]+PEA") %>% 
                  purrr::map(., str_c, collapse = "_") %>% 
                  unlist(.), 
