...
setwd("/common/WORK/fhorvat/Projekti/Svoboda/Other/L1_elements/mapping/STAR/whole_genome/coverage")
features <- import.bed(con = "L1_6000bp_whole_genome_mapping_ordered.bed")
...
plotRangesMulti <- function(name_index, x){
  ...
  current_plot <- ggplot() + 
    ...
    scale_x_continuous(limits = c(start(features[name_index, ]) - 100, end(features[name_index, ])), 
                       breaks = seq(from = start(features[name_index, ]), to = end(features[name_index, ]), by = 100),
                       labels = seq(0, width(features[name_index, ]), by = 100)) +
...
plot1 <- plotRangesMulti(name_index = 1, x = 500)
plot2 <- plotRangesMulti(name_index = 2, x = 100)
plot3 <- plotRangesMulti(name_index = 3, x = 50)
plot4 <- plotRangesMulti(name_index = 4, x = 5)

plot_1_4 <- list(plot1, plot2, plot3, plot4)
plot_all <- lapply(5:30, FUN = plotRangesMulti, x = 2)
plot_all <- c(plot_1_4, plot_all)

pdf("L1_6000bp_complete_genome_mapping_coverage_bins_30.pdf", width = 19.9, height = 10)
