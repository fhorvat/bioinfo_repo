...
setwd("/common/WORK/fhorvat/Projekti/Svoboda/Other/MT_elements/review/LTR_alignment/files/seq_logo/test_ordered")
...
msaPlot <- function(aligned_sequences, plot_file_name){
  ...
  aligned_sequences_df <- 
    ...
    mutate(value = replace(value, value == "-", NA), 
           ...
           ID = factor(ID, levels = rev(names(aligned_sequences))))
  ...
  ggplot(aligned_sequences_df, aes(x = variable, y = ID)) +
    ...
    theme(axis.title.x = element_blank(),
          ...
          axis.text.y = element_text(size = 2, hjust = 1, vjust = 0),
...
signalLogo <- function(sequences, signal_pattern){
  
  sequences = LTR_seqences
  
  # get sequences with pattern
  match_pattern <- vmatchPattern(signal_pattern, sequences)
  
  # get the outliers 
  is_outlier <- 
    as.data.frame(match_pattern) %$%
    start %>% 
    removeOutliers() %>% 
    is.na()
  ...
  # get pattern in df
  pattern_names <- 
    cbind(as.data.frame(match_pattern), 
          is_outlier, 
          seq_names = names(unlist(match_pattern))) %>% 
    dplyr::filter(is_outlier == F) %$%
    seq_names %>% 
    as.character()
  
  # order sequences
  sequences <- c(sequences[names(sequences) %in% pattern_names], 
                 sequences[!names(sequences) %in% pattern_names])
  ...
  sub_sequences_end <- ifelse(width(sequences) < max(signal_start) + 5, width(sequences), signal_start + 5)
  
  # get the subsequence 40 nt around the signal start
  sub_sequences <- subseq(x = sequences, start = min(signal_start) - 20, end = sub_sequences_end)
...
LTR_samples_all <- unique(LTR_data$LTR_class)
LTR_samples_all <- LTR_samples_all[!LTR_samples_all == "MLT2"]
...
for(LTR_sample in unique(LTR_data$LTR_class)){
  ...
  seq_width <- 20
  pattern_mismatch <- 1
  full_alignment_iterations <- 0
  logo_alignment_iterations <- 0
  
  #   # align by Clustal Omega
  #   LTR_seqences_aligned <- ClustalO(infile = paste0(getwd(), "/", file_name, ".fasta"), 
  #                                    outfile = paste0(getwd(), "/", file_name, "_iter", full_alignment_iterations, "_msa.fasta"), 
  #                                    threads = 8, 
  #                                    force = T, 
  #                                    iterations = full_alignment_iterations)
  #   
  #   # plot MSA as "heatmap"
  #   msaPlot(aligned_sequences = LTR_seqences_aligned, 
  #           plot_file_name = paste0(file_name, "_iter", full_alignment_iterations, "_msa_plot.pdf"))
  ...
  signalLogo(sequences = LTR_seqences, signal_pattern = "AATAAA")
