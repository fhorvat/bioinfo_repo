...
######################################################## WORKING DIRECTORY
# set data/reference
reference <- "Ishino_2021_NAR_PRJDB10777"

# set working directory
setwd(file.path("/common/WORK/fhorvat/Projekti/Svoboda/hamster_KO/datasets/hamster_Siomi_WT.Piwil1_Piwil3_IP.small_RNAseq/Data/Raw/Links"))
...
rename_file <- 
  ...
  dplyr::mutate(
    ...
    tissue = str_extract(sample_name, "Testis|Oocyte|Ovary|2Cell") %>% tolower(.) %>% 
      str_replace(., "2cell", "2C"), 
    IP = str_extract(sample_name, "PIWIL1|PIWIL3")
...
rename_file %>%
  dplyr::select(sample_id = name, 
                ...
                IP,
                sample_name) %T>%
  readr:::write_csv(., file = file.path(outpath, "../../Documentation", str_c("hamster_Siomi_WT.Piwil1_Piwil3_IP", ".sampleTable.csv"))) %T>%
  readr:::write_csv(., file = file.path(sra_path, str_c("hamster_Siomi_WT.Piwil1_Piwil3_IP", ".sampleTable.csv")))
