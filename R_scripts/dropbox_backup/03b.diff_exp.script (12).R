...
experiment='hamster_testis_Mov10l.small_RNAseq'
single_end=TRUE
threads=1
mapped_path='/common/WORK/fhorvat/Projekti/Svoboda/hamster_KO/datasets/hamster_testis_Mov10l.small_RNAseq/Data/Mapped/bbmap_mesAur1'
...
feature_coordinates='/common/WORK/fhorvat/Projekti/Svoboda/hamster_KO/datasets/hamster_testis_Mov10l.small_RNAseq/Analysis/piRNA_clusters/expression/piRNA_clusters.testis_Mov10l.PILFER.saf'
feature_names='piRNA_clusters.testis_Mov10l.PILFER'
grouping_variables='genotype'
results_groups='Mov10l_KO,Mov10l_WT'
...
# counts path
counts_path <- list.files(inpath, ".*\\.unique\\.counts\\.txt$", full.names = T)
...
sample_table_dds <- 
  sample_table %>% 
  # dplyr::filter(!(sample_id %in% c("s_testis_Mov10l_KO_F63-M1_r4.SE", "s_testis_Mov10l_KO_F63-M2_r5.SE"))) %>%
  as.data.table(.) %>% 
