...
######################################################## WORKING DIRECTORY
setwd("/common/WORK/fhorvat/Projekti/Svoboda/smallRNASeq_2018/Eliska_mESC_MosIR/Data/Raw/Links")
...
# raw files
raw_path <- list.files(path = "/common/RAW/Svoboda/smallRNASeq_2018/2018-07-05-CCGH0ANXX/Eliska_mESC_MosIR", pattern = "*.txt.gz", full.names = T)
...
rename_file <- 
  sample_table %>% 
  dplyr::select(sample_id = `Sample name`, genotype, transfection = Transfection, cell = `cell type`, 
                barcode = `index sequence read`, NEBnext_index = `NEBnext index no`) %>% 
