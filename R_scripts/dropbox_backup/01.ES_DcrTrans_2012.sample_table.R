...
######################################################## WORKING DIRECTORY
setwd("/common/WORK/fhorvat/Projekti/Svoboda/in_house_sequencing/Dicer_RNASeq_SOLiD/ES_DcrTrans_2012/Data/Documentation")
...
# raw path
raw_path <- "/common/RAW/Svoboda/old_data/ES_DcrTrans_2012.SOLiD.smallRNASeq"
...
sample_table_filt <- 
  ...
  dplyr::mutate(genotype = 
                  str_remove_all(sample_id, "s_ES_|MII.|_utra|_tran|_r[1-9]$") %>% 
                  str_replace(., "Dicer", "Dcr") %>% 
                  str_replace(., "Som.Dcr", "DcrSom"), 
                transfection = 
                  str_extract(sample_id, "tran|utra") %>% 
                  str_replace(., "utra", "utran"), 
                cell = str_extract(sample_id, "ES|ES_MII")) %>% 
