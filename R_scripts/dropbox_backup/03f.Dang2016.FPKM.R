...
######################################################## PATH VARIABLES
# set experiment name
experiment <- "Dang_2016_GenomeBiol_GSE71318"
experiment_name <- "Dang2016"
...
fpkm_ind_filt %>%
  ...
  dplyr::mutate(gene_name = factor(gene_name, levels = str_c("Cnot", c(1:6, "6l", 7:11))),
                stage = factor(stage, levels = c("MII", "1C", "2C", "4C", "8C", "morula", "early_blast", "middle_blast", "late_blast", "ICM", "troph", "ESC"))) %>%
  ...
  ggsave(filename = file.path(outpath, "results", str_c("boxplot.CNOT.GRCh38.91.20180312.UCSCnames.clean.", experiment_name, ".FPKM.png")), 
         height = 20, width = 10, limitsize = F)
...
# save plot
ggsave(filename = file.path(outpath, "results", str_c("PCAplot.", experiment_name, ".GRCh38.91.rlog.notblind.png")),
