...
######################################################## MAIN CODE# set name
# set experiment name
experiment <- "Piwil1_Piwil3_IP.small_RNAseq"
...
# save
readr::write_csv(rpkm_tb, file.path(inpath, rpm_tb_path %>% basename(.) %>% 
                                      str_replace(., "FPM_mean.csv", str_c("RPKM_mean", experiment, "csv", sep = "."))))
