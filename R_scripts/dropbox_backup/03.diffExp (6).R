...
purrr::map(stages, function(stage){
  ...
  ma_plot <- 
    ggplot() + 
    geom_point(data = plot_df, aes(x = mean, y = lfc, color = regulation, alpha = regulation, shape = mito, size = mito)) +
    ...
    scale_size_manual(values = c(other = 5, mitochondrion = 7)) +
    xlab("") +
    ylab("") +
    theme_bw() +
  ...
  ggsave(filename = file.path(outpath, str_c("diffExp", 
                                             ...
                                             "DESeq2.protein_coding.MA_plot.big.png", sep = ".")),
