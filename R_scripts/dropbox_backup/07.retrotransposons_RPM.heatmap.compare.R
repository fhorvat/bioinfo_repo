...
se_path <- file.path(inpath, "old/summarizedExperiments.all_LINEs_LTRs")
...
rpm_heatmap_tb <- 
  retro_rpm[["removed_genes"]] %>% 
  as.tibble(.) %>% 
  filter(!((stage == "2C") & (experiment == "Hamazaki")),
         stage != "Sertoli") %>% 
  mutate(log10_rpm = log10(sum_avg_rpm)) %>%
  right_join(., stage_clean, by = "stage") %>% 
  right_join(., retro_clean, by = "class") %>% 
...
which.gap.row <- which(rownames(rpm_heatmap_tb) %in% c("LINE 1 all", "L1Md_F", "IAPEy_int + IAPLTR1_Mm", "MERVL_int + MT2_Mm", "MTA_int + LTRs", "ORR1A_int + LTRs"))
...
pheatmap::pheatmap(rpm_heatmap_tb,
                   ...
                   filename = file.path(outpath, str_c("heatmap.rmsk.L1_and_LTRs.20190306.RPM.old.png", sep = ".")),
                   height = 20,
                   width = 20)
