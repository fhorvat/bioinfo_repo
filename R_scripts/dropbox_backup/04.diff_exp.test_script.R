...
if(exploratory_analysis == "yes"){
  ...
  pca_tb <-
    ...
    dplyr::mutate(stage = rep(c("GV", "MII", "1C", "2C", "4C", "8C"), c(4, 8, 6, 7, 10, 10)),
                  grouped_variables = str_c(stage, genotype, sep = "_")) %>% 
    dplyr::mutate(stage = factor(stage, levels = c("GV", "MII", "1C", "2C", "4C", "8C")), 
                  genotype = factor(genotype, levels = c("WT", "Exosc10_KO"))) %>% 
  ...
  if(length(grouping_variables) == 1){
    ...
    pca_plot <-
      pca_plot +
      geom_point(aes_string(fill = grouping_variables[1]), color = "black", size = 5, shape = 21) +
      guides(color = guide_legend(override.aes = list(shape = 23, size = 7.5)))
    
  }else{
    
    ## color = first grouping variable, shape = second grouping variable
    # get length of unique values of second grouping variable
    grouping_variables_lengths <- 
      pca_tb %>% 
      dplyr::select(length_1 = grouping_variables[1], length_2 = grouping_variables[2]) %>% 
      dplyr::summarise_all(list(~length(unique(.))))
    
    # tidy legend labels
    legend_labels <- 
      pca_tb$grouped_variables %>% 
      unique(.) %>% 
      str_replace_all(., "_", " ")
    
    # # create plot with shapes with outlines
    # pca_plot <- 
    #   pca_plot +
    #   geom_point(aes(fill = grouped_variables, shape = grouped_variables), color = "black", size = 7.5) +
    #   scale_fill_manual(values = gg_color_hue(grouping_variables_lengths$length_1)[as.integer(pca_tb$stage)], 
    #                     labels = legend_labels) + 
    #   scale_shape_manual(values  = c(21:25, 4)[1:grouping_variables_lengths$length_2][as.integer(pca_tb$genotype)],
    #                      labels = legend_labels) +
    #   labs(fill = str_c(grouping_variables[1], " / ", grouping_variables[2]), 
    #        shape = str_c(grouping_variables[1], " / ", grouping_variables[2])) +
    #   guides(fill = guide_legend(override.aes = list(size = 7.5)))
    
    # create plot with shapes with outlines
    pca_plot <- 
      pca_plot +
      geom_point(aes(size = grouped_variables)
                 fill = gg_color_hue(grouping_variables_lengths$length_1)[as.integer(pca_tb$stage)], 
                 shape = c(21:25, 4)[1:grouping_variables_lengths$length_2][as.integer(pca_tb$genotype)], 
                 color = "black", 
                 size = 7.5) + 
        guides(size = guide_legend(override.aes = list(size = 7.5)))
  ...
  # save plot
  ggsave(filename = file.path(outpath, str_c("test.PCA_plot.PC1_PC2", "rlog", 
  ...
  ## add labels to points and save another plot
  # add labels
  pca_plot <- 
    pca_plot +
  ...
  # save labeled plot
  ggsave(filename = file.path(outpath, str_c("PCA_plot.PC1_PC2", "rlog", 
                                             str_c(grouping_variables, collapse = "_"),
                                             ifelse(protein_coding_only == "yes", "protein_coding", "all_biotype"), 
                                             "labeled", "png", sep = ".")),
         plot = pca_plot, width = 12, height = 10)
  ...
  if(interactive == "yes"){
    ...
    htmlwidgets::saveWidget(plotly::as_widget(p),
                            file = file.path(outpath,
                                             str_c("PCA_plot.PC1_PC2", "rlog", str_c(grouping_variables, collapse = "_"),
                                                   ifelse(protein_coding_only == "yes", "protein_coding", "all_biotype"), "html", sep = ".")),
                            selfcontained = T)
  }
