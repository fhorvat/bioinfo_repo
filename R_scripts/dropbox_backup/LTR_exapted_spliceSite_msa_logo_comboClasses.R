...
changeWD("/common/WORK/fhorvat/Projekti/Svoboda/Other/MT_elements/review/LTR_alignment/files/seq_logo/exapted_LTR")
...
sj_splice_start_LTR %<>%  
  ...
  mutate(start = start - expand_range, 
         ...
         LTR_name = fullName) %>% 
  separate(LTR_name, c("LTR_range", "LTR_subclass", "LTR_class"), sep = "\\|") %>% 
  dplyr::select(-LTR_range, -LTR_subclass) 

################################################################################## get different classes 
comboClasses <- list(
  "MT2" = c("MT2", "MT2A", "MT2B", "MT2C"),
  "MT"   = c("MTA", "MTB", "MTC", "MTD", "MTE", "MTE2"),
  "ORR1" = c("ORR1A", "ORR1B", "ORR1C", "ORR1D", "ORR1E", "ORR1F", "ORR1G")
)

invisible(lapply(X = names(comboClasses), FUN = function(X){
  
  # filter by LTR_selection 
  sj_splice_start_LTR_select <- 
    sj_splice_start_LTR %>% 
    dplyr::filter(LTR_class %in% comboClasses[[X]]) %>% 
    makeGRangesFromDataFrame(keep.extra.columns = T)
  
  # get splice site sequences
  LTR_seqences <- 
    getSeq(x = Mmusculus, sj_splice_start_LTR_select) %>% 
    setNames(sj_splice_start_LTR_select$fullName)
  
  # set file names
  file_name <- paste0("spliceSite_", expand_range, "nt_", X)
