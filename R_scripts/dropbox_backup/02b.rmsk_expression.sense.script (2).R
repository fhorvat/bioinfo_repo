...
# find all overlaps
overlaps <- findOverlaps(bam_gr, rmsk_bed, ignore.strand = F)
...
overlaps_tb <- 
  ...
  dplyr::sample_n(1) %>% 
  dplyr::ungroup(.) %>% 
  dplyr::group_by(repName, read_width) %>% 
  dplyr::summarise(count = n()) %>% 
  dplyr::ungroup(.)

# save as .RDS
saveRDS(overlaps_tb, file.path(outpath, "expression_sum.sense.RDS_files", str_c(bam_name, ".expression.RDS")))
