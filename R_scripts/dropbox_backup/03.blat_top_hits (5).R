...
######################################################## WORKING DIRECTORY
setwd("/common/WORK/fhorvat/Projekti/Svoboda/lncRNA_KO/Analysis/lnc1_paper/rodents_evolution/lnc1_locus/identify_locus/blat")
...
# fasta index path
fasta_index_path <- list.files(path = file.path(blat_path, ".."), pattern = ".*\\.fa\\.fai$", full.names = T)
...
blat_results_tb <- 
  ...
  dplyr::select(species, gene_name, everything()) %T>%
  readr::write_csv(., file.path(outpath, "blat.top_hits.csv"))
