...
invisible(purrr::map(names(plot_list), function(bam_name){
  ...
  arm1 <- 
    plot_df %>% 
    dplyr::filter(str_detect(arm, "antisense_arm"))
  ...
  p <- 
    plot_ly(type = "scatterpolar", 
            fill = "toself",
            mode = "lines") %>%
    add_trace(r = c(arm2$percent, arm2$percent[1]),
              theta = str_c("r", c(arm2$read_register, arm2$read_register[1])),
              name = "sense arm", 
              color = "#00BFC4") %>%
    add_trace(r = c(arm1$percent, arm1$percent[1]),
              theta = str_c("r", c(arm1$read_register, arm1$read_register[1])),
              name = "antisense arm", 
              color = "#F8766D") %>% 
    layout(polar = list(angularaxis = list(rotation = 90, direction = "clockwise"), 
                        radialaxis = list(visible = F, range = c(0, 50))))
  
  plot_df2 <- 
    dplyr::bind_rows(arm1, arm1[1, ], arm2, arm2[1, ]) %>% 
