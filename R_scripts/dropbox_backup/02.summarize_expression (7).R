...
purrr::map(names(read_lengths), function(rlen){
  ...
  rmp_filt <- 
    ...
    dplyr::group_by(genotype_age, category_I, sense) %>% 
    dplyr::summarise(RPM = round(mean(RPM), 3)) %>% 
    dplyr::ungroup(.) %>% 
    dplyr::filter(!str_detect(genotype_age, "HET")) %>% 
    tidyr::pivot_wider(id_cols = c("genotype_age", "category_I"), names_from = c(genotype_age, sense), values_from = RPM, names_sep = ".") %>% 
...
saveWorkbook(wb, file.path(outpath, str_c("all_LTR_classes.200730", "small_RNA.testis", "mean_RPM", "xlsx", sep = ".")), overwrite = TRUE)
