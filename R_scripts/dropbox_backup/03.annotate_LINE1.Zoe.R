...
######################################################## MAIN CODE
### add info about Zoe's annotation overlap
# to GRanges
line1_gr <- 
  line1_tb %>% 
...
line1_zoe_foverlaps <- findOverlaps(line1_gr, zoe_gr, ignore.strand = T)
# overlaps <- pintersect(line1_gr[queryHits(line1_zoe_foverlaps)], zoe_gr[subjectHits(line1_zoe_foverlaps)], ignore.strand = T)
# percentOverlap <- width(overlaps) / width(line1_gr[queryHits(line1_zoe_foverlaps)])

# extract ID's and genes
line1_zoe_tb <- 
  tibble(rmsk_id = line1_gr[queryHits(line1_zoe_foverlaps)]$rmsk_id,
         id = zoe_gr[subjectHits(line1_zoe_foverlaps)]$id) %>% 
  dplyr::group_by(rmsk_id) %>%
  dplyr::summarise(zoe_id_overlap = str_c(id, collapse = ", "))

# add info about overlaping exons to table
line1_tb %<>% dplyr::left_join(., line1_zoe_tb, by = "rmsk_id")

# save table with info about exon overlap
line1_tb %T>%
  readr::write_csv(., file.path(outpath, "LINE1.4000nt_plus.ORFs.annotated_exons_and_Zoe.csv"))
