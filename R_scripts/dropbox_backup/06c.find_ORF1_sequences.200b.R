...
line1_orfs_tb <- 
  ...
  dplyr::mutate(orf_abs_start = ifelse(strand == "+", start + orf_start - 1, end - orf_start + 1), 
                ...
                seq_start = ifelse(strand == "+", orf_abs_start, orf_abs_start - 200), 
                seq_end = ifelse(strand == "+", orf_abs_start + 200, orf_abs_start)) %>% 
...
line1_seq_consensus <- purrr::map(names(line1_seq_list), function(seq_name){
  ...
  # write as fasta
  Biostrings::writeXStringSet(seq_consensus, file = file.path(outpath, str_c("LINE1.CriGriPICR.ORF1_expand_200", seq_name, "consensus.fasta", sep = ".")))
