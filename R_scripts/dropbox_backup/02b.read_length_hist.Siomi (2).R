...
######################################################## WORKING DIRECTORY
setwd("/common/WORK/fhorvat/Projekti/Svoboda/hamster_KO/datasets/hamster_testis_Mov10l.8.5dpp.small_RNAseq/Analysis/read_length_distribution")
...
# hamster path 
hamster_path <- "/common/WORK/fhorvat/Projekti/Svoboda/hamster_KO/datasets/hamster_testis_Mov10l.8.5dpp.small_RNAseq/Data/Mapped/STAR_Siomi"

# list library sizes files
library_path <- list.files(hamster_path, pattern = "library_hist\\..*\\.SE\\.txt", full.names = T, recursive = T)

# # sample table path
sample_table_path <- "/common/WORK/fhorvat/Projekti/Svoboda/hamster_KO/datasets/hamster_testis_Mov10l.8.5dpp.small_RNAseq/Data/Documentation/hamster_testis_Mov10l.8.5dpp.20201028.sampleTable.csv"
...
library_tb <- purrr::map(library_path, function(path){
  ...
  readr::read_delim(file = path, delim = "\t", col_names = c("count", "cigar")) %>% 
    dplyr::mutate(sample_id = path %>% basename(.) %>% str_remove_all(., "^library_hist\\.|\\.SE\\.perfect\\.txt$") %>%
                    str_replace_all(., c("s_testis_Mov10l" = "Mov10l"))) %>% 
...
read_length_plot <- 
  ggplot() +
  geom_histogram(data = read_length_tb %>% dplyr::filter(alignment_length <= 78), 
                 mapping = aes(x = alignment_length, y = percentage, fill = sample_id), width = 0.9, stat = "identity") +
  scale_fill_viridis_d() +
  facet_grid(sample_id ~ .) +
  scale_x_discrete(limits = 16:78, breaks = 16:78, labels = as.character(16:78)) +
  ...
  ggsave(filename = file.path(outpath, "read_length.histogram.all.Siomi.png"), width = 10, height = 15)
...
ridge_plot <-
  ...
  ggsave(filename = file.path(outpath, "read_length.joy_division.Siomi.png"), width = 15, height = 12)
...
read_length_plot <- 
  ...
  theme(axis.text.x = element_text(angle = 90, hjust = 1, size = 12, vjust = 0.5),
        ...
        panel.grid.minor = element_blank(),
        legend.position = "bottom") +
  ggsave(filename = file.path(outpath, "read_length.stacked_barplot.Siomi.png"), width = 10, height = 10)
