...
plotCumFPKMBinArea <- function(tile_width){
  ...
  # MT2
  all_coverage_counts_df_merged_mt2<- all_coverage_counts_df_merged[all_coverage_counts_df_merged$pos >= 0 & all_coverage_counts_df_merged$pos <= 6522, ]
  all_coverage_counts_df_merged_mt2 <- 
    all_coverage_counts_df_merged_mt2 %>%
    select(2:6) %>%
    summarise_each(funs(sum))
  all_coverage_counts_df_merged_mt2$position <- "mt2" 
  all_coverage_counts_df_merged_mt2$indx <- 1
  all_coverage_counts_df_merged_mt2 <- all_coverage_counts_df_merged_mt2[, c(7, 1:6)]
  
  # downstream
  all_coverage_counts_df_merged_downstream <- all_coverage_counts_df_merged[all_coverage_counts_df_merged$pos > 6522 & all_coverage_counts_df_merged$pos <= 1506522, ]
  ...
  all_coverage_counts_df_sum_fpkm_melt$mt2 <- ifelse((all_coverage_counts_df_sum_fpkm_melt$position == "mt2"),"in_mt2",  "out_mt2")
  all_coverage_counts_df_sum_fpkm_melt$width <- ifelse((all_coverage_counts_df_sum_fpkm_melt$position == "mt2"), 6522, tile_width)
  all_coverage_counts_df_sum_fpkm_melt$pos <- rep(c(seq(-150000, -tile_width, tile_width), 0, seq(6522, (156522 - tile_width), tile_width)), 5)
  ...
  # all_coverage_counts_df_sum_fpkm_output$pos <- c(seq(-150000, -tile_width, tile_width), 0, seq(6522, (156522 - tile_width), tile_width))
  ...
  ggplot(all_coverage_counts_df_sum_fpkm_melt, aes(x = pos, y = fpkm)) +
    ...
    scale_x_continuous(limits = c(-1.5e5, 1.5e5 + 6522), 
                       breaks = c(seq(-150000, -10000, 10000), 0, seq(6522, 146522, 10000)), 
    ...
    ggsave(filename = paste0("MT2_top100_AllStages_cummulativeFPKM_bins", tile_width, "_binPlot.pdf"), width = 30, height = 10)
  ...
  ggplot(all_coverage_counts_df_sum_fpkm_melt, aes(x = pos, y = fpkm)) +
    ...
    theme(axis.text.x = element_text(angle = 90, hjust = 1), 
          panel.grid.major = element_blank(),
          panel.grid.minor = element_blank()) +
    ggsave(filename = paste0("MT2_top100_AllStages_cummulativeFPKM_bins", tile_width, "_areaPlot.pdf"), width = 30, height = 10)
  ...
  ggplot(all_coverage_counts_df_sum_fpkm_melt, aes(x = pos, y = fpkm)) +
    ...
    theme(axis.text.x = element_text(angle = 90, hjust = 1), 
          ...
          panel.grid.minor = element_blank()) +
    facet_grid(stage ~ .) + 
    ggsave(filename = paste0("MT2_top100_AllStages_cummulativeFPKM_bins", tile_width, "_facetAreaPlot.pdf"), width = 30, height = 10)
}
plotCumFPKMBinArea(1)
