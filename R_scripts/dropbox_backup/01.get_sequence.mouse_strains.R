...
ids <- 
  ...
  unlist(.) %>% 
  magrittr::set_names(., ifelse(names(.) == "ensembl_gene_id", "m129s1svimj", str_replace(names(.), "_.+", ""))) %>% 
  .[!duplicated(.)]
...
introns_coord <- 
  ...
  dplyr::left_join(., tibble(dataset = animals,
                   ...
                   by = "dataset") %>% 
  dplyr::filter((dataset == "mbalbcj" & intron_index == "4_5") | (dataset != "mbalbcj" & intron_index == intron_filt))

# save coordinates
introns_coord %>% 
  dplyr::mutate(intron3_4_coordinates = str_c(chromosome_name, ":", start, "-", end)) %>% 
  dplyr::select(sci_name = animal_name, ensembl_dataset = dataset, intron3_4_coordinates, strand) %T>%
  readr::write_csv(., "Ago2.intron3_4.mouse_strains.csv")
...
# set names, order
names(ensembl_seq) <- 
  str_replace(seq_data$animal_name, "/", "_") %>% 
  str_c(., gene_symbol, str_c("intron", seq_data$intron_index), sep = '.')

# write sequences
ensembl_seq %<>% 
  .[order(width(.))] %>% 
  .[(letterFrequency(., letters = "N", as.prob = T) %>% as.vector(.)) < 1]

# write in loop
for(index in 1:length(ensembl_seq)){
  writeXStringSet(ensembl_seq[index], file.path(outdir, str_c("m", names(ensembl_seq[index]), "fa", sep = ".")), format = "fasta")
}
