...
# sample table path
sample_table_path <- "/common/WORK/fhorvat/Projekti/Svoboda/Analyses/retrotransposon_expression/developmental_stages/developmental_stages.RNAseq.sample_table.20190501.csv"

# datasets path
datasets_path <- "/common/WORK/fhorvat/Projekti/Svoboda/Analyses/retrotransposon_expression/developmental_stages/datasets"

# experiment list
experiment_list <- "Fugaku"
...
sample_table <- 
  ...
  mutate(sample_id = str_remove(sample_id, "\\.total\\.bam|\\.bam")) %>% 
  as.data.table(.)
...
# clean stage names
stage_clean <- 
  tibble(stage = c("GV.WE.PE", "MII.WE.PE", "1cell.WE.PE", "2cell.WE.PE", "4cell.WE.PE", "Blast.WE.PE", "Molura.WE.PE"), 
         stage_clean = c("GV", "MII", "1-cell", "2-cell", "4-cell", "Blastula", "Morula")) %>%
...
pheatmap::pheatmap(rpm_heatmap_tb,
                   ...
                   filename = file.path(outpath, str_c("heatmap.rmsk.L1_and_LTRs.20190502.Fugaku.RPM.png")),
                   height = 20,
                   width = 20)
