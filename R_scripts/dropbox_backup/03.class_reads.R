...
experiment_name <- "Zuzka_3T3_PAPD7"
...
# add read group
read_hits_sum <- read_hits_sum[ , read_group := ifelse("rRNA" %in% gene_biotype, "rRNA",
                                                              ...
                                                              ifelse("SINE" %in% gene_biotype, "SINE",
                                                                     ifelse("LTR" %in% gene_biotype, "LTR",
                                                                            ifelse("other_repeat" %in% gene_biotype, "other_repeat",
                                                                                   ifelse("miRNA" %in% gene_biotype, "miRNA",
                                                                                          ifelse("lincRNA" %in% gene_biotype, "lincRNA",
                                                                                                 ifelse("protein_coding" %in% gene_biotype, "protein_coding",
...
read_hits_final <-
  ...
  readr::write_csv(x = ., path = file.path(outpath, str_c("smallRNA.21to23nt", experiment_name, bam_name, "csv", sep = ".")))
