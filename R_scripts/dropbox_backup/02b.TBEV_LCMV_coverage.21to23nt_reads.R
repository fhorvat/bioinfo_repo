...
coverageScaffold <- function(bam_path, which_gr, on_minus_strand){
  ...
  # take only perfect reads
  bam_gr <- bam_gr[mcols(bam_gr)$nM == 0]
...
ggsave(plot = coverage_plot,
       filename = file.path(outpath, str_c("TBEV_coverage.21to23nt_perfect_reads.whole_virus.RPM.png", sep = ".")),
       width = 8,
       height = 15)
...
# save
ggsave(plot = coverage_plot,
       filename = file.path(outpath, str_c("TBEV_coverage.21to23nt_perfect_reads.zoom.whole_virus.RPM.png", sep = ".")),
       width = 8,
       height = 15)
