...
# results path
results_path.8.5dpp <- list.files(expression_path, "all_biotype.*\\.all_results\\.xlsx$", full.names = T, recursive = T)

# significant results path 
results_path.8.5dpp.sign <- list.files(expression_path, "all_biotype.*\\.significant_results\\.xlsx$", full.names = T, recursive = T)
...
# results path
results_path.13dpp <- list.files(expression_path, "all_biotype.*\\.all_results\\.xlsx$", full.names = T, recursive = T)
...
# save plot
ggsave(filename = file.path(outpath, str_c(table_name, "Mov10l_KO_WT.8.5dpp_run2_vs_13dpp.log2_ratio.DESeq2.crosshair", "jpg", sep = ".")),
       plot = cross_plot,
       width = 10, height = 10)
