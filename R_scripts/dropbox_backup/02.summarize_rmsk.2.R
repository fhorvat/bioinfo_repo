...
repclass_plot_df <- 
  ...
  dplyr::mutate(repClass = replace(repClass, repClass == "Simple_repeat", "simple repeat"), 
                total_fraction = total_fraction * 100, 
                repClass = ifelse(repClass %in% repClass_categories, repClass, "other repeat")) %>% 
  dplyr::group_by(repClass, experiment_id) %>% 
  dplyr::summarise(total_fraction = sum(total_fraction)) %>% 
  dplyr::ungroup(.) %>% 
  dplyr::left_join(., experiment_table, by = "experiment_id") %>% 
  dplyr::mutate(repClass = factor(repClass, levels = c(repClass_categories, "other repeat")), 
                total_fraction = round(total_fraction, 1))

# plot
ggplot(data = repclass_plot_df, mapping = aes(repClass, total_fraction, fill = animal)) +
  geom_bar(stat = "identity", position = position_dodge2(width = 1)) +
  geom_text(mapping = aes(label = str_c(total_fraction, "%"), y = total_fraction, color = animal), 
  ...
  ggsave(file.path(outpath, "repClass_percent.total_fraction.png"), width = 15, height = 10)
