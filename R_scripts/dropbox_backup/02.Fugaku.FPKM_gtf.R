...
### RUN: qsub -q MASTER -M fihorvat@gmail.com -m n -N pbs.Rscript.summarizeOverlaps -l select=ncpus=10:mem=100g -j oe 02.Fugaku.FPKM_gtf.R
...
######################################################## READ DATA
# # read ENSEMBL gtf
# txdb <- makeTxDbFromGFF(file = gtf_path, format = "gtf", circ_seqs = character())
...
# exons_gr <- GenomicFeatures::exonsBy(x = txdb, by = "tx", use.names = T)
# saveRDS(exons_gr, file = file.path(outpath, "ensembl.GRCm38.89.exonsByTx.RDS"))
exons_gr <- readRDS(file = file.path(outpath, "ensembl.GRCm38.89.exonsByTx.RDS"))
...
#                                            singleEnd = FALSE,
#                                            ignore.strand = TRUE)
# saveRDS(se, file = file.path(outpath, "ensembl.GRCm38.89.tx.Fugaku.summarizedOverlaps.RDS"))

# read summarizedExperiment from RDS file
se <- readRDS(file = file.path(outpath, "ensembl.GRCm38.89.tx.Fugaku.summarizedOverlaps.RDS"))
...
fpkm_df <-
  ...
  readr::write_csv(., path = file.path(outpath, "ensembl.GRCm38.89.tx.Fugaku.FPKM.csv"))
