...
setwd("/common/WORK/fhorvat/Projekti/Svoboda/smallRNASeq_2018/Analysis/MosIR_expression/whole_plasmid")
...
for(experiment in experiment_list){
  ...
  for(bam_path in bam_paths){
    ...
    plot_df <- 
      rbind(coverageMosIR(bam_path = bam_path, which_gr = plasmid_gr, on_minus_strand = F) %>% 
                       dplyr::mutate(strand = "plus"), 
                     coverageMosIR(bam_path = bam_path, which_gr = plasmid_gr, on_minus_strand = T) %>% 
    ...
    ggsave(plot = coverage_plot,
           filename = file.path(outpath, experiment, str_c(experiment, bam_name, "MosIR_coverage.whole_plasmid.RPM.lim.png", sep = ".")),
           width = 15,
           height = 10)
