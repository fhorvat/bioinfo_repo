...
GOBubble2 <- function(data, display, title, colour, labels, ID = T, table.legend = T, table.col = T, bg.col = F, pvalue_threshold = 0.01){
  ...
  if (display == "single") {
    ...
    if(ID){
      g <- 
        g + 
        ggrepel::geom_text_repel(data = sub2, aes(x = zscore, y = adj_pval, label = id), size = 5, segment.alpha = 0)
    } else{
      g <- 
        g + 
        ggrepel::geom_text_repel(data = sub2, aes(x = zscore, y = adj_pval, label = term), size = 4, segment.alpha = 0)
    } 
    ...
    if(ID){
      ...
      g <- 
        g + 
        ggrepel::geom_text_repel(data = sub2, aes(x = zscore, y = adj_pval, label = term), size = 5, segment.alpha = 0) + 
        theme(axis.title = element_text(size = 14, face = "bold"), 
              axis.text = element_text(size = 14),
              axis.line = element_line(colour = "grey80"),
              axis.ticks = element_line(colour = "grey80"),
...
# enrichment results path
go_enrich_list_path <- file.path(inpath, "go_enrich.RDS")
...
go_enrich_list <- readRDS(go_enrich_list_path)
...
purrr::map(stages, function(stage){
  ...
  genes_info_df <-
    ...
    dplyr::rename(padj = padj, ID = gene_id, logFC = log2FoldChange)
  ...
  # png(file = file.path(outpath, str_c("GOEnrich.bubble.single.table", stage, "png", sep = ".")),
  #     width = 1800, 
  #     height = 1000, 
  #     units = "px", 
  #     type = "cairo")
  ...
  #           bg.col = F,
  #           title = str_c("Enriched GO terms - ", stage))
  # dev.off()
  
  # GO bubble plot facet
  png(file = file.path(outpath, str_c("GOEnrich.bubble.facet", stage, "png", sep = ".")),
      width = 1000, 
      height = 1000, 
      units = "px", 
      type = "cairo")
