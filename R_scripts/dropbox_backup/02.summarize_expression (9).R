...
######################################################## WORKING DIRECTORY
setwd("/common/WORK/fhorvat/Projekti/Svoboda/piRNA.Zuzka/Analysis/2020_paper/retrotransposons_annotation/golden_hamster.Siomi_assembly/raw_rmsk.expression/hamster_testis_Mov10l.RNAseq")
...
rpm_tb <- 
  ...
  dplyr::select(sample_id, repName, RPM)

# join with sample table, summarize per genotype
rpm_mean <- 
  rpm_tb %>% 
  dplyr::left_join(., sample_tb, by = "sample_id") %>% 
  dplyr::group_by(genotype, age, repName) %>% 
  dplyr::summarise(RPM = round(mean(RPM), 3)) %>% 
  dplyr::ungroup(.) %>% 
  dplyr::filter(!str_detect(genotype, "HET")) 

# save
readr::write_csv(rpm_mean, file.path(outpath, "rmsk.Siomi.20200701.raw.SINE_LINE_LTRs.testis.RNAseq.RPM.csv"))
