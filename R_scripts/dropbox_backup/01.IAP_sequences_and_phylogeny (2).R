...
# ### plot histogram of ORF widths
# # get long table
# iap_orf_width <- 
#   iap_tb %>% 
#   dplyr::select(rmsk_id, ltr_name, longest_orf_1, longest_orf_2) %>% 
#   tidyr::pivot_longer(cols = -c(rmsk_id, ltr_name), names_to = "orf", values_to = "orf_width")
# 
# # plot
# hist_width <-
#   ggplot(data = iap_orf_width, aes(orf_width, color = orf)) +
#   geom_histogram(binwidth = 1) +
#   facet_wrap(vars(ltr_name, orf), nrow = 11, ncol = 2, scales = "free_y") +
...
# 
# # save
# ggsave(filename = file.path(outpath, "IAP.potentially_young.ORF_width.hist.png"), plot = hist_width, width = 10, height = 20)

# filter based on ORF length
iap_tb %<>% 
  dplyr::filter(longest_orf_1 > 2000) %>% 
  dplyr::filter(longest_orf_2 > 1500)

# save as table
readr::write_csv(iap_tb, file.path(outpath, "IAP.potentially_young.filtered_by_ORFs.csv"))
...
# save as .bed
rtracklayer::export.bed(iap_gr, file.path(outpath, "IAP.potentially_young.filtered_by_ORFs.bed"))

# get sequences
iap_seq <- getSeq(BSgenome.Mmusculus.UCSC.mm10, iap_gr)

# # save as fasta
# Biostrings::writeXStringSet(x = iap_seq, filepath = file.path(outpath, "IAP.potentially_young.filtered_by_ORFs.fasta"))
...
iap_seq_consensus <- purrr::map(names(iap_seq_list), function(seq_name){
  ...
  # write as fasta
  seq_msa@unmasked %>%
    Biostrings::writeXStringSet(., file = file.path(outpath, str_c("IAP.potentially_young.filtered_by_ORFs", seq_name, "msa.fasta", sep = ".")))
...
# save the consensus
Biostrings::writeXStringSet(iap_seq_consensus, file = file.path(outpath, str_c("IAP.potentially_young.filtered_by_ORFs", "consensus.fasta", sep = ".")))
