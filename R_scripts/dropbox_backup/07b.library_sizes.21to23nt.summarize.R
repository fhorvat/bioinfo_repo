...
for(experiment in experiment_list){
  ...
  # library size path
  library_size_path <- list.files(path = file.path(mapped_path, "6_filtered_21to23nt"), pattern = "library_hist.*mis_[0-2]{1}.21to23nt.txt", full.names = T)
  
  # class reads table path
  class_reads_path <- file.path("/common/WORK/fhorvat/Projekti/Svoboda/smallRNASeq_2018/Analysis/class_reads", str_c("class_reads.", experiment, ".21to23nt.counts.xlsx"))
    
  # class reads table path
  rmsk_class_reads_path <- file.path("/common/WORK/fhorvat/Projekti/Svoboda/smallRNASeq_2018/Analysis/class_reads", str_c("rmsk.class_reads.", experiment, ".21to23nt.counts.xlsx"))
  ...
  library_size_df <-
    ...
    dplyr::summarise(sum_21to23nt = sum(alignment_count[(alignment_length >= 21) & (alignment_length <= 23)])) %>% 
    dplyr::ungroup(.) %>% 
    ...
    dplyr::select(sample_id, str_c("mis_", 0:2, ".sum_21to23nt")) %>% 
  ...
  # write to repeatMasker class reads results
  write.xlsx(x = library_size_df %>%
               dplyr::select(sample_id, mis_0.21to23nt_reads) %>% 
               as.data.frame(.),
             file = rmsk_class_reads_path,
             sheetName = "library_sizes",
             append = TRUE,
             row.names = FALSE)
