...
######################################################## WORKING DIRECTORY
setwd("/common/WORK/fhorvat/Projekti/Svoboda/RNAi.Eliska/DicerX_viral_infection/datasets/2019_Dec/Analysis/expression/smallRNA_clusters.without_rRNA_tRNA")
...
counts_path <- "/common/WORK/fhorvat/Projekti/Svoboda/RNAi.Eliska/DicerX_viral_infection/datasets/2019_Dec/Analysis/expression/smallRNA_clusters"
l.files <- list.files(counts_path, full.names = T, pattern = ".*filt.RData")
# l.files <- l.files[!str_detect(l.files, "KO_7B_r4|KO_2_r5")]
...
# annotation RDS path
annot_path <- file.path(inpath, "l.annot.rRNA_tRNA.RDS")
...
# 
# # save
# saveRDS(clust_list, file.path(outpath, str_c("clusters.DicerX_embryos", "all_samples", "rpm", rpkm1, "width", width, "RDS", sep = ".")))

# # read clusters
# clust_list <- readRDS(file.path(outpath, str_c("clusters.DicerX_embryos", "all_samples", "rpm", rpkm1, "width", width, "RDS", sep = ".")))


### merge clusters in replicates - WT vs. KO
# merge clusters from WT replicates (intersect)
WT_samples <- names(clust_list)[str_detect(names(clust_list), "WT")]
WT_list <- clust_list[WT_samples]
WT_clust <- purrr::reduce(WT_list, ClusterMerge, method = "intersect")

# merge clusters from KO replicates (intersect)
KO_samples <- names(clust_list)[str_detect(names(clust_list), "KO")]
KO_list <- clust_list[KO_samples]
...
do <- 
  ...
  tibble::as_tibble(.) %T>%
  readr::write_csv(., file.path(outpath, str_c("clusters.DicerX_embryos", "all_samples", "WT", "KO", "union", "rpm", rpkm1, "width", width, "csv", sep = ".")))
...
do_mean <- 
  ...
  readr::write_csv(., file.path(outpath, str_c("clusters.DicerX_embryos", "all_samples", "WT", "KO", "union", "rpm", rpkm1, 
...
plot_df <- 
  do_long %>% 
  # dplyr::filter(class != "RNA_rRNA_tRNA") %>%
  dplyr::mutate(class = replace(class, class == "not_annotated", "siRNA_control"), 
                class = factor(class, levels = c("siRNA_control", "miRNA", "TE", "mRNA", "RNA_other", 
                                                 "RNA_rRNA_tRNA", 
                                                 "other"))) %>% 
  dplyr::filter(!(1:nrow(.) %in% dicer_hits)) %>%
  dplyr::arrange(desc(class)) %>% 
  dplyr::select(coordinates, width, strand, class, x = x_sample, y = y_sample, class) %>%
...
# save table
plot_df %>% 
  dplyr::select(coordinates, width, strand, class, mean_RPM = mean, log2FoldChange = lfc, WT.RPM = y, KO.RPM = x) %>% 
  dplyr::filter(!str_detect(coordinates, "antiEGFP|antiGL3|antiLuc")) %>% 
  dplyr::arrange(desc(log2FoldChange)) %T>% 
  readr::write_csv(., file.path(outpath, str_c("clusters", "DicerX_embryos", "all_samples", "diff_exp", "WT", "KO", "union", "rpm", rpkm1,
                                               "width", width, "mean_RPM", "csv", sep = ".")))
...
# save
ggsave(filename = file.path(outpath, str_c("clusters", "DicerX_embryos", "all_samples", "MA_plot", "WT", "KO", "union", "rpm", rpkm1,
...
#                                            "width", width, "mean_RPM", filter_name, "without_rRNA_tRNA", "png", sep = ".")), 
#        plot = clusters_plot,
#        width = 12, height = 12)
