...
######################################################## WORKING DIRECTORY
# set working dir
setwd("/common/WORK/fhorvat/Projekti/Svoboda/piRNA.Zuzka/Analysis/2020_paper/retrotransposons_annotation/golden_hamster.Siomi_assembly/MYSERV6/expression/small_RNAseq")
...
dataset_name_list <- c("hamster_testis_Mov10l.8.5dpp.run_2.small_RNAseq")
...
purrr::map(dataset_name_list, function(dataset_name){
  ...
  inpath <- file.path(inpath, "MYSERV6/expression/small_RNAseq", dataset_name, "expression_sum.RDS_files")
  ...
  expression_tb_full <- purrr::map(expression_path, function(path){
    ...
    dplyr::bind_rows(.) %>% 
    dplyr::mutate(repName = "MYSERV6_full")
  ...
  ### clean data
  # get right normalization factor in library size table
  library_size_tb %<>% 
  ...
  counts_tb_list <- list(MYSERV6_full = expression_tb_full)
  ...
  RPM_summarized <- purrr::map(names(counts_tb_list), function(subset_name){
    ...
    rpm_tb %>% 
      ...
      readr::write_csv(file.path(outpath, str_c(subset_name, dataset_name, "RPM", "sense_antisense", "read_length.histogram", "20201104", "csv", sep = ".")))
