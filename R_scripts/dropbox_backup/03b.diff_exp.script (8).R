...
single_end=TRUE
threads=1
mapped_path='/common/WORK/fhorvat/Projekti/Svoboda/piRNA.Zuzka/datasets/2019_Dec.Papd7/Data/Mapped/STAR_mm10'
documentation_path='/common/WORK/fhorvat/Projekti/Svoboda/piRNA.Zuzka/datasets/2019_Dec.Papd7/Data/Documentation'
...
grouping_variables=c("tissue", "genotype")
...
# FPKM path
fpkm_path <- list.files(inpath, str_c(features_name, "\\.FPKM_mean\\.csv$"), full.names = T)
...
sample_table_dds <-
  ...
  .[sample_id != "s_GV_Ago2_HET_30330_r1.SE", ] %>%
  as.data.frame(.) %>%
  ...
  results_list <- purrr::map(results_groups, function(result){
    ...
    if((length(result_clean) == 2) & (all(result_clean %in% sample_table_dds$grouped_variables))){
      ...
      results_df <-
        ...
        dplyr::arrange(padj) %>%
        dplyr::left_join(fpkm_tb %>% dplyr::select(-one_of(sample_table_dds %>%
