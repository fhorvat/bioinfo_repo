...
# loaded_variables <- grep("s_.*_bam|s_.*_coverage|repeatMasker", ls(), value = T)
...
positionCoverageForPlot <- function(sample_name){
  
  # get individual sample coverage
  element_coverage <- all_coverage_list[[sample_name]]
...
# .bam files
s_GV_bam <- readGAlignments(filenames[1])
s_MII_bam <- readGAlignments(filenames[2])
s_4c_bam <- readGAlignments(filenames[3])
s_8c_bam <- readGAlignments(filenames[4])
s_16c_bam <- readGAlignments(filenames[5])
s_Blast_bam <- readGAlignments(filenames[6])

# coverage of BAM files
s_GV_coverage <- coverage(s_GV_bam)
s_MII_coverage <- coverage(s_MII_bam)
s_4c_coverage <- coverage(s_4c_bam)
s_8c_coverage <- coverage(s_8c_bam)
s_16c_coverage <- coverage(s_16c_bam)
s_Blast_coverage <- coverage(s_Blast_bam)
...
genes_ranges <- 
  ...
  genes(.)
names(mcols(genes_ranges)) <- "repName"

# making TxDb object from genes gtf, getting genes 
genes_ranges_2 <- 
  makeTxDbFromGFF("/common/WORK/fhorvat/reference/cow/bosTau7/UCSC_augustusGene_bosTau7_20170206.gtf.gz") %>% 
  genes(.)
names(mcols(genes_ranges_2)) <- "repName"
...
######################################################################### getting top 100 ERV1-2-LTR_BT in 8C
# # take only chosen LTR filtered by consensus length +- 5%
# repeatMasker_filtered <- 
#   repeatMasker %>% 
#   as.data.frame() %>% 
#   dplyr::filter(repName %in% "ERV1-2-LTR_BT") %>% 
...
# # filter by consensus length
# as.data.frame() %>% 
# dplyr::filter((width > (447 - 0.05 * 447)) & (width < 447 + 0.05 * 447)) %>% 
# makeGRangesFromDataFrame(., keep.extra.columns = T)
...
# # RPKM in 8C from counts
# rpmk_ordered_LTR_df <-
#   assay(se) %>% 
#   as.data.frame() %>%
#   mutate(sample = "s_8c") %>% 
#   cbind(as.data.frame(repeatMasker_filtered)) %>% 
#   left_join(library_size_df, by = "sample") %>% 
#   mutate(RPKM_in_8c = (reads / (library_size * (width / 1000)))) %>% 
#   dplyr::select(-c(width, library_size, sample, reads, repName)) %>% 
#   arrange(desc(RPKM_in_8c)) %T>% 
#   write_csv(path = "ERV1_2_LTR_BT_RPKM_in_8C_Graf2014.csv", col_names = T) 

# read from table, remove some undefined DNA
all_elements_original <- 
  read_csv(file = "/common/WORK/fhorvat/Projekti/Svoboda/Analyses/GenomeResearch_2016/review/cow_expression/ERV1_2_LTR_BT_RPKM_in_8C_Graf2014.csv") %>% 
  dplyr::filter(!str_detect(seqnames, "Un")) %>% 
  dplyr::rename(FPKM = RPKM_in_8c)
...
all_elements_expanded_ranges <- 
  all_elements_original_ranges %>% 
  as.data.frame() %>% 
...
# finding overlaps
all_elements_overlaps <- findOverlaps(all_elements_expanded_ranges, all_elements_original_ranges)

# combining both in data.frame, removing self-overlaps
all_elements_overlaps_df <- 
  data.frame(fullName_original = all_elements_original_ranges[subjectHits(all_elements_overlaps)]$fullName,
             fullName_expanded = all_elements_expanded_ranges[queryHits(all_elements_overlaps)]$fullName, 
             stringsAsFactors = F) %>% 
  dplyr::filter(fullName_original != fullName_expanded) %>% 
  left_join(., all_elements_original[, c("fullName", "FPKM")], by = c("fullName_original" = "fullName")) %>% 
  dplyr::rename(FPKM_original = FPKM) %>% 
  left_join(., all_elements_original[, c("fullName", "FPKM")], by = c("fullName_expanded" = "fullName")) %>% 
  dplyr::rename(FPKM_expanded = FPKM) %>% 
  mutate(original_smaller = FPKM_original < FPKM_expanded) 

# get names of elements with smaller FPKM
picked_element_names <- 
  c(all_elements_overlaps_df %>% filter(original_smaller) %$% fullName_original, 
    all_elements_overlaps_df %>% filter(!original_smaller) %$% fullName_expanded) %>% 
  unique()

# filtering elements from original table 
all_elements_original_ranges <- all_elements_original_ranges[!(all_elements_original_ranges$"fullName" %in% picked_element_names), ]

##################################################################################### filtering based on distance to nearest gene  
# find distance to nearest gene 
distance_to_genes <- distanceToNearest(all_elements_original_ranges, genes_ranges, ignore.strand = T)
all_elements_original_ranges$distance_to_genes <- mcols(distance_to_genes)$distance

# setting parameters
experiment <- "Graf2014"
element_name <- "ERV1-2-LTR_BT"
distances <- c(0, 50000)
...
for(min_distance in distances){
  ...
  # setting file name
  file_name <- paste0(experiment, "_", element_name, "_distanceToGenes", (min_distance / 1000), "kb_top", length(element_original_ranges))
  file_name <- paste0(experiment, "_", element_name, "_refSeqAndAUGUSTUSfilteredGenes_top", length(element_original_ranges))
  
  ##################################################################################### filtering with repeatMasker and/or genes
  # the problem is that some of the elements are closer than 150kb to each other
  # They have to be filtered, but original element ranges have to be conserved 
  # in each of 100 expanded ranges
  
  # removing everything included in repeatMasker (including original element ranges)
  genomic_filter <- c(repeatMasker, genes_ranges, genes_ranges_2)
  seqlevels(genomic_filter, force = T) <- seqlevels(s_GV_bam)
  
  element_genomic_filtered_list <- 
  ...
  # creating GRlist from original element ranges
  element_original_ranges_list <- split(element_original_ranges, element_original_ranges$fullName)
  element_original_ranges_list <- element_original_ranges_list[match(names(element_genomic_filtered_list), names(element_original_ranges_list))]
  
  # returning original element ranges to filtered ranges, but only element belonging to its expanded range
  element_genomic_filtered_df <- 
    GenomicRanges::union(element_genomic_filtered_list, element_original_ranges_list, ignore.strand = T) %>% 
  ...
  element_genomic_filtered_fpkm <- 
    lapply(list(s_GV_bam, s_MII_bam, s_4c_bam, s_8c_bam, s_16c_bam, s_Blast_bam), 
           FUN = countFunction, 
  ...
  # GV
  s_GV_element_coverage <- 
    lapply(X = 1:length(element_fpkm_filtered[["s_GV"]]), 
  ...
  # MII
  s_MII_element_coverage <- lapply(X = 1:length(element_fpkm_filtered[["s_MII"]]), 
                                   function(X) coverageForSummedPlot(X, 
                                                                     sample_coverage = s_MII_coverage, 
                                                                     sample_name = "s_MII", 
                                                                     element_grList = element_fpkm_filtered)) 
  ...
  # 8-cell
  s_8c_element_coverage <- lapply(X = 1:length(element_fpkm_filtered[["s_8c"]]), 
                                  function(X) coverageForSummedPlot(X, 
                                                                    sample_coverage = s_8c_coverage, 
                                                                    sample_name = "s_8c", 
                                                                    element_grList = element_fpkm_filtered)) 
  
  # 16-cell
  s_16c_element_coverage <- lapply(X = 1:length(element_fpkm_filtered[["s_16c"]]), 
                                   function(X) coverageForSummedPlot(X, 
                                                                     sample_coverage = s_16c_coverage, 
                                                                     sample_name = "s_16c", 
                                                                     element_grList = element_fpkm_filtered)) 
  
  # Blastula
  s_Blast_element_coverage <- lapply(X = 1:length(element_fpkm_filtered[["s_Blast"]]), 
                                     function(X) coverageForSummedPlot(X, 
                                                                       sample_coverage = s_Blast_coverage, 
                                                                       sample_name = "s_Blast", 
                                                                       element_grList = element_fpkm_filtered)) 

  ######################################################################################################## plot individual elements coverage
  # put all coverage files to one named list
  all_coverage_list <-
    list(s_GV_element_coverage, s_MII_element_coverage, s_4c_element_coverage, s_8c_element_coverage, s_16c_element_coverage, s_Blast_element_coverage) %>%
    set_names(library_size_df$sample)

  # get element order
  element_order <- names(element_fpkm_filtered[["s_GV"]])

  # get one big position matrix with rbinded position matrix of all samples
  position_matrix_all <-
    lapply(X = names(all_coverage_list),
           FUN = positionCoverageForPlot) %>%
    bind_rows(.)

  # plot each individual column (=one element)
  plot_list <- lapply(X = 1:length(element_order), FUN = function(X){

    # create data.frame for plot
    element_coverage_for_plot <-
      position_matrix_all[, c("pos", X, "sample")] %>%
      set_colnames(c("pos", "count", "sample")) %>%
      mutate(element = ifelse(pos >= 0 & pos <= element_width, "in_element", "out_element"),
    ...
    # get plot
    ylim_value <- 100
    element_coverage_plot <-
      ggplot(element_coverage_for_plot, aes(x = pos, y = count)) +
      ...
      theme(axis.text.x = element_text(angle = 90, hjust = 1),
            panel.grid.major = element_blank(),
            panel.grid.minor = element_blank())  +
      ggtitle(element_order[X]) +
      facet_grid(sample ~ .)


    return(element_coverage_plot)

  })

  pdf(file = paste0("./results/", file_name, "_ylim", ylim_value, "_individualElementsCoverage.pdf"), width = 30, height = 10)
  ...
  element_coverage_all_df <- 
    lapply(list(s_GV_element_coverage, s_MII_element_coverage, s_4c_element_coverage, s_8c_element_coverage, s_16c_element_coverage, s_Blast_element_coverage), 
           FUN = positionCoverageDF) %>% 
    bind_rows(.) %>% 
  ...
  for(ylim_values in c(100)){
    ...
    ggplot(element_coverage_all_df, aes(x = pos, y = count)) +
      ...
      theme(axis.text.x = element_text(angle = 90, hjust = 1), 
            ...
            panel.grid.minor = element_blank()) +
      facet_grid(sample ~ .) +
      ggsave(filename = paste0("./results/", file_name, "_", "ylim", ylim_values, "_summedCoverage.pdf"), width = 30, height = 10)
