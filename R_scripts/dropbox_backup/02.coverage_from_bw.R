...
######################################################## WORKING DIRECTORY
setwd("/common/WORK/fhorvat/Projekti/Svoboda/accessory_data_sets/assemblies/golden_hamster.mesAur1/Mapped/CriGriPICR/LINE1/assemble_individual_LINE1s/LINE1.CriGriPICR.5K_to_7k.ORFs.split/L1_1_CGr.1052918")
...
coverage_tb <- 
  coverage_tb_full %>% 
  dplyr::mutate(sample_id = str_remove_all(sample_id, "\\.24to31nt\\.bam|\\.bam|s_mesAur_|.merged|.bw"))
...
ggsave(plot = baseplot, 
       filename = file.path(outpath, str_c("coverage", "bw", "png", sep = ".")), 
       width = 15, height = 7.5)
