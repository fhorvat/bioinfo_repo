...
# hamster path 
hamster_path <- "/common/WORK/fhorvat/Projekti/Svoboda/hamster_KO/datasets/hamster_testis_Mov10l.small_RNAseq/Data/Mapped/STAR_mesAur1"

# mouse path
mouse_path <- "/common/WORK/fhorvat/Projekti/Svoboda/hamster_KO/datasets/mouse_testis.small_RNAseq/Data/Mapped/STAR_mm10"

# list library sizes files
library_path <- list.files(c(hamster_path, mouse_path), pattern = "library_hist\\..*\\.SE\\.perfect\\.txt", full.names = T)
...
read_length_plot <- 
  ...
  ggsave(filename = file.path(outpath, "read_length.histogram.perfect.png"), width = 10, height = 15)
...
read_length_plot <- 
  ...
  theme(axis.text.x = element_text(angle = 90, hjust = 1, size = 12, vjust = 0.5),
        ...
        panel.grid.minor = element_blank(),
        legend.position = "bottom") +
  ggsave(filename = file.path(outpath, "read_length.stacked_barplot.perfect.png"), width = 10, height = 10)
