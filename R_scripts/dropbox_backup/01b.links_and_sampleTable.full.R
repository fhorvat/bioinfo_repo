...
# raw files path
raw_path <- "/common/RAW/Svoboda/golden_hamster.testis.smallRNAseq.reseq.2020/joined_lanes"
...
sample_tb <- 
  ...
  dplyr::mutate(sample_id = str_c("s", stage, genotype, age, sample_name, str_c("r", replicate), sep = "_") %>% str_c(., ".reseq.SE")) %>% 
...
sample_tb %>% 
  dplyr::select(sample_id, stage, genotype, age, sample_name, replicate, file_name) %>% 
  dplyr::distinct(.) %>% 
