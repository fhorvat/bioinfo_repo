...
### INFO: get expression of all genes in Dang data
...
######################################################## PATH VARIABLES
# set experiment name
experiment <- "Hendrickson_2017_NatGenet_GSE72379"
experiment_name <- "Hendrickson2017"
...
mapped_path <- file.path("/common/WORK/fhorvat/Projekti/Svoboda/accessory_data_sets/RNAseq", experiment, "/Data/Mapped/STAR_hg38")
...
se <- readRDS(file = file.path(outpath, str_c("ensembl.GRCh38.91.20180312.UCSCnames.clean.", experiment_name, ".se.RDS")))
...
fpkm_avg <-
  ...
  readr::write_csv(., path = file.path(outpath, str_c("ensembl.GRCh38.91.20180312.UCSCnames.clean.", experiment_name, ".avgFPKM.csv")))
...
fpkm_avg_filt <-
  ...
  readr::write_csv(., path = file.path(outpath, str_c("CNOT.GRCh38.91.20180312.UCSCnames.clean.", experiment_name, ".avgFPKM.csv")))
...
FPKM_boxplot <- 
  ...
  dplyr::mutate(gene_name = factor(gene_name, levels = str_c("Cnot", c(1:6, "6l", 7:11))),
                stage = factor(stage, levels = c("GV", "MI", "MII", "1C", "2_4_8C", "Morula", "ICM", "troph"))) %>%
  ...
  ggsave(filename = file.path(outpath, "results", str_c("boxplot.CNOT.GRCh38.91.20180312.UCSCnames.clean.", experiment_name, ".FPKM.png")), 
         plot = FPKM_boxplot, height = 20, width = 10, limitsize = F)
...
se_filt$stage <- factor(se_filt$stage, levels = c("GV", "MI", "MII", "1C", "2_4_8C", "Morula", "ICM", "troph"))
...
PCA_plot <- 
  ...
  dplyr::left_join(sample_table_dds %>% dplyr::select(-bam_path), by = "sample_id") %>%
  dplyr::mutate(sample_id = str_replace(sample_id, "^s_", "") %>% str_replace(., "_r", "."), 
                stage = factor(stage, levels = c("GV", "MI", "MII", "1C", "2_4_8C", "Morula", "ICM", "troph"))) %>%
...
# save plot
ggsave(filename = file.path(outpath, "results", str_c("PCAplot.", experiment_name, ".GRCh38.91.rlog.png")),
...
# save plot
ggsave(filename = file.path(outpath, "results", str_c("PCAplot.", experiment_name, ".GRCh38.91.logFPKM.png")),
       plot = PCA_plot, width = 10, height = 10)
