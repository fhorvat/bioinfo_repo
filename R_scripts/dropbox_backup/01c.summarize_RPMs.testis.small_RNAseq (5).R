...
rpm_tb <- purrr::map(rpm_paths, function(path){
  ...
  tidyr::pivot_wider(., id_cols = subset, names_from = c(genotype_age, sense), values_from = RPM.24to31nt, names_prefix = "RPM.24to31nt.") %>%
  dplyr::mutate(subset = replace(subset, subset == "intact_guys", "IAP_FLIs")) %T>%
  readr::write_csv(., file.path(outpath, "IAPs_subsets.testis.8.5dpp.small_RNAseq.RPM.split_sense_antisense.complete.csv"))
