...
changeWD("/common/WORK/fhorvat/Projekti/Svoboda/Other/MT_elements/review/LTR_alignment/files/seq_logo/iterations_noGaps_globalMedian")
  ...
  repeat{
    ...
    gap_index <- 
      ...
      mutate(left = lag(gap), 
             right = lead(gap), 
             id = rownames(.), 
             count = consensusMatrix(sub_sequences_aligned, as.prob = F, baseOnly = T)[5, ]) %>% 
      dplyr::filter(left == F & gap == T & right == F & count > 175) %$%
      id %>% 
      gsub("V", "", .) %>% 
      as.integer()
...
signal_start_median <- 
  ...
  mutate(start_med = replace(start_med, LTR_class == "MTE", 308))
