...
######################################################## WORKING DIRECTORY
setwd("/common/WORK/fhorvat/Projekti/Svoboda/piRNA.Zuzka/Analysis/2020_paper/retrotransposons_annotation/golden_hamster.Siomi_assembly/FLI_elements/IAP/methylation/Bismark_nondirect")
...
coverage_tb <- 
  coverage_tb_full %>% 
  dplyr::mutate(sample_id = str_remove_all(sample_id, "\\.converted_bismark_bt2.bw"))
...
ggsave(plot = baseplot, 
       filename = file.path(outpath, str_c("methylation_coverage", "bw", "png", sep = ".")), 
       width = 15, height = 7.5)
