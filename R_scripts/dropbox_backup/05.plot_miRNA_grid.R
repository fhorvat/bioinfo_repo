...
# grid path
grid_path <- file.path(inpath, "grid.Fugaku.Encode.binned_fpkm.relative_order.50.csv")
...
# prepare full grid
full_grid_df <- 
  as.tibble(expand.grid(1:50, 1:50)) %>% 
...
ggplot(match_2to8_smooth, aes(bin_absolute, bin_relative)) + 
  ...
  theme_tufte() +
  ggsave(filename = file.path(outpath, str_c("grid.50x50.relative_order.mmu-let-7d-5p.", k, "smooth.png")))
