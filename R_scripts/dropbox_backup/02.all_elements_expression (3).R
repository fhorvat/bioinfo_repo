...
######################################################## WORKING DIRECTORY
# set working dir
setwd("/common/WORK/fhorvat/Projekti/Svoboda/piRNA.Zuzka/Analysis/2020_paper/retrotransposons_annotation/golden_hamster.Siomi_assembly/FLI_elements/IAP/expression/hamster_testis_Mov10l.0.5dpp.RNAseq/individual_elements/expression")
...
expression_tb_full <- purrr::map(dataset_name_list, function(dataset_name){
  ...
  inpath <- file.path(inpath, "FLI_elements", "IAP", "expression", dataset_name, "individual_elements/expression", "expression_sum.RDS_files")
...
expression_tb_mean <- 
  ...
  dplyr::mutate(tissue = str_extract(sample_id, "testis|GV"), 
                ...
                age = str_extract(sample_id, "GV|0.5dpp|13dpp|21dpp|8.5dpp|adult|8.5"), 
  ...
  dplyr::mutate(tissue = factor(tissue, levels = c("oocyte", "testis")),
                age = factor(age, levels = c("GV", "0.5dpp", "8.5dpp", "8.5dpp.run_2", "13dpp", "21dpp", "adult")),
