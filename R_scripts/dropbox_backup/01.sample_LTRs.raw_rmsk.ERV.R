...
######################################################## WORKING DIRECTORY
# set working dir
setwd("/common/WORK/fhorvat/Projekti/Svoboda/piRNA.Zuzka/Analysis/2020_paper/retrotransposons_annotation/golden_hamster.Siomi_assembly/substitution_rate/random_sampled_LTRs.ERV")
...
# get LTRs with width >= 30% of consensus sequence
ltrs_filt <- 
  ltrs_raw %>% 
  dplyr::mutate(perc_align = 100 * round(genomeWidth / consensusWidth, 3))
  # dplyr::filter(perc_align >= 30)
...
# save table
readr::write_csv(ltr_sample_tb, file.path(outpath, "LTRs.random_50_per_repName.ERV.raw_rmsk.csv"))
...
purrr::map(names(ltr_sample_seq_list), function(ltr_name){
  ...
  ltr_sample_seq_list[[ltr_name]] %T>% 
    Biostrings::writeXStringSet(., file.path(outpath, str_c("LTRs.random_50_per_repName", ltr_name, "ERV.raw_rmsk.single.fasta", sep = ".")))
