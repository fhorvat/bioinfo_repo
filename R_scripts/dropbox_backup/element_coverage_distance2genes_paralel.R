...
setwd("/common/WORK/fhorvat/Projekti/Svoboda/Other/MT_elements/review/coverage_scanning/test_paralel/")

################################################################################### functions vfranke
# finds the chromosome names from the bam file 
...
# 
# # .bam files
# bam_GV <- GetReads(filenames[1])
# bam_1C <- GetReads(filenames[2])
# bam_2C <- GetReads(filenames[3])
# bam_2C_aphi <- GetReads(filenames[4])
# bam_4C <- GetReads(filenames[5])
...
# 
# # repeatMasker table
# repeatMasker <- read.delim("/common/WORK/fhorvat/reference/mm10/UCSC_repeatMasker_mm10_20161012.txt.gz", header = T, stringsAsFactors = F)
...
# knownGenesTxDb <- makeTxDbFromGFF("/common/WORK/fhorvat/reference/mm10/UCSC_knownGene_mm10_20161126.gtf.gz")
...
knownGenes_gtf <- read.delim("/common/WORK/fhorvat/reference/mm10/UCSC_knownGene_mm10_20161126.gtf.gz", sep = "\t")
...
MT2_ORR1A0_all <- read.delim("/common/WORK/fhorvat/Projekti/Svoboda/Other/MT_elements/coverage/MT2_ORR1A0_solo_LTRs_orderedByFPKMin2cell.txt", stringsAsFactors = F, header = T)[, -4]
MT2_full <- read.delim("/common/WORK/fhorvat/Projekti/Svoboda/Other/MT_elements/coverage/MT2_full_orderedByFPKMIn2cell.txt", stringsAsFactors = F, header = T)[, -c(4, 8)]
...
all_elements_original_gr$distance_to_genes <- mcols(distanceToNearest(all_elements_original_gr, knownGenes))$distance
...
# not working, getting "Error in { : task 1 failed - "could not find object element_original_ranges" " error

registerDoParallel(4)

x <- foreach(element_name = c("MT2_Mm", "MT2_full")) %:%
  
  foreach (min_distance = c(100000, 150000), .combine = "list", .export = c("all_elements_original_gr")) %dopar% {
    ...
    return(as.data.frame(element_genomic_filtered_gr))
    ...
    element_coverage_all_df$stage <- rep(c("GV", "1C", "2C", "2C_aphi", "4C"), each = (2 * 1.5e5 + element_width + 1))
    element_coverage_all_df$stage <- factor(element_coverage_all_df$stage, levels = c("GV", "1C", "2C", "2C_aphi", "4C"))
