...
######################################################## WORKING DIRECTORY
setwd("/common/WORK/fhorvat/Projekti/Svoboda/Analyses/rmsk_full_elements")
...
rmsk_df <- readr::read_table2(file = rmsk_raw_path, skip = 3, col_names = F)
...
rmsk_tidy <- 
  ...
  dplyr::mutate(strand = replace(strand, strand == "C", "-"), 
                id = str_c(repName, "_", seqnames, ":", start, "-", end)) %>% 
  GRanges(.)
...
rmsk_all <- 
  ...
  readr::write_delim(., path = file.path(genome_dir, basename(rmsk_raw_path) %>% str_replace(., "raw.fa.out.gz", "joined_rmsk_id.fa.out.gz")), delim = "\t")
