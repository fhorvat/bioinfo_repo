...
######################################################## WORKING DIRECTORY
setwd("/common/WORK/fhorvat/Projekti/Svoboda/piRNA.Zuzka/Analysis/2020_paper/retrotransposons_annotation/STAR.CriGriPICR.LINE1_consensus.testis_smallRNA")
...
# mapped path
mapped_path <- file.path(base_path, "Data/Mapped/STAR_mesAur1/4_library_size")
...
# deviate table path
deviate_tb_path <- list.files(inpath, "*L1.*|*.Lx6", full.names = T)
...
deviate_tb_full <- purrr::map(deviate_tb_path, function(path){
  ...
  frame <- 
    ...
    dplyr::mutate(sample_id = sample_id %>% basename(.) %>% str_remove(., "\\.fastq\\.fused\\.sort\\.bam|\\.24to31nt\\.bam")) %>% 
...
# read stats table
stats_tb <- readr::read_delim(stats_tb_path, delim = "\t", col_names = c("sample_id", "library_size"))
...
sample_tb_filt <-
  ...
  dplyr::left_join(., stats_tb %>% dplyr::mutate(sample_id = str_remove(sample_id, "\\.24to31nt\\.bam")), by = "sample_id") %>% 
...
purrr::map(repeat_names, function(repeat_name){
  ...
  ggsave(plot = baseplot, 
         filename = file.path(outpath, str_c("LINE1", repeat_name, "CriGriPICR.5K_to_7k.ORFs.consensus", 
                                             "Mov10l1", "testis_smallRNA", "png", sep = ".")), 
         width = 15, height = 7.5)
