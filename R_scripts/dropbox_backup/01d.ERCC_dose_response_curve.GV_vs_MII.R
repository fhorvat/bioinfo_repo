...
######################################################## WORKING DIRECTORY
setwd("/common/WORK/fhorvat/Projekti/Svoboda/lncRNA_KO/Analysis/lnc1_paper/ERCC_spike_expression")
...
# fpkm path
fpkm_path_GV <- file.path(inpath, "ensembl.93.GRCm38.p6.20180919.UCSCseqnames.ERCC.FPKM.csv")
fpkm_path_MII <- file.path(inpath, "MII", "ensembl.93.GRCm38.p6.20180919.UCSCseqnames.ERCC.FPKM.csv")
...
######################################################## READ DATA
# read FPKM table
fpkm_tb_GV <- readr::read_csv(fpkm_path_GV)
fpkm_tb_MII <- readr::read_csv(fpkm_path_MII)
...
ercc_fpkm <- 
  ...
  dplyr::left_join(., fpkm_tb_GV %>% dplyr::select_at(vars(gene_id, starts_with("s_"))), by = c("id" = "gene_id")) %>% 
  dplyr::left_join(., fpkm_tb_MII %>% dplyr::select_at(vars(gene_id, starts_with("s_"))), by = c("id" = "gene_id")) %>% 
...
plot_ercc_tb <- 
  ercc_fpkm %>% 
  dplyr::mutate(log_fpkm = log10(fpkm), 
                log_molecules_n = log10(molecules_n), 
                genotype = str_extract(sample_id, "GV|MII"))

# plot without gene labels
ln_plot <- 
  ggplot() +
  geom_point(data = plot_ercc_tb, 
             mapping = aes(x = log_molecules_n, y = log_fpkm, color = genotype), alpha = 0.4, size = 3) +
  geom_smooth(data = plot_ercc_tb,
              mapping = aes(x = log_molecules_n, y = log_fpkm),
              method = lm) +
...
# save
ggsave(filename = file.path(outpath, "figure_2A_genes.GV_MII.ERCC.molecules_n.fpkm.log10.plot.png"), plot = ln_plot, height = 10, width = 10)
