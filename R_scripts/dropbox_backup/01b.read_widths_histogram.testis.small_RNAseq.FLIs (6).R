...
######################################################## WORKING DIRECTORY
setwd("/common/WORK/fhorvat/Projekti/Svoboda/piRNA.Zuzka/Analysis/2020_paper/retrotransposons_annotation/golden_hamster.Siomi_assembly/IAP/expression/small_RNAseq/hamster_testis_Mov10l.small_RNAseq")
...
inpath <- file.path(inpath, "FLI_elements/IAP/expression/hamster_testis_Mov10l.small_RNAseq")
...
RPM_summarized <- purrr::map(names(counts_tb_list), function(subset_name){
  ...
  rpm_tb %>% 
    ...
    readr::write_csv(str_c(subset_name, "testis", "small_RNAseq", "RPM", "sense_antisense", "read_length.histogram", "20201102", "csv", sep = "."))
