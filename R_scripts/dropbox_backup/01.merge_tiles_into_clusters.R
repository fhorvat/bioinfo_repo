...
# RPKM table path
rpkm_tb_path <- list.files(inpath, pattern = ".*\\.RPKM_mean\\.csv$", full.names = T, recursive = T)
...
clusters_13dpp <- 
  ...
  readr::write_csv(., file.path(outpath, "MesAur1.1k_windows.rpkm_cutoff.1.prepachytene.merged_clusters_and_superclusters.RPKM.csv"))
...
clusters_21dpp <- 
  ...
  dplyr::rename(width_kb = width, norm_width_kb = norm_width) %>% 
  dplyr::arrange(-norm_width_kb) %T>% 
  readr::write_csv(., file.path(outpath, "MesAur1.1k_windows.rpkm_cutoff.1.pachytene.merged_clusters_and_superclusters.RPKM.csv"))
...
plot_tb <- 
  clusters_all %>% 
  dplyr::select(logfc_x = log2FC_21dpp, logfc_y = log2FC_13dpp, stage)
