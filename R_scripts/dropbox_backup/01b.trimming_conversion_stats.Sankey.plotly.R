...
######################################################## WORKING DIRECTORY
setwd("/common/WORK/fhorvat/Projekti/Svoboda/hamster_KO/datasets/hamster_bisulfite.test_run/Data/Raw/Cleaned")
...
# mapping and duplication stats log
mapped_path <- "/common/WORK/fhorvat/Projekti/Svoboda/hamster_KO/datasets/hamster_bisulfite.test_run/Data/Mapped/Bismark_Siomi"
...
mapping_log_tb <- purrr::map(mapping_log, function(path){
  ...
  report_tb <- 
    ...
    dplyr::select(sample_id = File, 
                  mapped_reads, unmapped_reads, 
                  duplicated_reads_removed = `Duplicate Reads (removed)`, 
                  unique_reads_left = `Unique Reads (remaining)`)
  ...
  dplyr::summarise(mapped_reads = sum(mapped_reads),
                   ...
                   duplicated_reads_removed = sum(duplicated_reads_removed), 
                   unique_reads_left = sum(unique_reads_left)) %>% 
  dplyr::ungroup(.)
