...
count_21to23 <-
  purrr::map(class_21to23nt, function(table_in){
    ...
    readr::read_csv(table_in) %>%
      dplyr::mutate(sample = basename(table_in) %>% str_remove_all(., "smallRNA.*.Eliska_mESC_MosIR.|.detailed_other.csv"),
                    experiment = "Eliska_mESC_MosIR",
                    count = replace(count, is.na(count), 0)) %>% 
      dplyr::rename(count_21to23 = count, read_group = gene_biotype)
  ...
  purrr::map(class_allLengths, function(table_in){
    ...
    readr::read_csv(table_in) %>%
      dplyr::mutate(sample = basename(table_in) %>% str_remove_all(., "smallRNA.*.Eliska_mESC_MosIR.|.detailed_other.csv"),
                    experiment = "Eliska_mESC_MosIR",
                    count = replace(count, is.na(count), 0)) %>% 
      dplyr::rename(count_allLengths = count, read_group = gene_biotype)
...
write.xlsx(x = count_df_final %>% dplyr::select_at(vars("read_group", contains("count_all"))) %>% as.data.frame(.), 
           file = file.path(outpath, str_c("smallRNA.", experiment_name, ".other.read_summary.xlsx")), 
           sheetName = "count_all", 
           row.names = FALSE)
