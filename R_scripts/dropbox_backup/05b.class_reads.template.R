...
# get gene info path
info_path <- list.files(genome_path, pattern = "ensembl.93.*[0-9]{6}.UCSCseqnames.geneInfo.csv", full.names = T)

# get paths of reduced exons
exons_path <- list.files(genome_path, pattern = "ensembl.93.*[0-9]{6}.UCSCseqnames.reducedExons.RDS", full.names = T)
...
######################################################## MAIN CODE
### set class hierarchy
class_hier <- c("miRNA", "pseudogene", "MT", "SINE", "LINE", "LTR", "protein_coding.sense", "protein_coding.antisense", 
                "other", "not_annotated")
...
rmsk_gr <-
  ...
  dplyr::mutate(gene_biotype = NA, 
                gene_biotype = replace(x = gene_biotype, str_detect(repName, "MT[ABCD]"), "MT"), 
                gene_biotype = replace(x = gene_biotype, repClass == "SINE", "SINE"), 
                gene_biotype = replace(x = gene_biotype, repClass == "LINE", "LINE"), 
                gene_biotype = replace(x = gene_biotype, (repClass == "LTR" & is.na(gene_biotype)), "LTR"), 
                gene_biotype = replace(x = gene_biotype, is.na(gene_biotype), "other")) %>% 
...
exons_gr %<>%
  ...
  dplyr::select(seqnames:strand, gene_id, gene_biotype) %>%
  dplyr::mutate(gene_biotype = replace(gene_biotype, str_detect(gene_biotype, "pseudogene"), "pseudogene"), 
                gene_biotype = replace(gene_biotype, gene_biotype == "miRNA", "miRNA"), 
                gene_biotype = replace(gene_biotype, !gene_biotype %in% c("pseudogene", "miRNA", "protein_coding"), "other")) %>%
  GenomicRanges::GRanges(.)
...
mcols(mirna_gr)$gene_biotype <- "miRNA"
