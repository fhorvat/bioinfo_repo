...
meth_path <- c(str_c(meth_path, "/s_GV_Mov10l1_KO_So821_r1.PE_bismark_bt2_pe.deduplicated.bismark.cov.gz"),
               str_c(meth_path, "/s_GV_Mov10l1_WT_So812_r1.PE_bismark_bt2_pe.deduplicated.bismark.cov.gz"),
...
meth_sites <- 
  ...
  dplyr::filter((Mov10l1_HET >= 4 | Mov10l1_WT >= 4) & Mov10l1_KO >= 4)
...
# 
# # test siginficance between intergenic KO/HET values
# plot_tb %>%
#   dplyr::filter(hit_class == "intergenic") %>% 
#   tidyr::pivot_wider(id_cols = "coordinates", names_from = "genotype", values_from = "percentage") %$%
#   t.test(x = Mov10l1_KO, y = Mov10l1_HET, alternative = "less", paired = T)
# 
# # plot repeats
...
# # save as table
# readr::write_csv(plot_tb, file.path(outpath, "mean_methylation_percentage_per_category.with_WT.1kb_prom.ooctye.csv"))
...
purrr::map(names(plot_list), function(name){
  ...
  bar_plot <-
    ...
    scale_fill_manual(values = c("Mov10l1_WT" = "gray40", "Mov10l1_HET" = "gray60", "Mov10l1_KO" = "black")) +
    ylab("percentage of methylation") +
    xlab("genomic region") +
    # guides(fill = TRUE) +
    theme_bw() +
  ...
  ggsave(plot = bar_plot,
         filename = file.path(outpath, str_c("barplot", "methylation_percentage", name, "with_WT", "1kb_prom", "png", sep = ".")),
         width = 12, height = 10)
