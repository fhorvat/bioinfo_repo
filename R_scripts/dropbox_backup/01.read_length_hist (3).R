...
# base path
base_path <- "/common/WORK/fhorvat/Projekti/Svoboda/piRNA.Zuzka/datasets/mouse_testis.Papd7.small_RNAseq.Apr_2021"
...
read_length_tb <- 
  ...
  dplyr::arrange(desc(age), desc(genotype)) %>% 
...
read_length_stats <- 
  read_length_tb %>% 
  dplyr::group_by(genotype, age, alignment_length) %>% 
  dplyr::summarise(N = n(), 
                   mean = mean(rpm), 
                   median = median(rpm),
                   sd = sd(rpm), 
