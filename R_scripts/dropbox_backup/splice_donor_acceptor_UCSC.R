...
GffToGRanges = function(gff, filter=NULL){
  ...
  ids <- ids[, c("gene_id", "transcript_id")]
  gff$V7[!gff$V7 %in% c('+', '-')] <- '*'
...
gtf <- read.table("/common/WORK/fhorvat/reference/mm10/UCSC_knownGene_mm10_20161126.gtf.gz", header = FALSE, sep = '\t', stringsAsFactors = F)
...
write.table(splice.don, file = "UCSC_knownGene_mm10_20161126_splice_donor.txt", sep = "\t", row.names = F)
...
write.table(splice.acc, file = "UCSC_knownGene_mm10_20161126_splice_acceptor.txt", sep = "\t", row.names = F)

# joins transcripts with one and with more exons, gets ranges, finds introns
gtf.genes <- c(gtf.trans, gtf.trans.single)
write.table(unname(unlist(gtf.genes)), file = "UCSC_knownGene_mm10_20161126_exons.txt", sep = "\t", row.names = F)
