...
go_enrich_list <- purrr::map(comparison_list, function(comparison){
  ...
  go_enrich <- purrr::map(go_categories, function(go_cat){
    ...
    # enrich GO terms
    ego <- clusterProfiler::enrichGO(gene = diff_df$mmusculus_homolog_ensembl_gene,
    ...
    if(nrow(ego) > 0){
      ...
      # save to pdf
      pdf(file = file.path(outpath, str_c("GOEnrich.visualization", comparison, go_cat, "full.pdf", sep = ".")),
          width = 15,
          height = 10)
      ...
      if(nrow(ego_simple) > 0){
        ...
        # save to pdf
        pdf(file = file.path(outpath, str_c("GOEnrich.visualization", comparison, go_cat, "simple.pdf", sep = ".")),
            width = 15,
            height = 10)
