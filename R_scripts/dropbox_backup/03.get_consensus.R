...
insertions_path <- file.path(inpath, "MYSERV.ltr-1_family-16.2.7_to_3k.ORFs.csv")
...
# save as .bed
# rtracklayer::export.bed(insertions_gr, file.path(outpath, "MYSERV.ltr-1_family-16.2.7_to_3k.ORFs.bed"))

# get sequences
insertions_seq <- getSeq(BSgenome.Maur.UCSC.Siomi, insertions_gr)
insertions_seq <- insertions_seq[!(names(insertions_seq) %in% c("MYSERV.3003428", "MYSERV.2434897", 
                                                                "MYSERV.921244", "MYSERV.2018839", 
                                                                "MYSERV.2153927"))]
...
# write as fasta
seq_msa@unmasked %>%
  Biostrings::writeXStringSet(., file = file.path(outpath, "MYSERV.ltr-1_family-16.2.7_to_3k.ORFs.msa.fasta"))
...
# save the consensus
Biostrings::writeXStringSet(seq_consensus, file = file.path(outpath, "MYSERV.ltr-1_family-16.2.7_to_3k.ORFs.consensus.fasta"))
