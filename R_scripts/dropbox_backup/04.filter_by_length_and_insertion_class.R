...
line1_out <- 
  ...
  dplyr::filter(width >= 5000, width <= 7000, 
                ...
                longest_orf_1 >= 1200*3, 
                longest_orf_2 >= 370*3, 
                is.na(ensembl_id_overlap)) %>% 
  dplyr::filter(repName != "L1-1_CGr/L1-1_CGr") %>% 
...
# get sequences
line1_seq <- getSeq(BSgenome.Cgriseus.Ensembl.CriGriPICR, line1_gr)

# save as fasta
Biostrings::writeXStringSet(x = line1_seq, filepath = file.path(outpath, "LINE1.CriGriPICR.5K_to_7k.ORFs.fasta"))
...
# # write as fasta
# seq_msa@unmasked %>%
#   Biostrings::writeXStringSet(., file = file.path(outpath, str_c("LINE1.CriGriPICR.5K_to_7k.ORFs", "together", "msa.fasta", sep = ".")))
# 
...
#   # write as fasta
#   seq_msa@unmasked %>%
#     Biostrings::writeXStringSet(., file = file.path(outpath, str_c("LINE1.CriGriPICR.5K_to_7k.ORFs", seq_name, "msa.fasta", sep = ".")))
# 
#   # get the consensus
#   seq_consensus <- 
...
# 
# # save the consensus
# Biostrings::writeXStringSet(line1_seq_consensus, file = file.path(outpath, str_c("LINE1.CriGriPICR.5K_to_7k.ORFs", "consensus.fasta", sep = ".")))
