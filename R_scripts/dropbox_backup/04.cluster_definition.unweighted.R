...
######################################################## WORKING DIRECTORY
setwd("/common/WORK/fhorvat/Projekti/Svoboda/smallRNASeq_2018/Analysis/smallRNA_clusters/T3T_DcrTrans_2011/whole_counts")
...
l.files <- list.files(counts_path, full.names = T, pattern = ".*filt.unweighted.RData")
...
for(i in 1:length(l.files)){
  ...
  l.file <- l.files[i]
  name <- stringr::str_remove(basename(l.file), ".filt.unweighted.RData")
  print(name)
  Assigner(l.file, "l")
  l.mat[[name]] <- l
...
cluster_annotations <- AnnotateRegions(dicero_clust, l.annot)
...
do <- 
  ...
  tibble::as.tibble(.) %T>%
  readr::write_csv(., file.path(outpath, str_c("clusters", "DicerO", "intersect", "rpm", rpkm1, "width", width, "unweighted", "csv", sep = ".")))
...
do_mean <- 
  ...
  readr::write_csv(., file.path(outpath, str_c("clusters", "DicerO", "intersect", "rpm", rpkm1, "width", width, "unweighted", "mean", "csv", sep = ".")))
...
for(y in y_list){
  ...
  # set filter name
  filter_name <- "filtered.anyMedian50.Dicer"
  ...
  ggplot() +
    ...
    ggsave(filename = file.path(outpath, 
                                str_c("clusters", "DicerO", "intersect", "rpm", rpkm1, "width", width, x, "vs", y, 
                                      "meanRPM", filter_name, "unweighted", "png", sep = ".")), 
           width = 10, height = 10)
