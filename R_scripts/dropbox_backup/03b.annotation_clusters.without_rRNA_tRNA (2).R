...
sample_bam_path <- "/common/WORK/fhorvat/Projekti/Svoboda/RNAi.Eliska/DicerX_viral_infection/datasets/2019_Dec/Data/Mapped/STAR_mm10/2_original_mapping/s_E15.5_DicerX_KO_2_r5.SE.bam"
...
# repeatMasker rRNA and tRNA
rmsk_rRNA_tRNA <-
  rmsk_df %>%
  dplyr::filter(str_detect(repClass, "RNA"),
                repClass %in% c("rRNA", "tRNA")) %>%
  dplyr::select(-c(repName, repClass, repFamily, rmsk_ID)) %>%
  GenomicRanges::GRanges(.)
...
# ensembl rRNA and tRNA
ensembl_rRNA_tRNA <- 
  exons_df %>% 
  dplyr::filter(gene_biotype %in% c("Mt_rRNA", "Mt_tRNA", "rRNA")) %>% 
  GenomicRanges::GRanges(.)
...
ensembl_RNA <-
  ...
  dplyr::filter(!str_detect(gene_biotype, "protein_coding"), 
                !(gene_id %in% ensembl_rRNA_tRNA$gene_id)) %>%
  GenomicRanges::GRanges(.)
...
# joined rRNA, tRNA
RNA_rRNA_tRNA <- c(rmsk_rRNA_tRNA, ensembl_rRNA_tRNA)
...
l.annot <- list(RNA_rRNA_tRNA = RNA_rRNA_tRNA, 
                miRNA = mirna_gr,
                TE = rmsk_te,
                mRNA = ensembl_mrna,
...
# save RDS
saveRDS(l.annot, file = file.path(outpath, "l.annot.rRNA_tRNA.RDS"))
