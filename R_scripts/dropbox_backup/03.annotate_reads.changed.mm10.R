...
# gtf path
gene.path <- file.path(genome_dir, "ensembl.93.GRCm38.p6.20180919.UCSCseqnames.gtf.gz")

# rmsk path
reps.path <- file.path(genome_dir, "rmsk.mm10.20180919.clean.fa.out.gz")

# miRBase path
mirbase.path <- file.path(genome_dir, "miRBase.22.mm10.20181605.gff3")

# get bam file path
bamfile <- "../datasets/Tam_2008_Nature_GSE10364/s_oocyte_19to30.SE.21to23nt.bam"

######################################################## READ DATA
# read repeatMasker
reps <- 
  readr::read_delim(reps.path, delim = "\t") %>% 
  dplyr::select(-rmsk_id) %>% 
  GRanges(.)
...
# pseudogenes
pseudo <- gff[str_detect(gff$gene_biotype, 'pseudogene')]
...
# annotates ranges
gannot <- AnnotateRanges(g, lannot, ignore.strand = F) # here ignore.strand = T doesn't make sense since we use sense+antisense exonic annotation
...
dtabm <- tabm[, length(unique(name)), by = list(annot, uniq)]
