...
######################################################## WORKING DIRECTORY
setwd("/common/WORK/fhorvat/Projekti/Svoboda/hamster_KO/datasets/hamster_oocyte_Mov10l.smallRNAseq/Data/Documentation")
...
# filter
sample_tb %<>% 
  dplyr::filter(!(tissue %in% c("testis", "testis_RNAseq")))
...
sample_tb %>% 
  dplyr::select(sample_id, tissue, genotype, age, replicate, sample, sample_file_name, barcode, barcode_no, raw_path) %>% 
  dplyr::distinct(.) %>% 
