...
################################################################################### PATH VARIABLES
outpath <- "/common/WORK/fhorvat/Projekti/Svoboda/CNOT6L/Analysis/CNOT6L_2017/spliced_reads/documentation"

inpaths <- "/common/WORK/fhorvat/Projekti/Svoboda/CNOT6L/Analysis/CNOT6L_2017/spliced_reads/RData/Count_ExonIntron_RData"
...
tot_counts_path <- "/common/WORK/fhorvat/Projekti/Svoboda/CNOT6L/Analysis/CNOT6L_2017/spliced_reads/documentation/CNOT6L_library_size.txt"
...
names(l_bam) <- 
  left_join(tibble(ID = str_replace(basename(rfiles_counts), "_.*", "")),
            sample_table, 
            by = "ID") %$% 
  name
...
# repeats removed
gene_r <- gene_d
gene_r <- cbind(gene_r[, 1:10], gene_r[, str_detect(colnames(gene_r), "(reps_rem_uniq)|(ex_uniq)")])
colnames(gene_r) <- str_replace(colnames(gene_r), "_uniq", "")
colnames(gene_r) <- str_replace(colnames(gene_r), "_rem.uniq", "")
...
sample_names_unique <- unique(str_trunc(colnames(rpkm_table), width = 7, side = "right", ellipsis = ""))

gene_reps_rpkm_average <- lapply(sample_names_unique, function(x){
  
  # get one sample columns
  rpkm_table_average <- rpkm_table[, str_detect(colnames(rpkm_table), x)]
  # get sample name
  sample_name <- unique(str_replace(colnames(rpkm_table_average), "_X.*", ""))
  ...
  rpkm_table_average <- 
    ...
    as.data.frame(.) %>% 
    round(., 2) %>% 
    set_colnames(c(str_c(sample_name, ".ex", sep = ""), 
                   str_c(sample_name, ".int_reps_rem", sep = "")))
  
  return(rpkm_table_average)
  
})

# bind to one data.frame, add transcript data, write as table
gene_reps_rpkm_average <- bind_cols(gene_reps_rpkm_average)
gene_reps_rpkm_average <- cbind(gene_reps_rpkm[, c(1:10)], gene_reps_rpkm_average)
