...
line1_coords <- read.xlsx(line1_coords_path, sheet = "full length filtered")
line1_coords_2 <- read.xlsx(line1_coords_path, sheet = 2)

######################################################## MAIN CODE
# tidy sheet 2 - L1s_nested_ours_20180516 
# line1_tidy_2 <- 
#   line1_coords_2 %>% 
#   magrittr::set_names(., make.unique(colnames(.))) %>% 
#   as.tibble(.) %>% 
#   dplyr::slice(282:nrow(.)) %>%
#   dplyr::select(seqnames = chrName, start = chrStart, end = chrEnd, strand, id = uniqName, repClass, repFamily, repName) %T>%
...
line1_tidy <- 
  ...
  as.tibble(.) %>% 
  dplyr::distinct(.) %>% 
  write_csv(., file.path(outpath, "Documentation", "LINE1_full_length.20180517.ZJM.tidy.csv"))
...
line1_gr <- 
  ...
  sortSeqlevels(.) %>% 
  sort(.) %T>%
  rtracklayer::export(object = ., con = file.path(outpath, "Documentation", "LINE1_full_length.20180517.ZJM.tidy.bed"))
