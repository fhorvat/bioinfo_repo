...
experiment <- "Kabayama_2017_NucleicAcidsRes_PRJDB4628.small_RNA"
experiment_name <- "Kabayama"
...
stats_and_tracks <- 
  tracks_tbl_tidy %>% 
  dplyr::mutate(subset = str_extract(sample_id, "(?<=KO\\.).*$|(?<=WT\\.).*$"),
                subset = ifelse(is.na(subset), "whole_set", subset),
                sample_id = str_remove(sample_id, str_c("\\.", subset))) %>%
  left_join(., stats_tbl, by = "sample_id") %>%
  dplyr::select(experiment, sample_id, subset, RPM_scaled.coverage, raw.individual_reads, everything()) %>% 
  dplyr::select(-raw.coverage)

# multimappers
stats_and_tracks %>% 
  dplyr::filter(!str_detect(subset, "unique")) %T>%
  readr::write_csv(., path = file.path(outpath, str_c("log", experiment, "merged.filter_by_length.stranded.multimappers", "stats_and_tracks.csv", sep = ".")))

# unique
stats_and_tracks %>% 
  dplyr::filter(str_detect(subset, "unique")) %T>%
  readr::write_csv(., path = file.path(outpath, str_c("log", experiment, "merged.filter_by_length.stranded.unique", "stats_and_tracks.csv", sep = ".")))
