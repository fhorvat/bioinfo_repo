...
######################################################## WORKING DIRECTORY
setwd("/common/WORK/fhorvat/Projekti/Svoboda/piRNA.Zuzka/Analysis/2020_paper/retrotransposons_annotation/golden_hamster.Siomi_assembly/FLI_elements/MYSERV/expression/hamster_testis_Mov10l.8.5dpp.run_2.RNAseq")
...
# repeat info path
repeats_info_path <- file.path(inpath, "../..")
repeats_info_path <- list.files(repeats_info_path, ".*\\FLI_elements\\.csv$", full.names = T)
...
# read repeats info
repeats_info <- readr::read_csv(repeats_info_path)
...
rpm_tb <- 
  ...
  dplyr::select(rmsk_name, sample_id, RPM)
...
rpm_mean <- 
  ...
  dplyr::ungroup(.) %>% 
  tidyr::pivot_wider(id_cols = c("rmsk_name"), names_from = genotype, values_from = RPM, names_prefix = "RPM.") %>% 
  dplyr::filter(RPM.Mov10l1_WT <= 2*RPM.Mov10l1_KO)
