...
######################################################## PATH VARIABLES
# set inpath 
inpath <- "/common/WORK/fhorvat/Projekti/Svoboda/piRNA.Zuzka/Analysis/2020_paper/retrotransposons_annotation/golden_hamster.Siomi_assembly/FLI_elements/IAP/expression/hamster_oocyte_Mov10l.RNAseq"
...
subset_rpms_path <- file.path(outpath, "IAPs_subsets.oocyte.RNAseq.RPM.csv")
...
sample_tb %<>% 
  ...
  dplyr::filter(!(sample_id %in% c("s_GV_Mov10l_KO_F66_r4.SE", "s_GV_Mov10l_HET_F53_r1.SE")))
...
RPM_summarized <- purrr::map(names(counts_tb_list), function(subset_name){
  ...
  rpm_tb <- 
    ...
    dplyr::ungroup(.) %>% 
    dplyr::left_join(., library_size_tb, by = c("genotype")) %>% 
    ...
    tidyr::pivot_wider(id_cols = subset, names_from = genotype, values_from = RPM, names_prefix = "RPM.oocyte.")
...
# save as table
readr::write_csv(RPM_summarized, file.path(outpath, str_c("IAPs_subsets", "oocyte", "RNAseq", "RPM", "complete", "csv", sep = ".")))
