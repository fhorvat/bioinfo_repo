...
# clusters:
# +/- vs. O3 = s_ES_WT_tran_r1.SE vs. s_ES_Dcr_Rsc_tran_r2.SE
# S4 vs. O3 = s_ES_Dcr_Rsc_tran_r2.SE vs. s_ES_Dcr_Rsc_tran_r2.SE
dicero_bams <- c("s_ES_Dcr_Rsc_tran_r2.SE.mis_0", "s_ES_WT_tran_r1.SE.mis_0", "s_ES_DcrSom_Rsc_tran_r1.SE.mis_0", "s_ES_DcrSom_Rsc_tran_r2.SE.mis_0")
names(dicero_bams) <- c("DicerO3", "Dicer+_-", "DicerS11", "DicerS4")

# set cutoff values
rpkm1 <- 3
width <- 50
clust_list <- purrr::map(dicero_bams, function(x) FindClusters(bams[[x]], library_size[[x]], rpkm1, width, 1e6))

# merge clusters from replicates (method either intersect or union)
# dicero_clust <- ClusterMerge(clust_list[[1]], clust_list[[2]], method = "intersect") 
dicero_clust <- clust_list[[1]]
...
do <- 
  ...
  tibble::as.tibble(.) %T>%
  readr::write_csv(., file.path(outpath, str_c("clusters", "DicerO3", "rpm", rpkm1, "width", width, "csv", sep = ".")))
...
do_mean <- 
  do_long %>% 
  dplyr::select_at(vars(coordinates, contains("mean"))) %T>% 
  readr::write_csv(., file.path(outpath, str_c("clusters", "DicerO3", "rpm", rpkm1, "width", width, "mean", "csv", sep = ".")))
