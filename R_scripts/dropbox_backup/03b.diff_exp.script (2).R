...
if(exploratory_analysis == "yes"){
  ...
  if(length(grouping_variables) == 1){
    ...
    grouping_variable_2_length <- pca_tb[, grouping_variables[2], drop = T] %>% unique(.) %>% length(.)
    
    # if length of second grouping variable is 5 or less, use shapes with outline
    if(length(grouping_variable_2_length) <= 5){
      
      pca_plot <- 
        pca_plot +
        geom_point(aes_string(fill = grouping_variables[1], shape = grouping_variables[2]), color = "black", size = 7.5) +
        scale_shape_manual(values  = c(21, 22, 23, 24, 25)[1:grouping_variable_2_length]) +
        guides(fill = guide_legend(override.aes = list(shape = 23, size = 5)), 
               shape = guide_legend(override.aes = list(size = 5)))
      
    }else{
      
      # use default shapes without outline
      pca_plot <- 
        pca_plot +
        geom_point(aes_string(color = grouping_variables[1], shape = grouping_variables[2]), size = 7.5) +
        guides(color = guide_legend(override.aes = list(shape = 23, size = 5, fill = gg_color_hue(grouping_variable_2_length))), 
               shape = guide_legend(override.aes = list(size = 5)))
