...
# 3'UTR sequences path
utr_seq_path <- list.files(path = inpath, pattern = str_c("ensembl\\.", ensembl_version, ".*3pUTR\\.fasta$"), full.names = T)
...
# read 3'UTRs sequences
utr_seq <- Biostrings::readDNAStringSet(utr_seq_path)
...
mirna_targets_list <- purrr::map(names(mir_pattern), function(mir_seed_name){
  ...
  seed_match_tb <- 
    ...
    set_names(., c("gene_id", mir_seed_name))
...
mirna_targets_fpkm <- 
  ...
  dplyr::select(gene_id, gene_name, `6mer`:`7mer-1a`, FPKM_in_GV = GV, coordinates, gene_description)

# save
readr::write_csv(mirna_targets_fpkm, file.path(outpath, str_c("ensembl", ensembl_version, "Sscrofa11.1.20200424.3pUTR", mirna_name, "targets.csv", sep = ".")))
