...
######################################################## WORKING DIRECTORY
setwd("/common/WORK/fhorvat/Projekti/Svoboda/Analyses/smallRNA/GarciaLopez_2014_GSE45983")
...
######################################################## PATH VARIABLES
outpath <- "/common/WORK/fhorvat/Projekti/Svoboda/Analyses/smallRNA/GarciaLopez_2014_GSE45983"
lib_path <- "/common/WORK/fhorvat/code_library/R_scripts"

bam_path <- "/common/WORK/fhorvat/Projekti/Svoboda/accessory_data_sets/small_RNAseq/GarciaLopez_2014_development_smallRNA_GSE45983/Data/Mapped/STAR_mm10"
bam_list <- list.files(path = bam_path, pattern = "s_[oocyte,zygote].*bam$", full.names = T)
log_list <- list.files(path = bam_path, pattern = "s_[oocyte,zygote].*Log.final.out$", full.names = T)
...
# })
# 
# # get set of miRNA ranges for all samples
# bam_gr_all <-
...
smallRNA_pos <- 
  ...
  readr::write_csv(., path = file.path(outpath, "GarciaLopez_smallRNA_clusters_5reads.csv")) %$%
  full_pos
...
rpm_df <- 
  ...
  dplyr::select(-count, -library_size) %>% 
...
smallRNA_plot <- 
  ...
  readr::write_csv(., path = file.path(outpath, "GarciaLopez_RPM.csv")) %>% 
  dplyr::mutate(subtype = ifelse(!is.na(repeat_class), yes = repeat_class, no = "other"), 
                ...
                log2RPM_oocyte = log2(RPM_oocyte), 
                log2RPM_1cell = log2(RPM_1cell),
                log2FC_1cell_oocyte = (log2RPM_1cell - log2RPM_oocyte)) %>% 
  dplyr::rowwise() %>% 
...
# MA plot
ggplot(data = smallRNA_plot, aes(x = mean_RPM, y = log2FC_1cell_oocyte, color = type)) + 
  geom_point(size = 0.2) +
  ...
  ggsave(filename = file.path(outpath, "GarciaLopez_MAplot_1CvsGV.png"), width = 9, height = 7)

# scatter plot
ggplot(data = smallRNA_plot, aes(x = log2RPM_oocyte, y = log2RPM_1cell, color = type)) + 
  geom_point(size = 0.2) +
  ...
  theme(axis.text.x = element_text(size = 10, angle = 90, hjust = 1, vjust = 0), 
        ...
        panel.grid.minor = element_blank(), 
        axis.ticks.x = element_blank()) +
  ggsave(filename = file.path(outpath, "GarciaLopez_scatterPlot_1CvsGV.png"), width = 7, height = 7)

### color = repeat class
# MA plot
ggplot(data = smallRNA_plot, aes(x = mean_RPM, y = log2FC_1cell_oocyte, color = subtype)) +
  geom_point(size = 0.2) +
  scale_x_log10(breaks = c(10, 100, 1000, 10000)) +
  scale_y_continuous(limits = c(-10, 10)) +
  ...
  theme(axis.text.x = element_text(size = 10, angle = 90, hjust = 1, vjust = 0),
        ...
        panel.grid.minor = element_blank(),
        axis.ticks.x = element_blank()) +
  ggsave(filename = file.path(outpath, "GarciaLopez_MAplot_color_1CvsGV.png"), width = 9, height = 7)

# scatter plot
ggplot(data = smallRNA_plot, aes(x = log2RPM_oocyte, y = log2RPM_1cell, color = subtype)) +
  geom_point(size = 0.2) +
  scale_x_continuous(limits = c(-3, 15)) +
  scale_y_continuous(limits = c(-3, 15)) +
  scale_colour_manual(values = c(SINE = "red2", LTR = "green", LINE = "blue", miRNA = "black", other = "gray60")) +
  ...
  theme(axis.text.x = element_text(size = 10, angle = 90, hjust = 1, vjust = 0),
        ...
        panel.grid.minor = element_blank(),
        axis.ticks.x = element_blank()) +
  ggsave(filename = file.path(outpath, "GarciaLopez_scatterPlot_color_1CvsGV.png"), width = 7, height = 7)
