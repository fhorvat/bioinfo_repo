...
wd <- "/common/WORK/fhorvat/Projekti/Svoboda/Other/MT_elements/review/LTR_alignment/files/seq_logo/test"
if(!dir.exists(wd)){
...
signalLogo <- function(sequences, signal_pattern){
  ...
  # get gap index
  gap_index <- 
    as.data.frame(consensusMatrix(sub_sequences_aligned, as.prob = F, baseOnly = T)) %>% 
    apply(., 2, which.max) %>% 
    equals(., 5) %>% 
    data.frame(gap = .) %>% 
    mutate(left = lag(gap), 
           right = lead(gap), 
           id = rownames(.)) %>% 
    dplyr::select(id, left, gap, right) %>% 
    dplyr::filter(left == F & gap == T & right == F) %$%
    id %>% 
    gsub("V", "", .) %>% 
    as.integer()
