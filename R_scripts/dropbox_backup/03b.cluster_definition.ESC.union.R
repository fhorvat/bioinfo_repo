...
# library size path
library_size_path <- "/common/WORK/fhorvat/Projekti/Svoboda/smallRNASeq_2018/Analysis/MosIR_expression/mosIR.ES_DcrTrans_2012.counts_summary.xlsx"

# sample table path 
sample_table_path <- "/common/WORK/fhorvat/Projekti/Svoboda/smallRNASeq_2018/SOLiD/ES_DcrTrans_2012/Data/Documentation/ES_DcrTrans_2012.sample_table.csv"
...
# find DicerS clusters
s4.clust <- FindClusters(bams[["s_ES_DcrSom_Rsc_tran_r2.SE.mis_0"]], library_size[["s_ES_DcrSom_Rsc_tran_r2.SE.mis_0"]], rpkm1 = 3, clust.width = 100, 1e6)
s11.clust <- FindClusters(bams[["s_ES_DcrSom_Rsc_tran_r1.SE.mis_0"]], library_size[["s_ES_DcrSom_Rsc_tran_r1.SE.mis_0"]], rpkm1 = 3, clust.width = 100, 1e6)
s.clust <- ClusterMerge(s4.clust, s11.clust)

# find DicerO clusters
o2.clust <- FindClusters(bams[["s_ES_Dcr_Rsc_tran_r1.SE.mis_0"]], library_size[["s_ES_Dcr_Rsc_tran_r1.SE.mis_0"]], rpkm1 = 3, clust.width = 50, 1e6)
o3.clust <- FindClusters(bams[["s_ES_Dcr_Rsc_tran_r2.SE.mis_0"]], library_size[["s_ES_Dcr_Rsc_tran_r2.SE.mis_0"]], rpkm1 = 3, clust.width = 50, 1e6)
o.clust <- ClusterMerge(o2.clust, o3.clust)

# merge DicerO and DicerS clusters
all_clust <- ClusterMerge(o.clust, s.clust, method = "union")
...
d.cnts <- 
  do.call(cbind, lapply(bams, function(x) GetOverlaps(all_clust, x, "nh"))) %>% 
  as.tibble(.)
...
cp <- GetOverlaps(all_clust, bams[["s_ES_Dcr_Rsc_tran_r2.SE.mis_0"]][strand(bams[["s_ES_Dcr_Rsc_tran_r2.SE.mis_0"]]) == "+"], "nh")
cm <- GetOverlaps(all_clust, bams[["s_ES_Dcr_Rsc_tran_r2.SE.mis_0"]][strand(bams[["s_ES_Dcr_Rsc_tran_r2.SE.mis_0"]]) == "-"], "nh")
perc <- cp / (cp + cm)
...
do <- 
  cbind(all_clust_df, d.cnts.norm) %>% 
  tibble::as.tibble(.)
...
### plot clusters
## sample names
sample_names <- c("s_ES_Dcr_Rsc_tran_r1.SE.mis_0", "s_ES_Dcr_Rsc_tran_r2.SE.mis_0", 
               "s_ES_DcrSom_Rsc_tran_r2.SE.mis_0", "s_ES_DcrSom_Rsc_tran_r1.SE.mis_0", 
               "s_ES_WT_tran_r1.SE.mis_0")
names(sample_names) <- c("DicerO2", "DicerO3", "DicerS4", "DicerS11", "Dicer+_-")
...
# get samples
x <- sample_names[name_x]
y <- sample_names[name_y]
...
ggplot() +
  ...
  ggsave(filename = file.path(outpath, str_c("clusters", "all", "rpm", rpkm1, "width", width, 
                                             name_x, "vs", name_y, "RPM", "png", sep = ".")), 
         width = 10, height = 10)
