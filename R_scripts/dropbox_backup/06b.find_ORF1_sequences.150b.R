...
line1_orfs_tb <- 
  ...
  dplyr::mutate(orf_abs_start = ifelse(strand == "+", start + orf_start - 1, end - orf_start + 1), 
                seq_start = ifelse(strand == "+", orf_abs_start - 150, orf_abs_start - 150), 
                seq_end = ifelse(strand == "+", orf_abs_start + 150, orf_abs_start + 150)) %>% 
...
line1_orfs_seq <- line1_orfs_seq[names(line1_orfs_seq) != "L1_1_CGr.1718399"]

# # save as fasta
# Biostrings::writeXStringSet(x = line1_orfs_seq, filepath = file.path(outpath, "LINE1.CriGriPICR.ORF1_expand_150b.fasta"))
...
line1_seq_consensus <- purrr::map(names(line1_seq_list), function(seq_name){
  ...
  # write as fasta
  Biostrings::writeXStringSet(seq_consensus, file = file.path(outpath, str_c("LINE1.CriGriPICR.ORF1_expand_150b", seq_name, "consensus.fasta", sep = ".")))
