...
signalLogo <- function(sequences, signal_pattern){
  ...
  match_pattern <- 
    vmatchPattern(signal_pattern, sequences) %>% 
    unlist() %>% 
    as.data.frame() %>% 
  ...
  # get sequences of pattern and 20nt downstream
  sequences_pattern <- 
    lapply(X = match_pattern$names, FUN = function(X){
    substr(sequences[X], 
           start = match_pattern[match_pattern$"names" == X, "start"], 
           stop = match_pattern[match_pattern$"names" == X, "end"])
  }) %>% 
    unlist() %>% 
    DNAStringSet()
  
  # write .fasta 
  write.fasta(as.list(sequences_pattern),
              nbchar = 80,
