...
library_size_df <- tibble(stage = names(number_of_reads), 
                          library_size = number_of_reads)
...
# unique(all_elements_top100$element_name)
element_coverage_raw_list <- lapply("MT2_full", function(choosen_element_name){
  
  ### get elements of one type
  ...
  element_coverage_all_df <- 
    lapply(list(element_coverage_Oo, element_coverage_2C), positionCoverageDF, element_width_fun = element_width) %>% 
    bind_rows(.) %>% 
    mutate(stage = rep(c("s_Oo", "s_2C"), each = (2 * 1.5e5 + element_width + 1)), 
           ...
           element_name = choosen_element_name, 
           min_distace = 0, 
...
# save as .RDS
saveRDS(object = element_coverage_raw_list, file = "figure_S5B_coverage_list.RDS")
