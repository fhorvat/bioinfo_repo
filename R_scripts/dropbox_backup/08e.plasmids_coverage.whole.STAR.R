...
# sample bam path
sample_bam_path <- list.files(path = "/common/WORK/fhorvat/Projekti/Svoboda/smallRNASeq_2018/NIH3T3_transfected.2018/Data/Mapped/STAR.only_plasmids", 
                              pattern = "*.mis_0.bam$", full.names = T)[1]
...
# experiment paths
experiment_paths <- 
  "/common/WORK/fhorvat/Projekti/Svoboda/smallRNASeq_2018/NIH3T3_transfected.2018/Data/Mapped/STAR.only_plasmids" %>% 
...
for(bam_path in bam_paths){
  ...
  # bam_path <- "/common/WORK/fhorvat/Projekti/Svoboda/smallRNASeq_2018/NIH3T3_transfected.2018/Data/Mapped/STAR.only_plasmids/s_pCag_EGFP_Lin28IR_r1.SE.bam" 
  # ir_name <- str_remove_all(bam_name, "^s_|_r[1,2].SE") %>% str_replace(., "pCag_EGFP", "pCag-EGFP")
  ...
  plasmid <- plasmid_gr[seqnames(plasmid_gr) == ir_name]
  ...
  coverage_plot <-
    ggplot() +
    geom_rect(data = plot_df, aes(xmin = pos, xmax = pos + 1, ymin = 0, ymax = coverage, fill = strand)) +
  ...
  ggsave(plot = coverage_plot,
         filename = file.path(outpath, "coverage_plots/whole_plasmids/only_plasmids/STAR.only_plasmids", str_c(bam_name, "STAR.whole.coverage.RPM.png", sep = ".")),
         width = 15,
         height = 10)
