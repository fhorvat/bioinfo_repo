...
# make DESeqDataSet
dds <- 
  DESeqDataSet(se_filt, design = ~ stage + genotype) %>% 
  DESeq(.)
...
write.xlsx(x = results_all, 
           file = file.path(outpath, str_c("diffExp.Lnc1_KO.2018_Dec.GRCm38.91.all_biotypes.all.xlsx")), 
...
write.xlsx(x = results_significant, 
           file = file.path(outpath, str_c("diffExp.Lnc1_KO.2018_Dec.all_biotypes.significant.xlsx")), 
...
purrr::map(names(results_all), function(stage){
  ...
  # save plot
  ggsave(filename = file.path(outpath, str_c("MAplot.stage.", stage, ".Null_vs_WT.png")),
...
purrr::map(names(results_all), function(stage){
  ...
  if(nrow(MA_df) > 0){
    ...
    htmlwidgets::saveWidget(as_widget(interactive_ma_plot),
                            file = file.path(outpath, str_c("MAplot.stage.", stage, ".Null_vs_WT.html")),
                            selfcontained = T)
