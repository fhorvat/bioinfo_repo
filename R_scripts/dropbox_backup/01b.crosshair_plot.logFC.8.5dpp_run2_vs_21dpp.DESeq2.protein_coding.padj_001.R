...
# results path
results_path.21dpp <- list.files(expression_path, "protein_coding.*\\.all_results\\.xlsx$", full.names = T, recursive = T)
results_path.21dpp <- results_path.21dpp[!str_detect(results_path.21dpp, "old")]
...
### 21dpp 
# read results table 
results.21dpp <- openxlsx::read.xlsx(results_path.21dpp, sheet = "Mov10l_KO_21dpp_vs_Mov10l_WT_21") %>% as_tibble(.)
...
results.21dpp_tidy <- 
  results.21dpp %>%
  dplyr::select(gene_id, log2FC_21dpp = log2FoldChange)


### plot crosshair plot - 8.5dpp vs. 21dpp logFC 
# join results to one table
crosshair_tb_plot <- 
  left_join(results.8.5dpp_tidy, results.21dpp_tidy, by = "gene_id") %>% 
  dplyr::filter(gene_biotype == "protein_coding") %>%
  dplyr::arrange(regulation)

# get limits
axis_limits <-
  c(crosshair_tb_plot$log2FC_8.5dpp, crosshair_tb_plot$log2FC_21dpp) %>%
...
# crosshair plot
cross_plot <-
  ggplot(crosshair_tb_plot, aes(x = log2FC_21dpp, y = log2FC_8.5dpp, color = regulation, alpha = regulation)) +
  ...
  xlab(str_c("log2FC DESeq2 Mov10l1 KO vs. Mov10l1 WT - 21 dpp")) +
...
ggsave(filename = file.path(outpath, str_c(table_name, 
                                           "Mov10l_KO_WT.8.5dpp_run2_vs_21dpp.log2_ratio.DESeq2.crosshair", 
