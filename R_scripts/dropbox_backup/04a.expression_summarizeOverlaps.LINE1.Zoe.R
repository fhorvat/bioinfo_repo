...
se_SE <- 
  GenomicAlignments::summarizeOverlaps(features = line1_gr,
                                       ...
                                       singleEnd = TRUE,
                                       ignore.strand = TRUE) %>% 
  saveRDS(., file = file.path(outpath, "results", "L1s_nested_ours_20180516.ZJM.tidy.SE.se.RDS"))
...
se_PE <- 
  GenomicAlignments::summarizeOverlaps(features = line1_gr,
                                       ...
                                       singleEnd = FALSE,
                                       ignore.strand = TRUE) %>% 
  saveRDS(., file = file.path(outpath, "results", "L1s_nested_ours_20180516.ZJM.tidy.PE.se.RDS"))
