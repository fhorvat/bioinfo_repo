...
######################################################## WORKING DIRECTORY
# set data/reference
reference <- "Yang_2019_NatCommun_GSE95218"
...
rename_file <- 
  ...
  dplyr::mutate(RNA_Input = replace(RNA_Input, RNA_Input == "single oocyte" , "1_GV") %>% 
                  str_remove_all(., "1ng input of total RNA derived from | oocytes") %>% 
                  str_replace(., " ", "_")) %>% 
