...
######################################################## WORKING DIRECTORY
setwd("/common/WORK/fhorvat/Projekti/Svoboda/antiviral_RNAi.Marcos/datasets/mouse_mESC.MosIR.SOLiD.small_RNAseq.2011_May/Analysis/expression")
...
# base path
base_path <- "/common/WORK/fhorvat/Projekti/Svoboda/antiviral_RNAi.Marcos/datasets/mouse_mESC.MosIR.SOLiD.small_RNAseq.2011_May"

# documentation path
documentation_path <- file.path(base_path, "Data/Documentation")

# counts path
counts_path <- file.path(inpath, "miRBase.18.mm9.20211006.counts.txt")

# FPM path
fpm_mean_path <- file.path(inpath, "miRBase.18.mm9.20211006.FPM_mean.csv")
fpm_path <- file.path(inpath, "miRBase.18.mm9.20211006.FPM.csv")
...
# create sample table
sample_table <- 
  tibble(sample_id = colnames(counts_tb)[str_detect(colnames(counts_tb), "^s_")] %>% 
           str_remove(., "\\.21to23nt\\.bam")) %>% 
  dplyr::mutate(genotype = str_extract(sample_id, "Dgcr8_KO|MII.Dicer_Rsc|Som.Dicer_Rsc|WT|Dicer_KO"), 
                transfection = str_extract(sample_id, "tran|utra")) %>% 
  as.data.table(.)
...
# make DESeqDataSet
dds <- DESeqDataSet(se_filt, design = ~genotype + transfection)
...
if(TRUE){
  ...
  pca_tb <-
    ...
    dplyr::mutate(sample_id = str_remove_all(sample_id, "s_|r[0-9]+\\.SE|Dicer_SOM") %>% 
                    str_replace_all(., "_", " ") %>% 
                    str_trim(.)) %>% 
    dplyr::mutate(genotype = factor(genotype, levels = c("Dgcr8_KO", "Dicer_KO", 
                                                         "MII.Dicer_Rsc", "Som.Dicer_Rsc", 
                                                         "WT")))
  ...
  pca_plot <- 
    ...
    geom_point(aes(color = genotype, shape = transfection), size = 6) +
    ...
    scale_color_manual(values = c("Dgcr8_KO" = "#00B050", "Dicer_KO" = "#0000FF", "MII.Dicer_Rsc" = "#FF0000", 
                                  "Som.Dicer_Rsc" = "#FFC000", "WT" = "#00B0F0")) +
  ...
  # save plot
  ggsave(filename = file.path(outpath, str_c("miRBase", "plot", "PCA.PC1_PC2", "rlog", "genotype", "png", sep = ".")),
  ...
  # save plot
  ggsave(filename = file.path(outpath, str_c("miRBase", "plot", "PCA.PC1_PC2", "rlog", "genotype", "labeled", "png", sep = ".")),
         plot = pca_plot, width = 10, height = 10)
...
if(TRUE){
  ...
  annotation_df$genotype = factor(annotation_df$genotype, levels = c("Dgcr8_KO", "Dicer_KO", 
                                                                     "MII.Dicer_Rsc", "Som.Dicer_Rsc", 
                                                                     "WT"))
  
  # annotation colors
  ann_colors <- list(genotype = c("Dgcr8_KO" = "#00B050", "Dicer_KO" = "#0000FF", 
                                  "MII.Dicer_Rsc" = "#FF0000", 
                                  "Som.Dicer_Rsc" = "#FFC000", "WT" = "#00B0F0"))
  ...
  pheatmap::pheatmap(dist_matrix,
                     ...
                     file = file.path(outpath, str_c("miRBase",
                                                     "plot", "dist_heatmap", "rlog", "genotype",
                                                     "labeled", "png", sep = ".")),
                     height = 10,
                     width = 14)
