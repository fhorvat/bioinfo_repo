...
countIR <- function(bam_path, on_minus_strand = NA){
  ...
  # get counts over inverted repeats
  ir_hits <- IRanges::subsetByOverlaps(bam_gr, ir_filt, ignore.strand = T) 
  
  # return tibble
  count_table <- 
    tibble(alignment_length = width(ir_hits)) %>% 
...
counts_df <- 
  purrr::map(bam_paths, countIR, on_minus_strand = F) %>% 
  dplyr::bind_rows(.) %>% 
