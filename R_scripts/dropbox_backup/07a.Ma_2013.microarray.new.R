...
pm(rawAffy2) <- pmsVSN@hx
pm(rawAffy2) <- 2^pm(rawAffy2)
?vsn::vsnMatrix
vsn_affy <- vsn::vsn2(rawAffy)

rma_affy <- affy::rma(rawAffy2, normalize = FALSE, background = FALSE)

# vsn
vsn_affy <- vsn::vsnrma(rawAffy)
