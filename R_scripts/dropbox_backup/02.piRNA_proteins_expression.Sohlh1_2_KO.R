...
# experiment path
experiment_path <- "/common/WORK/fhorvat/Projekti/Svoboda/accessory_data_sets/RNAseq/Shin_2017_JClinInvest_PRJNA293873"

# FPKM table path
fpkm_path <- list.files(experiment_path, ".*\\.FPKM_long\\.csv", full.names = T, recursive = T)
...
# read FPKM table
fpkm_tb <- 
  purrr::map(fpkm_path, function(path){
...
pirna_genes_fpkm <- 
  ...
  dplyr::mutate(genotype = str_remove_all(sample_id, "s_NB_ovaries_|_r[0-9]+.[S,P]E")) 

# pivot to wider and save
pirna_fpkm_wide <- 
  pirna_genes_fpkm %>%
  dplyr::select(sample_id, fpkm, gene_name, gene_id, gene_description) %>% 
  tidyr::pivot_wider(id_cols = gene_name:gene_description, names_from = sample_id, values_from = fpkm) %T>%
  readr::write_csv(., path = file.path(outpath, "piRNA_pathway_genes.FPKM.Sohlh1_2_KO.csv"))
...
pirna_genes_fpkm_stats <- 
  ...
  dplyr::mutate(gene_name = factor(gene_name, levels = pirna_genes_tb$gene_name),
                ...
                genotype = factor(genotype, levels = c("WT", "Sohlh1_KO", "Sohlh2_KO", "Sohlh1_2_dKO")))
...
pirna_genes_plot <- 
  ...
  ggsave(filename = file.path(outpath, "piRNA_pathway_genes.FPKM.Sohlh1_2_KO.barplot.png"), width = 10, height = 10)
