...
read_class_tb <- 
  read_class_list %>% 
  dplyr::filter((str_detect(sample_id, "21dpp") & experiment_name == "hamster_testis_Mov10l.small_RNAseq") | 
                  (str_detect(sample_id, "13dpp") & experiment_name == "hamster_testis_Mov10l.small_RNAseq.reseq") |
                  (experiment_name == "hamster_testis_Mov10l.8.5dpp.run_2.small_RNAseq")) %>% 
  ...
  dplyr::mutate(genotype = str_extract(sample_id, "WT|KO|HET"), 
                age = str_extract(sample_id, "13dpp|21dpp|8.5dpp")) %>% 
...
purrr::map(names(read_class_tb_list), function(testis_age){
  ...
  read_class_tb_list[[testis_age]] %>% 
    ...
    readr::write_csv(., file.path(outpath, str_c("read_length.classes.fractions", testis_age, "20210104.csv", sep = ".")))
...
# save
ggsave(plot = read_class_barplot, filename = file.path(outpath, "read_length.classes.barplot.20210104.png"), width = 10, height = 10)
