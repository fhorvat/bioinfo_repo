...
######################################################## WORKING DIRECTORY
setwd("/common/WORK/fhorvat/Projekti/Svoboda/lncRNA_KO/datasets/2017Sep_sequencing")
...
sample_table_clean <- 
  sample_table %>% 
  dplyr::select(raw_path, genotype = Sample, barcode = `Barcode seq`) %>% 
  dplyr::mutate(raw_sample_id = raw_path %>% basename(.) %>% str_remove(., "_sequence.txt.gz"), 
                genotype = stringr::str_remove_all(genotype, " B6| [0-9]{1,}") %>% stringr::str_replace(., " ", ""), 
                stage = "GV", 
                sample_id = str_c("s", genotype, sep = "_"), 
                resequencing = str_detect(raw_path, "CBH5JACXX")) %>% 
  dplyr::select(raw_sample_id, genotype, stage, barcode, sample_id, resequencing, raw_path) %>% 
  ...
  dplyr::mutate(sample_id = replace(sample_id, sample_id == "s_Lnc1Het_r3.reseq.SE", "s_Lnc1Het_r2.reseq.SE"), 
                sample_id = replace(sample_id, sample_id == "s_Lnc1Null_r3.reseq.SE", "s_Lnc1Null_r2.reseq.SE")) %>% 
  dplyr::mutate(experiment = "2017Sep") %>% 
  dplyr::select(-rep) %>% 
