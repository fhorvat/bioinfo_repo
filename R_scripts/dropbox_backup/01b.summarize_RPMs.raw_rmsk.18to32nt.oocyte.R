...
# read lengths list
read_lengths <- list("18to32nt" = c(18, 32))
...
purrr::map(names(read_lengths), function(rlen){
  ...
  rpm_long <- 
    ...
    dplyr::mutate(age = "oocyte", 
                  genotype = str_extract(sample_id, "Mov10l1_KO|Mov10l1_WT")) %>%
    dplyr::mutate(age = factor(age, levels = "oocyte"),
                  genotype = factor(genotype, levels = c("Mov10l1_WT", "Mov10l1_KO"))) %>% 
...
saveWorkbook(wb_individual, file.path(outpath, str_c("all_LTR_classes", "hamster_oocyte_Mov10l.deduplicated.smallRNAseq", "all_samples_RPM", "20210517", "xlsx", sep = ".")), overwrite = TRUE)
saveWorkbook(wb_mean, file.path(outpath, str_c("all_LTR_classes", "hamster_oocyte_Mov10l.deduplicated.smallRNAseq", "mean_RPM", "20210517", "xlsx", sep = ".")), overwrite = TRUE)
