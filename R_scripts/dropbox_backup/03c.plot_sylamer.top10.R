...
purrr::map(names(syl_tb_list), function(kmer){
  ...
  # get top 10 kmers by p-value
  top_kmers <-
    kmer_tb_filt %>%
    dplyr::group_by(kmer) %>%
    dplyr::summarise(pvalue = max(abs(pvalue))) %>%
    dplyr::ungroup() %>%
    dplyr::top_n(., 10, abs(pvalue))
  
  # get labels
  kmer_tb_labels <-
    kmer_tb_filt %>% 
    dplyr::filter(kmer %in% top_kmers$kmer) %>%
    dplyr::group_by(kmer) %>% 
    dplyr::filter(abs(pvalue) == max(abs(pvalue)))
  
  # get kmers not in top 10
  other_kmers <- 
    kmer_tb_filt %>%
    dplyr::filter(!(kmer %in% top_kmers$kmer)) %$% 
    kmer %>% 
  ...
  kmer_tb_levels <- 
    kmer_tb_filt %>% 
    dplyr::mutate(kmer = factor(kmer, levels = c(other_kmers, top_kmers$kmer)))
  
  # plot
  syl_plot <- 
    ggplot() +
    geom_line(data = kmer_tb_levels, aes(x = bin, y = pvalue, color = kmer, size = kmer)) +
    geom_label_repel(data = kmer_tb_labels, aes(label = kmer, x = bin, y = pvalue),
    ...
    scale_color_manual(values = c(rep("gray60", length(other_kmers)), 
                                  gg_color_hue(length(top_kmers$kmer)))) +
    scale_size_manual(values = c(rep(1, length(other_kmers)), 
                                 rep(2, length(top_kmers$kmer)))) +
    ...
    ggtitle(str_c("Sylamer enrichment -", kmer, ", top 10 by p-value", sep = " ")) +
    ggsave(filename = file.path(outpath, sylamer_path[1] %>% basename %>% str_replace("6mer\\.txt$", str_c(kmer, ".top10.png"))), 
           width = 12, height = 12)
