...
# read miRBase gff
mirna_gr <- 
  rtracklayer::import.gff(con = mirbase_path) %>% 
  .[mcols(.)$type == "miRNA"]
...
# save
saveRDS(clust_list, file.path(outpath, str_c("clusters.DicerX_embryos", "rpm", rpkm1, "width", width, "RDS", sep = ".")))

# # read clusters
# clust_list <- readRDS(file.path(outpath, str_c("clusters.DicerX_embryos", "rpm", rpkm1, "width", width, "RDS", sep = ".")))
...
## overlap with miRNAs
# get overlaps 
overlaps <- findOverlaps(clusters_gr, mirna_gr, ignore.strand = F)

# get table with miRNA hits
mirna_hits <- tibble(coordinates = mcols(clusters_gr[queryHits(overlaps)])$coordinates, 
                     ID = mcols(mirna_gr[subjectHits(overlaps)])$ID)
...
plot_df <- 
  ...
  dplyr::select(x = x_sample, y = y_sample, class, coordinates) %>%
  dplyr::mutate(mean = ((x + y) / 2),
                lfc = log2(x + 1) - log2(y + 1)) %>% 
  dplyr::left_join(., mirna_hits, by = "coordinates")
...
annotation_tb <- 
  plot_df %>% 
  dplyr::filter(ID %in% c("MIMAT0028043", "MIMAT0016981", "MIMAT0004624", "MIMAT0027789", 
                          "MIMAT0014935", "MIMAT0028025", "MIMAT0027817", "MIMAT0027865"))
...
clusters_plot <- 
  ...
  geom_point(data = annotation_tb, aes(x = mean, y = lfc), color = "red2", size = 7.5, shape = "triangle") +
  geom_text(data = annotation_tb,
...
# save
ggsave(filename = file.path(outpath, str_c("clusters", "MA_plot", "DicerX_embryos", "WT", "KO", "union", "rpm", rpkm1,
                                           "width", width, "mean_RPM", filter_name, "labeled.all_samples", "png", sep = ".")), 
       plot = clusters_plot,
       width = 20, height = 10)
