...
# loop through stages
results_list <- purrr::map(stages, function(stage){
  ...
  # get significant results, filter by p-adjusted (padj < 0.1)
  results_df_signif <- 
    results_df %>%
  ...
  # save plot
  ggsave(filename = file.path(outpath, str_c("MAplot.stage.", stage, ".Null_vs_WT.protein_coding.png")),
  ...
  if(nrow(MA_df) > 0){
    ...
    htmlwidgets::saveWidget(as_widget(interactive_ma_plot),
                            file = file.path(outpath, str_c("MAplot.stage.", stage, ".Null_vs_WT.protein_coding.html")),
                            selfcontained = T)

  }
  
  # return results
  list(results_all = results_df, results_signif = results_df_signif)
  
})

### save results
# get all results to one list
results_list_all <- purrr::map(results_list, function(results) results$results_all)
results_list_signif <- purrr::map(results_list, function(results) results$results_signif)

# save significant results
write.xlsx(x = results_list_signif, 
           file = file.path(outpath, str_c("diffExp.genotype_simple.Lnc1_KO.2018_Dec.protein_coding.significant.xlsx")), 
           asTable = rep(TRUE, length(results_list_signif)),
           sheetName = stages, 
           colWidths = "auto")

# save all results
write.xlsx(x = results_list_all,
           file = file.path(outpath, str_c("diffExp.Lnc1_KO.2018_Dec.GRCm38.91.protein_coding.all.xlsx")),
           asTable = rep(TRUE, length(results_list_all)),
           sheetName = stages,
           colWidths = "auto")
