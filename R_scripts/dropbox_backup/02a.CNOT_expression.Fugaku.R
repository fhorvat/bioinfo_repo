...
exons_gr <- exons_gr[names(exons_gr) %in% genes_info$gene_id]
...
sample_table <- 
  ...
  dplyr::rename(library_size = mapped_genome_except_rDNA)
...
se <- GenomicAlignments::summarizeOverlaps(features = exons_gr,
                                           ...
                                           singleEnd = FALSE,
                                           ignore.strand = TRUE)
saveRDS(se, file = file.path(outpath, "CNOT.GRCm38.89.Fugaku.summarizedOverlaps.RDS"))

# read summarizedExperiment from RDS file
se <- readRDS(file = file.path(outpath, "CNOT.GRCm38.89.Fugaku.summarizedOverlaps.RDS")) 
