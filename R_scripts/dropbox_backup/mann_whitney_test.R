...
all_coverage_fpkm_binned <- 
  rbind(all_coverage_fpkm %>% #upstream
        ...
        all_coverage_fpkm %>% #downstream
          filter(pos > element_width & pos <= (500000 + element_width)) %>%
...
######## compare downstream 2C/4C
wilcox_results <- 
  all_coverage_fpkm_binned %>% 
  dplyr::select(pos, bin, stream, s_2C, s_GV) %>% 
  dplyr::filter(stream == "upstream") %>%
  dplyr::select(-stream) %>%
  group_by(bin) %>% 
  do(tidy(wilcox.test(.$s_2C, .$s_GV, alternative = "greater", paired = F))) %>% 
  ungroup(wilcox_results) %>% 
  as.data.frame() %>% 
  mutate(significant = p.value <= 0.05) %>% 
  dplyr::select(-method)
