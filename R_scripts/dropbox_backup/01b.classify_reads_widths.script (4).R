...
classReadsWidth <- function(bam_path, subject_ranges, yield = 1000000, isFirstInPair = NA){
  ...
  read_width_sum <-
    expand.grid(1:(length(class_hier) + 1), 16:86) %>%
    as.data.table(.) %>%
...
rmsk_gr <-
  ...
  dplyr::mutate(gene_biotype = replace(x = gene_biotype,
                                       list = !(gene_biotype %in% c("rRNA", "LINE", "SINE", "LTR", "tRNA")),
                                       values = "other_repeat")) %>%
  GenomicRanges::GRanges(.)

# clean ENSEMBL
ensembl_gr <- gtf_gr[mcols(gtf_gr)$gene_biotype != "miRNA"]
mcols(ensembl_gr)$gene_biotype <- ifelse(mcols(ensembl_gr)$gene_biotype == "protein_coding", "protein_coding", "other_ensembl")
