...
for(n in 1:length(cluster_path)){
  ...
  cluster_df <- 
    ...
    readr::write_csv(., file.path(outpath, str_c("clusters.Eliska_mESC_MosIR", str_c(sample_name, "_RS7"), "union", "rpm", 3, "width", 50, "csv", sep = ".")))
  ...
  mean.x <- str_c(sample_name, "_Mos", ".rpm_mean")
  ...
  p <-
    plotly::plot_ly(data = plot_df,
                    ...
                    height = 700) %>%
    add_markers() %>%
    layout(xaxis = list(title = sample_name),
           yaxis = list(title = y))
  
  # save as html widget
  htmlwidgets::saveWidget(as_widget(p),
                          file = file.path(outpath, str_c("clusters.Eliska_mESC_MosIR", str_c(sample_name, "_RS7"), "union", "rpm", 3, 
