...
experiment_polyA_name <- "Smallwood_2011_NatGenet_PRJEB2547"
...
FPKM_total_tidy <-
  FPKM_total %>% 
  dplyr::select(gene_id, GV.total = GV.WE, MII.total = MII.WE, MII.polyA = MII.PA, gene_biotype, gene_name, gene_description)

# polyA table
FPKM_polyA_tidy <-
  FPKM_polyA %>% 
  dplyr::select(gene_id, GV.polyA = FG_GV_oocyte, gene_biotype)
