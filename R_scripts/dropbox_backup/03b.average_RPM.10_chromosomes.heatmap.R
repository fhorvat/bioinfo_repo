...
read_stats_paths <- file.path("/common/WORK/fhorvat/Projekti/Svoboda/accessory_data_sets/RNAseq", experiment_list, "Data/Mapped/STAR_mm10/4_count_perfect_reads")

# read stats list 
read_stats_list <- list.files(read_stats_paths, pattern = ".*\\.read_stats.txt", full.names = T)

# counts path 
read_counts_paths <- file.path("/common/WORK/fhorvat/Projekti/Svoboda/accessory_data_sets/RNAseq", 
                               experiment_list, "Data/Mapped/STAR_mm10/5_rmsk_filtered_reads/filter_multimappers/count_10_chromosomes")
...
which.gap.col <- which(colnames(rpm_heatmap_tb) %in% c("female PGC 13.5", "round ST", "fully-grown oocyte", "blastocyst 94h", "placenta"))
...
pheatmap::pheatmap(rpm_heatmap_tb,
                   ...
                   filename = file.path(outpath, str_c("heatmap.rmsk.L1_and_LTRs.20190306.10_multimappers.10_chromosomes.RPM.png")),
                   height = 20,
                   width = 20)
