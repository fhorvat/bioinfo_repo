...
######################################################## WORKING DIRECTORY
# set working dir
setwd("/common/WORK/fhorvat/Projekti/Svoboda/piRNA.Zuzka/Analysis/2020_paper/retrotransposons_annotation/golden_hamster.Siomi_assembly/LTRs")
...
ltr_class_tb <- 
  ltr_tb %>% 
  dplyr::mutate(ltr_class = str_extract(ltr_name, "^ERV|^IAP|^MLT|^MMERVK|^MT|^ORR|^RMER|^RLTR|^MER|^LTR|Gyp|^BGL|^Eutr"))
...
ltrs_flanks_seq <- getSeq(BSgenome.Maur.UCSC.Siomi, GRanges(ltrs_flanks_tb))
...
# add pairwise score, edit distance, pairwise distance
ltrs_distances <- 
  ltrs_tb %>% 
  dplyr::mutate(align_score = pairwiseAlignment(sequence_5pLTR, sequence_3pLTR, scoreOnly = T), 
                ...
                kimura_distance = ape::dist.dna(sequence_5pLTR, sequence_3pLTR)) %>% 
  dplyr::left_join(., ltr_tb %>% 
                     tidyr::unite(insertion_coordinates, seqnames, start, end, sep = " ") %>% 
                     dplyr::select(insertion_coordinates, repName, ltr_class, ltr_name, rmsk_id, insertion_class), by = "rmsk_id") %>% 
  dplyr::select(insertion_coordinates, repName, ltr_class, ltr_name, rmsk_id, insertion_class, align_score, edit_distance, kimura_distance,
