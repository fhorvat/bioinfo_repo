...
rpm_paths <- rpm_paths[str_detect(rpm_paths, "hamster_testis_Mov10l.8.5dpp.run_2.small_RNAseq")]
...
rpm_tb_long <- 
  rpm_tb %>%
  dplyr::mutate(subset = factor(subset, levels = c("MYSERV6_full"))) %>% 
...
purrr::map(unique(rpm_tb_long$subset), function(element_subset){
  ...
  rpm_tb_hist %>% 
    ...
    readr::write_csv(x = ., file = file.path(outpath, str_c(element_subset, "hamster_testis_Mov10l.8.5dpp.run_2.small_RNAseq", 
                                                            str_c(19, "to", 32, "nt"), 
                                                            "sense_antisense", 
  ...
  rpm_tb_hist_mean %>% 
    ...
    readr::write_csv(., file.path(outpath, str_c(element_subset, "hamster_testis_Mov10l.8.5dpp.run_2.small_RNAseq", 
                                                 str_c(19, "to", 32, "nt"), 
                                                 "sense_antisense",
                                                 "mean_RPM.20210517.csv", sep = ".")))
