...
### added by fhorvat, uses data.table::fread insted of base::read.table()
### sets NA values to 0
# ccounts <- read.table( tb_1, row.names = 1, header=T)
args <- c("circRNA.5reads.1ind.countmatrix",
          0, 
          0, 
          "circRNA.5reads.1ind.genes")

ccounts <- data.table::fread(args[1])
ccounts <- as.data.frame(ccounts)
rownames(ccounts) <- ccounts$cRNA
ccounts <- ccounts[, -1]
...
if(samples - length(badsamples) > 1) {
  ...
  write.table(v_cRNA$E, paste("norm_log2_counts_", gsub("countmatrix", "", args[1]),  args[2], "cpm_", args[3], "samples.txt", sep = ""), 
  ...
  heatmap.2(pmin(v_cRNA$E[idx,], limit), labRow = cfeature[idx], 
            Rowv = T, scale = 'none', Colv = T, trace = 'none', 
            srtCol = 90, cexRow = 0.5, cexCol = 0.7, margins = c(7, 7), 
            col = my_palette)
