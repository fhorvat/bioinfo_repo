...
experiment='Roovers_2015'
threads=1
mapped_path='/common/WORK/fhorvat/Projekti/Svoboda/accessory_data_sets/small_RNAseq/Roovers_2015_CellRep_GSE64942/Data/Mapped/STAR_bosTau9'
documentation_path='/common/WORK/fhorvat/Projekti/Svoboda/accessory_data_sets/small_RNAseq/Roovers_2015_CellRep_GSE64942/Data/Documentation'
features_coordinates='/common/DB/genome_reference/cow/bosTau9.ARS-UCD1.2.GCA_002263795.2/ensembl.96.ARS-UCD1.2.20190809.UCSCseqnames.miRNA.gff3'
features_name='ensembl.96.ARS-UCD1.2.20190809.UCSCseqnames.miRNA'
genes_info_path=''
grouping_variables='stage'
counts_path='./ensembl.96.ARS-UCD1.2.20190809.UCSCseqnames.miRNA.counts.txt'

# gene info path
genes_info_path <- "/common/DB/genome_reference/cow/bosTau9.ARS-UCD1.2.GCA_002263795.2/ensembl.96.ARS-UCD1.2.20190809.UCSCseqnames.geneInfo.csv"
...
fpm_long_tb <-
  ...
  .[, c("gene_id", "gene_name", "sample_id", "counts", "fpm", "fpkm", grouping_variables, "coordinates"), with = F] %>%
  .[!is.na(gene_id)]
...
fpm_wide_tb <-
  fpm_long_tb %>%
  dcast(., gene_id + gene_name + coordinates ~ sample_id, value.var = "fpm") %T>%
...
fpm_mean_tb <-
  ...
  dcast(., gene_id + gene_name + coordinates ~ grouped_variables, value.var = "fpm_mean") %>% 
  .[order(GV, decreasing = T), ] %T>%
  write_csv(., file.path(outpath, str_c(features_name, ".FPM_mean.csv")))

# long FPM with mean, SD and standard error for each gene
fpm_stats <-
  fpm_long_tb %>%
  .[, list(gene_name = unique(gene_name), 
           avg_fpm = mean(fpm),
           SD = sd(fpm),
