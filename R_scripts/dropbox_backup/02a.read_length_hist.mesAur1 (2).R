...
# hamster path 
hamster_path <- "/common/WORK/fhorvat/Projekti/Svoboda/hamster_KO/datasets/hamster_testis_Mov10l.8.5dpp.small_RNAseq/Data/Mapped/STAR_mesAur1"
...
read_length_plot <- 
  ...
  ggsave(filename = file.path(outpath, "read_length.histogram.all.png"), width = 10, height = 15)
...
ridge_plot <-
  ...
  ggsave(filename = file.path(outpath, "read_length.joy_division.png"), width = 15, height = 12)
...
read_length_plot <- 
  ...
  theme(axis.text.x = element_text(angle = 90, hjust = 1, size = 12, vjust = 0.5),
        ...
        panel.grid.minor = element_blank(),
        legend.position = "bottom") +
  ggsave(filename = file.path(outpath, "read_length.stacked_barplot.png"), width = 10, height = 10)
