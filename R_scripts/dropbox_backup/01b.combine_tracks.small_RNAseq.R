...
# list of experiments
experiment_list <- c("/common/WORK/fhorvat/Projekti/Svoboda/hamster_KO/datasets/hamster_testis_Mov10l.8.5dpp.small_RNAseq",
                     "/common/WORK/fhorvat/Projekti/Svoboda/hamster_KO/datasets/hamster_testis_Mov10l.small_RNAseq", 
                     "/common/WORK/fhorvat/Projekti/Svoboda/hamster_KO/datasets/hamster_testis_Mov10l.small_RNAseq.reseq")
...
tracks_tbl_tidy_2 <- 
  ...
  dplyr::mutate(perfect = ifelse(perfect, "perfect", "mismatches"), 
                scaled = ifelse(str_detect(sample_id, "scaled"), "RPM_scaled_to_19to32nt", "raw"),
                file_type = ifelse(str_detect(sample_id, "\\.bw$"), "coverage", "individual_reads"), 
                sample_id = str_remove(sample_id, "\\.bam|\\.scaled_19to32\\.bw|\\.scaled\\.bw"),
  ...
  dplyr::mutate(read_lengths = str_extract(sample_id, "19to32nt$|21to23nt$|24to31nt$")) %>% 
  dplyr::mutate(sample_id = str_remove(sample_id, "\\.19to32nt$|\\.21to23nt$|\\.24to31nt$")) %>% 
  dplyr::select(experiment, sample_id, read_lengths, scaled, file_type, perfect, URL)

# continue cleaning table
tracks_tbl_tidy_3 <- 
  tracks_tbl_tidy_2 %>% 
  tidyr::pivot_wider(id_cols = c(experiment, sample_id, read_lengths), names_from = c("scaled", "file_type", "perfect"), values_from = "URL") %>% 
  dplyr::select(experiment, sample_id, read_lengths, starts_with("RPM"), starts_with("raw"))

# save
readr::write_csv(tracks_tbl_tidy_3, file = file.path(outpath, "Mov10l1_hamster.all_multimappers.merged_replicates.small_RNAseq.tracks.20201214.csv"))
