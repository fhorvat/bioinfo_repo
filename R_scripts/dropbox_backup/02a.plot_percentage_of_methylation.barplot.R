...
meth_tb <- purrr::map(meth_path, function(path){
  ...
  readr::read_delim(path, delim = "\t", col_names = c("seqnames", "start", "end", "percentage_meth", "count_meth", "count_unmeth")) %>% 
    dplyr::mutate(sample_id = basename(path) %>% str_remove(., "_bismark_bt2_pe\\.deduplicated\\.bismark\\.cov\\.gz"))
...
# save as table
readr::write_csv(plot_tb, file.path(outpath, "mean_methylation_percentage_per_categogory.ooctye.csv"))
...
purrr::map(names(plot_list), function(name){
  ...
  ggsave(plot = bar_plot,
         filename = file.path(outpath, str_c("barplot", "methylation_percentage", name, "500bp_prom", "png", sep = ".")),
         width = 12, height = 10)
