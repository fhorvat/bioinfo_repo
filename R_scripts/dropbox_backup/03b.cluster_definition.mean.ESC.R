...
sample_bams <- c("s_ES_Dcr_Rsc_tran_r2.SE.mis_0", "s_ES_Dcr_Rsc_utran_r2.SE.mis_0",
                 "s_ES_DcrSom_Rsc_tran_r1.SE.mis_0", "s_ES_DcrSom_Rsc_utran_r1.SE.mis_0", 
                 "s_ES_WT_tran_r1.SE.mis_0")
names(sample_bams) <- c("DcrO3.tran", "DcrO3.utran", "DcrS11.tran", "DcrS11.utran",  "Dcr+_-")

# list count matrices RData files
counts_path <- file.path(inpath, "filtered_bams")
l.files <- 
  list.files(counts_path, full.names = T, pattern = ".*filt.RData") %>% 
  .[str_detect(., str_c(sample_bams, collapse = "|"))]
...
sample_table %<>% 
  ...
  dplyr::filter(sample_id %in% str_remove(sample_bams, ".mis_0"))
...
library_size_df %<>% 
  ...
  dplyr::filter(sample_id %in% sample_bams)
...
dicerO_samples <- sample_bams[str_detect(names(sample_bams), "DcrO")]
dicerO_clust <- ClusterMerge(clust_list[[dicerO_samples[1]]], clust_list[[dicerO_samples[2]]], method = "intersect")

# merge clusters from DicerS replicates (intersect)
dicerS_samples <- sample_bams[str_detect(names(sample_bams), "DcrS")]
dicerS_clust <- ClusterMerge(clust_list[[dicerS_samples[1]]], clust_list[[dicerS_samples[2]]], method = "intersect")

# merge clusters from DicerS replicates (intersect)
dicerWT_samples <- sample_bams[str_detect(names(sample_bams), "Dcr\\+_-")]
dicerWT_clust <- clust_list[[dicerWT_samples[1]]]

# y-axis clusters
clusters_yaxis <- list(dicerS_clust, dicerWT_clust)
names(clusters_yaxis) <- c("DcrS", "Dcr+_-")
...
for(clust_name in names(clusters_yaxis)){
  ...
  do <- 
    ...
    tibble::as.tibble(.) %T>%
    readr::write_csv(., file.path(outpath, str_c("clusters.ESC", "DcrO", clust_name, "union", "rpm", rpkm1, "width", width, "csv", sep = ".")))
  ...
  do_long <- 
    ...
    dplyr::mutate(genotype = replace(genotype, genotype == "WT", "Dcr+_-.tran")) %>% 
    dplyr::group_by(coordinates, genotype) %>% 
    dplyr::summarise(rpm_mean = round(mean(rpm), 3)) %>% 
    dplyr::ungroup(.) %>% 
    tidyr::gather(summary, rpm, rpm_mean) %>% 
    tidyr::unite(genotype, genotype, summary, sep = ".") %>% 
    tidyr::spread(genotype, rpm) %>% 
  ...
  do_mean <- 
    ...
    readr::write_csv(., file.path(outpath, str_c("clusters.ESC", "DcrO", clust_name, "union", "rpm", rpkm1, "width", width, "mean", "csv", sep = ".")))
  ...
  ggplot() +
    ...
    ggsave(filename = file.path(outpath, str_c("clusters.ESC", "DcrO", clust_name, "union", "rpm", rpkm1, 
