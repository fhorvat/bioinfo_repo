...
######################################################## WORKING DIRECTORY
setwd("/common/DB/SRA/sra/Svoboda/2017_download/ENCODE/mouse/polyA_RNAseq")
...
# documentation path
documentation_path <- "/common/WORK/fhorvat/Projekti/Svoboda/accessory_data_sets/RNAseq/ENCODE/mouse/polyA/Data/Documentation"

# links path
links_path <- "/common/WORK/fhorvat/Projekti/Svoboda/accessory_data_sets/RNAseq/ENCODE/mouse/polyA/Data/Raw/Links"
...
rename_file %>%
  dplyr::select(sample_id = name, 
                tissue,
                run) %T>%
  readr:::write_csv(., file = file.path(documentation_path, str_c("ENCODE.mouse.polyA", ".sampleTable.csv"))) %T>%
  readr:::write_csv(., file = file.path(outpath, str_c("ENCODE.mouse.polyA", ".sampleTable.csv")))

# save make links script
rename_file %>% 
  dplyr::arrange(name) %>% 
  dplyr::mutate(name = str_c(links_path, "/", name, ".bam"),
                run = str_c(outpath, "/", run, ".bam")) %>% 
