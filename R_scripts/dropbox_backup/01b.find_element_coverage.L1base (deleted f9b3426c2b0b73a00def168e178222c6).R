...
# get width in table
line1_tb_tidy <- as_tibble(line1_gr)
...
line1_coverage_list <- purrr::map(names(coverage_list), function(sample_id){
  ...
  line1_coverage_sum <- 
    ...
    dplyr::ungroup(.) %>% 
    dplyr::left_join(., line1_tb_tidy %>% dplyr::select(rmsk_id, width), by = "rmsk_id") %>% 
...
# join to one table
line1_coverage_tb <- 
  purrr::reduce(c(list(line1_tb_tidy), line1_coverage_list), left_join, by = "rmsk_id") %>% 
...
line1_out <- 
  line1_tb %>% 
  dplyr::left_join(., line1_coverage_tb %>% dplyr::select(-c(seqnames:repName)), by = c("L1base_id" = "rmsk_id")) %T>%
  readr::write_csv(., path = file.path(outpath, "2019_RNAi_piRNA_file3_L1_table.Dicer_Mili_KO.coverage.csv"))
