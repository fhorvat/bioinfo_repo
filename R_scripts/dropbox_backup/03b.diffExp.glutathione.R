...
purrr::map(stages, function(stage){
  ...
  plot_df <- 
    results_df %>% 
    dplyr::select(mean = baseMean, lfc = log2FoldChange, padj, gene_id, gene_name, coordinates, gene_description) %>% 
  ...
  annotation_df <- 
    ...
    dplyr::filter((gene_id %in% c("ENSMUSG00000055839", "ENSMUSG00000110001", "ENSMUSG00000057534") | 
                     str_detect(gene_description, "glutathione")))
  
  # add black dots for labeled genes
  ma_plot <- 
    ma_plot + 
  ...
  ggsave(filename = file.path(outpath, str_c("diffExp",
                                             ...
                                             "DESeq2.protein_coding.MA_plot.glutathione.png", sep = ".")),
  ...
  ma_plot <- 
    ...
    geom_label_repel(data = annotation_df, aes(x = mean, y = lfc, label = gene_name), 
                     fontface = "bold", color = "black", box.padding = 0.35, point.padding = 0.5, segment.color = "black")
  
  # save plot
  ggsave(filename = file.path(outpath, str_c("diffExp",
                                             "ensembl", ensembl_version,
                                             stage, "lnc1_Null_vs_WT",
                                             "DESeq2.protein_coding.MA_plot.labels.glutathione.png", sep = ".")),
         plot = ma_plot, width = 10, height = 10)
