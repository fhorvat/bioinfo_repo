...
dist_grid <- readRDS(distance_grid_path)

######################################################## MAIN CODE
#### septamer ####
# set septamer pattern
...
# binned expression paths
binned_array_path <- file.path(inpath, str_c("grid.array.", tissue, ".50.bins.filtered.csv"))
binned_ngs_path <- file.path(inpath, str_c("grid.ngs.", tissue, ".50.bins.filtered.csv"))

# read and clean array grid
binned_array <- 
  readr::read_csv(binned_array_path) %>% 
  dplyr::select(gene_id, starts_with("bin"))

# read and clean NGS grid
binned_ngs <- 
  readr::read_csv(binned_ngs_path) %>% 
  dplyr::select(gene_id, starts_with("bin"))

# add to list
binned_list <- list(array = binned_array, ngs = binned_ngs)


#### plot ####
# plot on NGS and array grids
purrr::map(names(binned_list), function(grid){
  
  # join with grid table
  septamer_targets_grid <- 
    septamer_targets_tb %>% 
    dplyr::inner_join(., binned_list[[grid]], by = "gene_id") %>%
    ...
    .[dist_grid, on = c("bin_id" = "bin_id_dist")] %>% 
    .[is.na(n), n := 0] %>% 
    .[]
