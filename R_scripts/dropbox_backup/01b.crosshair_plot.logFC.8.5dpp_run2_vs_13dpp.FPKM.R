...
# crosshair plot
cross_plot <-
  ggplot(fpkm_tb_plot, aes(x = log2FC_13dpp, y = log2FC_8.5dpp, color = regulation, alpha = regulation)) +
  ...
  ylab(str_c("log2 (Mov10l1 KO / WT) FPKM - 8.5dpp")) +
  theme_bw() +
...
# save plot
ggsave(filename = file.path(outpath, str_c(table_name, "Mov10l_KO_WT.8.5dpp_run2_vs_13dpp.log2_ratio.crosshair", "any_WT", str_c(fpkm_cutoff, "_rpkm_cutoff"), "jpg", sep = ".")),
       plot = cross_plot,
       width = 10, height = 10)
