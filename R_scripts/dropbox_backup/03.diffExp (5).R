...
# set grouping variable(s) and groups for results
grouping_variables <- c("stage", "genotype")
results_groups <- c("GV_Null,GV_WT", "MII_Null,MII_WT")
...
ggsave(filename = file.path(outpath, str_c("diffExp", 
                                           "ensembl", ensembl_version,
                                           str_c(grouping_variables, collapse = "_"), 
                                           "rlog.PC1_2.labeled.PCA_plot.png", 
                                           sep = ".")),
       plot = pca_plot, width = 10, height = 10)
...
pheatmap::pheatmap(dist_matrix,
                   ...
                   filename = file.path(outpath,  str_c("diffExp", 
                                                        "ensembl", ensembl_version,
                                                        str_c(grouping_variables, collapse = "_"), 
                                                        "rlog.distance_heatmap.png", sep = ".")),
                   height = 10,
                   width = 12)
...
if(!is.null(results_groups)){
  ...
  openxlsx::saveWorkbook(wb = wb_significant, 
                         file = file.path(outpath, str_c("diffExp", 
                                                         "ensembl", ensembl_version,
                                                         str_c(grouping_variables, collapse = "_"), 
                                                         "DESeq2.significant_results.xlsx", sep = ".")), 
                         overwrite = TRUE)
