...
signalLogo <- function(sequences, signal_pattern){
  ...
  pattern_names <- 
    vmatchPattern(signal_pattern, sequences, max.mismatch = 1) %>% 
    unlist() %>% 
    names() %>% 
    unique()
  ...
  signal_start <- 
    as.data.frame(vmatchPattern(signal_pattern, sequences, max.mismatch = 1)) %$%
    start %>% 
    Mode()
  ...
  signal_start <- 
    as.data.frame(vmatchPattern(signal_pattern, sub_sequences_aligned, max.mismatch = 1)) %$%
    start %>% 
    Mode() 
