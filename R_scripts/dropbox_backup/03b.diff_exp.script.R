...
if(exploratory_analysis == "yes"){
  ...
  if(length(grouping_variables) == 1){
    ...
    pca_plot <- 
      ...
      scale_shape_manual(values  = c(21, 22, 23, 24, 25)[1:(pca_tb[, grouping_variables[2], drop = T] %>% unique(.) %>% length(.))]) +
      guides(fill = guide_legend(override.aes = list(shape = 21, size = 5)), 
             shape = guide_legend(override.aes = list(size = 5, gg_color_hue(2))))
