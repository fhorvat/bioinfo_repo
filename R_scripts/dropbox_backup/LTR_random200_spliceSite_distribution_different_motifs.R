...
## old
# MTs   -  AANTGTAAGC - red
# MT2s  -  GACTGTAAGT - blue
# ORR1s -  ANCTGTAAGC - black

## new
# MTs   -  AAGTGTAAGC - red
# MT2s  -  GACTGTAAGT - blue
# ORR1s - AANCTGTAAGC - black

## loose
# MTs   -  RASTGTAAGY - red
# MT2s  -  RASTGTAAGY - blue
# ORR1s - ARNSTGTAAGY - black
...
#        (G/C) TGTAAG (T/C)
#       A(G/C) TGTAAG (T/C)
#  (A/G)A(G/C) TGTAAG (T/C)
                  
# get MT pattern start positions - RED
MT_splice_distribution <- 
  unlist(vmatchPattern("AAGTGTAAGC", LTR_seqences, fixed = F)) %>% 
  as.data.frame() %>% 
  dplyr::select(start, names) %>% 
  separate(names, c("LTR_range", "LTR_subclass", "LTR_class"), sep = "\\|") %>% 
  mutate(pattern = "AAGTGTAAGC", 
         pattern_class = "MT")

# get MT2 pattern start positions - BLUE
MT2_splice_distribution <- 
  unlist(vmatchPattern("GACTGTAAGT", LTR_seqences, fixed = F)) %>% 
  as.data.frame() %>% 
  dplyr::select(start, names) %>% 
  separate(names, c("LTR_range", "LTR_subclass", "LTR_class"), sep = "\\|") %>% 
  mutate(pattern = "GACTGTAAGT", 
         pattern_class = "MT2")

# get ORR pattern start positions - BLACK
ORR_splice_distribution <- 
  unlist(vmatchPattern("AANCTGTAAGC", LTR_seqences, fixed = F)) %>% 
  as.data.frame() %>% 
  dplyr::select(start, names) %>% 
  separate(names, c("LTR_range", "LTR_subclass", "LTR_class"), sep = "\\|") %>% 
  mutate(pattern = "AANCTGTAAGC", 
         pattern_class = "ORR")
...
# LTR data frame
LTR_class_df <-
  data.frame(LTR_class = LTR_order, 
             LTR_family = c(rep("MLT", 2), rep("ORR", 7), rep("MT", 6), rep("MT2", 4)), 
             stringsAsFactors = F)

# join all distribution 
distribution_all <-
  rbind(polyA_distribution, MT2_splice_distribution, ORR_splice_distribution, MT_splice_distribution) %>% 
  ...
  dplyr::select(start, LTR_subclass, LTR_class = LTR_class.x, LTR_width, consensus_width, LTR_strand, pattern, pattern_class) %>% 
  mutate(start = -(LTR_width - start), 
         pattern = factor(pattern, levels = c("polyA", "AAGTGTAAGC", "GACTGTAAGT", "AANCTGTAAGC")),
         LTR_class = factor(LTR_class, levels = LTR_order)) %>% 
  dplyr::filter(start >= -(consensus_width)) %>% 
  left_join(LTR_class_df, by = "LTR_class") %>% 
  mutate(LTR_family = replace(LTR_family, pattern_class == "polyA", "polyA"), 
         forign = ifelse(pattern_class == LTR_family, F, T), 
         pattern_class = factor(pattern_class, levels = c("polyA", "ORR", "MT", "MT2"))) %>% 
  arrange(forign, pattern_class)
...
ggplot(data = distribution_all, aes(x = LTR_class, start)) +
  geom_bar(data = consensus_length_negative, aes(x = LTR_class, y = consensus_width), fill = "black", stat = "identity", alpha = 0.25) +
  geom_jitter(aes(colour = pattern),  size = 0.3, width = 0.25) +
  xlab("LTR class") +
  ylab("pattern start position") +
  ylim(c(min(consensus_length_negative$consensus_width), 0)) +
  scale_colour_manual(values = c("grey50", "red", 123, "black")) +
  ...
  ggsave(filename = paste0("LTR_random200_pattern_distribution_plot_new.png"), height = 20, width = 20, unit = "cm", dpi = 500)
