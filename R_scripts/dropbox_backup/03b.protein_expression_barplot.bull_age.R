...
fpkm_tb_list <- 
  purrr::map(fpkm_paths, function(path){
    ...
    if(str_detect(path, "bull")){
      ...
      fpkm_tb %<>% 
        dplyr::mutate(stage = str_extract(sample_id, "mature|postnatal|puberty"))
...
pirna_genes_fpkm <- 
  purrr::map(names(fpkm_tb_list), function(animal){
    ...
    fpkm_filt <- 
      ...
      dplyr::select(gene_id, sample_id, stage, fpkm) %>% 
      ...
      dplyr::select(gene_name, gene_id, mean, sd, SEM, animal, stage) %>% 
      dplyr::left_join(., pirna_genes_tb, by = "gene_name") %>% 
      dplyr::mutate(animal = str_c(animal, stage, sep = ".")) %>% 
      dplyr::select(-stage) %>% 
  ...
  dplyr::bind_rows(.) %>% 
  dplyr::mutate(animal = factor(animal, levels = c("mouse.GV", "rat.GV", "golden_hamster.GV", "cow.GV", "bull_testis.postnatal", "bull_testis.puberty", "bull_testis.mature")))
...
pirna_genes_plot <- 
  ...
  ggsave(filename = file.path(outpath, "piRNA_pathway_genes.FPKM.animals.barplot.2.png"), width = 15, height = 10)
