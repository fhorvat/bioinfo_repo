...
######################################################## WORKING DIRECTORY
setwd("/common/WORK/fhorvat/Projekti/Svoboda/hamster_KO/datasets/mouse_testis.small_RNAseq/Data/Documentation")
...
sample_table_tidy <- 
  ...
  dplyr::mutate(genotype = str_extract(genotype, "\\(WT|\\(KO") %>% str_remove(., "\\("))
...
sample_tb %<>% 
  ...
  dplyr::mutate(stage = "testis",
                genotype = str_c("Papd7_LacZ_", genotype)) %>% 
  dplyr::group_by(genotype) %>% 
  dplyr::mutate(replicate = 1:n(), 
                sample_id = str_c("s", stage, genotype, sample_name, str_c("r", replicate), sep = "_") %>% str_c(., ".SE"))
