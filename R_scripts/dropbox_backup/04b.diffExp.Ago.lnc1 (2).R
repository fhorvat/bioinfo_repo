...
sample_table_dds <- 
  ...
  dplyr::filter(str_detect(genotype, "Ago")) %>%
  as.data.frame(.) %>%
...
dds_shrink <- lfcShrink(dds_deseq, contrast = c("genotype", "Ago2_CI", "Ago2_WT"))
...
results_df <-
  ...
  dplyr::mutate(comparison = "Ago.CI_vs_WT")
  # write_csv(., path = file.path(outpath, str_c("diffExp", experiment, "Ago_CI_vs_WT", "ensembl", ensembl_version, "all_biotype.all.csv", sep = ".")))

# write only significant results, padj < 0.1
results_df %>%
  dplyr::filter(padj < 0.1)
  # write_csv(., path = file.path(outpath, str_c("diffExp", experiment, "Ago_CI_vs_WT", "ensembl", ensembl_version, "all_biotype.signif.csv", sep = ".")))
...
annotation_df <- 
  ...
  readr::write_csv(., "Sirena_Elob_Elobl.diffExp.Ago.CI_vs_WT.Stein.csv")
...
# save plot
ggsave(filename = file.path(outpath, str_c("MAplot", experiment, "Ago_CI_vs_WT", "ensembl", ensembl_version, "all_biotype.png", sep = ".")),
...
# save plot
ggsave(filename = file.path(outpath, str_c("MAplot", experiment, "Ago_CI_vs_WT", "ensembl", ensembl_version, "all_biotype.labels.png", sep = ".")),
       plot = ma_plot, width = 10, height = 10)
