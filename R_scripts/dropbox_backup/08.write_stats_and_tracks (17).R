...
tracks_tbl <-
  ...
  dplyr::mutate(sample_id = str_extract(URL, pattern = str_c(stats_tbl$sample_id, collapse = "|")),
                experiment = str_remove(URL, pattern = "\\/perfect_reads.genome.*") %>% basename(.),
                ...
                bw_name = ifelse(test = (scaled == "RPM_scaled"),
                                 yes = str_c(experiment, ".", str_replace(sample_id, "s_", ""), ".perfect.scaled.bw"),
                                 no = str_c(experiment, ".", str_replace(sample_id, "s_", ""), ".perfect.raw.bw")),
                bam_name = str_c(experiment, ".", str_replace(sample_id, "s_", ""), ".perfect.bam"),
...
stats_and_tracks <-
  ...
  readr::write_csv(., path = file.path(outpath, str_c("log.", unique(.$experiment), ".perfect.stats_and_tracks.csv")))
