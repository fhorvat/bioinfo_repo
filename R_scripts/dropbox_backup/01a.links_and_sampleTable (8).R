...
######################################################## WORKING DIRECTORY
setwd("/common/WORK/fhorvat/Projekti/Svoboda/Dicer_rat/datasets/tissues.Dicer_MT_HET.2021_Apr/Data/Documentation")
...
# raw files path
raw_path <- "/common/RAW/Svoboda/rat_tissues.Dicer_MT_HET.RNAseq.2021_Apr"
...
sample_table_tidy <- 
  sample_table_raw %>% 
  dplyr::select(tissue = sample, genotype, 
                barcode = `Barcode Sequence (6 nt)`, barcode_no = `Barcode number`) %>% 
  as_tibble(.) %>% 
  dplyr::filter(!is.na(barcode_no)) %>% 
  dplyr::mutate(tissue = tolower(tissue), 
                genotype = genotype %>% 
                  str_replace(., "MT \\+/-", "MT_HET") %>% 
                  str_replace(., "Wild-type", "WT")) %>% 
...
sample_tb <- 
  ...
  dplyr::mutate(barcode = raw_path %>% 
                  basename(.) %>% 
                  str_remove_all(., "\\.fastq\\.gz") %>% 
                  str_remove(., "_S[0-9].*")) %>% 
  dplyr::left_join(., sample_table_tidy, by = "barcode")
