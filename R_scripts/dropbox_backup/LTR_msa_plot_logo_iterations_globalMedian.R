...
# get sequences 
LTR_seqences <- getSeq(x = Mmusculus, rptmsk_filter_gr)
names(LTR_seqences) <- rptmsk_filter_gr$fullName

# get sequences with signal
signal_start_median <- 
  unlist(vmatchPattern("AATAAA", LTR_seqences)) %>% 
  as.data.frame() %>%
  separate(names, c("LTR_range", "LTR_subclass", "LTR_class"), sep = "\\|") %>% 
  group_by(LTR_class) %>% 
  summarize(start_med = round(median(start)))

signal_start_median[signal_start_median$LTR_class == "MTE", "start_med"] <- 308
...
for(LTR_sample in unique(LTR_data$LTR_class)[3:length(unique(LTR_data$LTR_class))]){
