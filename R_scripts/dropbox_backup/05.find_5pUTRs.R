...
line1_orfs_tb <- 
  ...
  dplyr::select(seqnames, start, end, strand, orf_start, repName, rmsk_id) %>% 
  dplyr::mutate(orf_abs_start = ifelse(strand == "+", start + orf_start + 1, end - orf_start - 1), 
                orf_abs_end = ifelse(strand == "+", orf_abs_start - 3000, orf_abs_start + 3000), 
                seq_start = ifelse(strand == "+", orf_abs_end, orf_abs_start), 
                seq_end = ifelse(strand == "+", orf_abs_start, orf_abs_end)) %>% 
  dplyr::select(seqnames, start = seq_start, end = seq_end, strand, rmsk_id, repName)
...
line1_orfs_seq <- reverseComplement(line1_orfs_seq)
line1_orfs_seq <- line1_orfs_seq[order(names(line1_orfs_seq))]

# save as fasta
Biostrings::writeXStringSet(x = line1_orfs_seq, filepath = file.path(outpath, "LINE1.CriGriPICR.3kb_up_from_ORF1.antisense.fasta"))


### MSA together
# msa
seq_msa <- msa::msa(line1_orfs_seq, method = "ClustalOmega")

# write as fasta
seq_msa@unmasked %>%
  Biostrings::writeXStringSet(., file = file.path(outpath, str_c("LINE1.CriGriPICR.3kb_up_from_ORF1.antisense", "ClustalO.msa", "fasta", sep = ".")))
...
# 
# # save with labels
# png(filename = file.path(outpath, str_c("LINE1.CriGriPICR.5K_to_7k.ORFs", "phylo_tree", "ClustalO.msa", "labels", "png", sep = ".")), width = 2000, height = 2000, units = "px")
