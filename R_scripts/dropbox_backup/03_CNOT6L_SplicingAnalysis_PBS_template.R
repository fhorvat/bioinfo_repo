...
################################################################################### PATH VARIABLES
# file <- %FILE
file <- "/common/WORK/fhorvat/Projekti/Svoboda/CNOT6L/Data/Mapped/STAR_mm10/11919X10_151023_D00550_0299_BC7HM5ANXX_6/11919X10_151023_D00550_0299_BC7HM5ANXX_6.bam"

r_outpath <- "/common/WORK/fhorvat/Projekti/Svoboda/CNOT6L/Analysis/CNOT6L_2017/spliced_reads/RData/Splicing_Analysis_RData"
...
# genes_sel_path <- "/common/WORK/fhorvat/Projekti/Svoboda/CNOT6L/Analysis/CNOT6L_2017/spliced_reads/documentation/GeneCount_RepsRemoved_RPKM.txt"
genes_sel_path <- "/common/WORK/fhorvat/Projekti/Svoboda/CNOT6L/Analysis/CNOT6L_2017/spliced_reads/documentation/GeneCount_RepsRemoved_RPKM_average.txt"
...
genes_sel_int <- with(genes_sel, genes_sel[order(-genes_sel$s_1C_WT.int_reps_rem), ][1:1000, ])
# get top 1000 genes by ratio of expression in introns / exons in 1-cell stage
genes_sel_rat <- with(genes_sel, genes_sel[order(-log2((s_1C_WT.int_reps_rem + 1) / (s_1C_WT.ex + 1))), ][1:1000, ])

# get genes which have expression in introns in 1-cell
genes_sel_samp_rat <- genes_sel[genes_sel$s_1C_WT.int_reps_rem != 0, ]
# get top 1000 of those genes based on ratio of reads mapped to introns/exons in 1-cell / MII stages 
genes_sel_samp_rat <- with(genes_sel_samp_rat, genes_sel_samp_rat[order(-log2((s_1C_WT.int_reps_rem + 1) / (s_1C_WT.ex + 1)) /
                                                                          log2((s_MII_WT.int_reps_rem + 1) / (s_MII_WT.ex + 1))), ][1:1000, ])

# get genes which are not expressed in MII exons and are expressed in 1-cell exons
genes_sel_mii_ex <- subset(genes_sel, s_MII_WT.ex == 0 & s_1C_WT.int_reps_rem != 0)
# order those genes by expression in introns in 1-cell stage
genes_sel_mii_ex <- genes_sel_mii_ex[order(-genes_sel_mii_ex$s_1C_WT.int_reps_rem), ]

# get genes which are not expressed in MII introns and are expressed in 1-cell introns
genes_sel_mii_int <- subset(genes_sel, s_MII_WT.int_reps_rem == 0 & s_1C_WT.int_reps_rem != 0)
# order those genes by expression in introns in 1-cell stage
genes_sel_mii_int <- genes_sel_mii_int[order(-genes_sel_mii_int$s_1C_WT.int_reps_rem), ]

# get genes which are not expressed in exons/introns in MII stage and which are expressed in 1-cell stage introns
genes_sel_mii <- subset(genes_sel, s_MII_WT.int_reps_rem == 0 & s_1C_WT.int_reps_rem != 0 & s_MII_WT.ex == 0)
