...
######################################################## WORKING DIRECTORY
setwd("/common/WORK/fhorvat/Projekti/Svoboda/lncRNA_KO/Analysis/lnc1_paper/rodents_evolution/lnc1_locus/flanking_genes.mRNA")
...
# blat main path
blat_path <- file.path(inpath, "blat")
...
# fasta index path
fasta_index_path <- list.files(path = inpath, pattern = ".*\\.fa\\.fai$", full.names = T)
