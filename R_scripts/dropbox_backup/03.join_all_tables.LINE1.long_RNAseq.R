...
######################################################## WORKING DIRECTORY
setwd("/common/WORK/fhorvat/Projekti/Svoboda/piRNA.Zuzka/Analysis/2020_paper/retrotransposons_annotation/golden_hamster.Siomi_assembly/LINE1/expression/long_RNAseq")
...
rpm_tb_path <- list.files(inpath, "LINE1_.*\\.RPM\\.[0-6]{8}\\.csv", full.names = T, recursive = F)
rpm_tb_path <- rpm_tb_path[str_detect(rpm_tb_path, "20210512")]
...
rpm_wide <- 
  ...
  readr::write_csv(., file.path(outpath, str_c("LINE1s_subsets.long_RNAseq", 
                                               "all_samples_RPM.20210512.csv", 
                                               sep = ".")))
...
# save
readr::write_csv(rpm_mean, file.path(outpath, str_c("LINE1s_subsets.long_RNAseq", 
                                                    "mean_RPM.20210512.csv", 
                                                    sep = ".")))
