...
genes_tb_path <- file.path(inpath, "hemoglobin_gene_coordinates.csv")

# exonic coordinates path
exons_tb_path <- file.path(inpath, "hemoglobin_exons_coordinates.csv")

# gene sequences path
genes_seq_path <- file.path(inpath, "hemoglobin_gene_sequences.DNAStringSet.RDS")

# mouse hemoglobin path
mouse_hemoglobin_path <- file.path(inpath, "P01942.2.hemoglobin.mmusculus.fasta")
...
exons_gr <- 
  ...
  dplyr::mutate(exon_start = start - gene_start + 1, 
                ...
                strand = "*") %>% 
  dplyr::select(seqnames = ensembl_name, start = exon_start, end = exon_end) %>% 
...
transcripts_tb <- 
  ...
  dplyr::mutate(ensembl_name = str_remove(exon_id, "\\.\\d$")) %>% 
...
names(transcripts_seq) <- transcripts_tb$common_name
