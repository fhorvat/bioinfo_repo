...
# experiment path 
experiment_path <- "/common/WORK/fhorvat/Projekti/Svoboda/accessory_data_sets/arrays/Morgan_2017_Nature_EMTAB5056"

# sample table path
sample_table_path <- file.path(experiment_path, "E-MTAB-5056.sdrf.txt")

# cel files path
cel_path <- list.files(experiment_path, pattern = ".CEL", full.names = T)
...
sample_table_filt <- 
  sample_table %>% 
  dplyr::select(sample_name = `Source Name`, stage = `Factor Value[cell type]`, 
                array_id = `Array Data File`) %>% 
  dplyr::mutate(genotype = toupper(stringr::str_extract(sample_name, "ko|ctrl")), 
                genotype = replace(genotype, genotype == "CTRL", "WT"), 
                cel_path = file.path(experiment_path, array_id)) %>% 
  as.data.frame(.) %>% 
...
exprs_matrix <-
  ...
  .[, str_detect(colnames(.), "gv_ctrl|gv_dcko")] %>%
  2^.
...
up_genes <-
  ...
  dplyr::mutate(gene_id1 = AnnotationDbi::mapIds(mogene20sttranscriptcluster.db, keys = probe_id, column = "ENSEMBL", keytype = "PROBEID", multiVals = "first"), 
                gene_id2 = AnnotationDbi::mapIds(mogene20stprobeset.db, keys = probe_id, column = "ENSEMBL", keytype = "PROBEID", multiVals = "first")) %>% 
  mutate_cond(!is.na(gene_id1), gene_id = gene_id1) %>% 
  mutate_cond(!is.na(gene_id2), gene_id = gene_id2) %>% 
  dplyr::filter(!is.na(gene_id)) %>% 
  dplyr::select(gene_id, logFC) %T>% 
  readr::write_csv(., path = file.path(outpath, str_c(experiment_name, ".affy.MoGene2.0st.KOvsWT.upregulated.csv")))
