...
mirna_targets_path <- file.path(accessory_data_path, "targetScan", "targetScan.mouse.7.1.20181109.conserved_family.conserved_and_unconserved_targets.mouse.txt.gz")

# mouse 3'UTR sequences from targetScan path
mouse_3UTRs_path <- file.path(accessory_data_path, "targetScan", "targetScan.mouse.7.1.20181112.UTR_sequences.mouse.txt")
...
mouse_3UTRs <- readr::read_delim(mouse_3UTRs_path, 
                                 delim = "\t", 
                                 skip = 1, 
...
purrr::map(mirna_top10, function(unique_mir_family){
  ...
  mirna_targets_list <-
    list(mirna_targets_manual, mirna_targets_targetScan) %>% 
