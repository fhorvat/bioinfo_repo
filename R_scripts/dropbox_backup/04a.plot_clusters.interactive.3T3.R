...
# mapped path
mapped_path <- "/common/WORK/fhorvat/Projekti/Svoboda/smallRNASeq_2018/SOLiD/T3T_DcrTrans_2011/Data/Mapped/Shrimp_mm10"

# sample bam path
sample_bam_path <- list.files(path = mapped_path, pattern = ".*mis_0.bam", full.names = T)[1]
...
for(n in 1:length(cluster_path)){
  ...
  sample_name <- 
    cluster_path[n] %>% 
    basename(.) %>% 
    stringr::str_extract("DicerS|DicerWT")
  ...
  cluster_df <- 
    ...
    readr::write_csv(., file.path(outpath, str_c("clusters.3T3", "DcrO", sample_name, "union", "rpm", 3, "width", 50, "mean.detailed", "csv", sep = ".")))
  ...
  p <-
    plotly::plot_ly(data = plot_df,
                    x = ~x,
                    y = ~y,
                    text = ~paste("</br>", str_remove(mean.x, ".tran.*|.utran.*"), "RPM:", rpm_x, 
                                  "</br>", str_remove(mean.y, ".tran.*|.utran.*"), "RPM:", rpm_y, 
                                  "</br>", coordinates,
                                  "</br>", class, 
  ...
  htmlwidgets::saveWidget(as_widget(p),
                          file = file.path(outpath, "test.html"),
                          selfcontained = T)
  
  # save as html widget
  htmlwidgets::saveWidget(as_widget(p),
                          file = file.path(outpath, str_c("clusters.3T3", "DicerO", sample_name, "union", "rpm", 3,
