...
# {1} LIBRARIES
lib.path='/common/WORK/fhorvat/code_library/R_scripts/vfranke'
...
# {2} CODE
	...
	# {{2}} INPUT VARIABLES 
		...
		# {{{1}}} PATH VARIABLES
		inpath='/common/WORK/vfranke/Projects/Dicer_RNASeq_29062012/Results/MTEvolution/RodentPhylogeny'
		outpath <- "/common/WORK/fhorvat/Projekti/Svoboda/Analyses/Shubhangini/Ago2_repeats_evolution"
		clustalo_path <- "/common/WORK/fhorvat/programi/clustal-omega-1.2.3/bin/clustalo"
		  
		seq.path = file.path(inpath, 'Sequences/Rodents')
		mult.path = file.path(outpath, '/MultipleAlignment/ClustalO')
	...
	for(i in 1:length(infiles)){
		...
		a = ClustalO(file, outfile, ClustalO=clustalo_path, what=what, format='fa', force=T)
