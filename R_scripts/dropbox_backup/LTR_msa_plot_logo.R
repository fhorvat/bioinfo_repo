...
setwd("/common/WORK/fhorvat/Projekti/Svoboda/Other/MT_elements/review/LTR_alignment/files/seq_logo")
...
msaPlot <- function(aligned_sequences, file_name){
...
#           axis.text.y = element_text(size = 1, hjust = 1, vjust = 0),
    ...
    ggsave(paste0(file_name))
  
  cat(paste0(file_name, " MSA plot done\n"))
  
}

# pattern matching in sequences, plots sequence logo around pattern 
signalLogo <- function(sequences, signal_pattern, file_name){
  
  sequences = LTR_seqences
  signal_pattern = "AATAAA"
  file_name = LTR_sample_file_name
  
  # get the most common signal start 
  signal_start <- 
    as.data.frame(vmatchPattern(signal_pattern, sequences)) %$%
    start %>% 
    removeOutliers() %>% 
    na.omit() %>% 
    median()
  ...
  sub_sequences_end <- ifelse(width(sequences) < signal_start + 20, width(sequences), signal_start + 20)
  
  # get the subsequence 40 nt around the signal start
  sub_sequences <- subseq(x = sequences, start = signal_start - 20, end = sub_sequences_end)
  ...
  signal_start <- 
    as.data.frame(vmatchPattern(signal_pattern, sub_sequences_aligned, max.mismatch = 0)) %$%
    start %>% 
    Mode() 
  
#   sub_sequences_aligned_2 <- subseq(x = sub_sequences_aligned, start = signal_start - 20, end = signal_start + 5)
#   sub_sequences_aligned_2 <- as.character(sub_sequences_aligned_2)
#   sub_sequences_aligned_2 <- paste(substr(sub_sequences_aligned_2, 1, 5), substr(sub_sequences_aligned_2, 7, nchar(x)), sep = '')
#   
  ...
  # plot MSA of signal
#   msaPlot(aligned_sequences = sub_sequences_aligned, file_name = paste0(file_name, "_signal"))
...
for(LTR_sample in unique(LTR_data$LTR_class)){
  ...
  LTR_sample_file_name <- paste0(LTR_sample, "_random_", length(LTR_rptmsk_sample))
  ...
  write.fasta(as.list(as.character(LTR_seqences)),
              nbchar = 80,
              names =  names(LTR_seqences),
              as.string = TRUE,
              file.out = paste0(LTR_sample_file_name, ".fasta"),
              open = "w")
  
  # align by Clustal Omega
  LTR_seqences_aligned <- ClustalO(infile = paste0(getwd(), "/", LTR_sample_file_name, ".fasta"), 
                                   outfile = paste0(getwd(), "/", LTR_sample_file_name, "_msa.fasta"), 
                                   threads = 8, 
                                   force = T)
  
  # plot MSA as "heatmap"
  msaPlot(aligned_sequences = LTR_seqences_aligned, file_name = LTR_sample_file_name)
  
  # get signal logo and MSA plot
  signalLogo(sequences = LTR_seqences, signal_pattern = "AATAAA", file_name = LTR_sample_file_name)
