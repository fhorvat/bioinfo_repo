...
######################################################## WORKING DIRECTORY
setwd("/common/WORK/fhorvat/Projekti/Svoboda/Split_RNAseq_2017/mouse/Data/transcriptome_assemblies")
...
trinity_path <- list.files(inpath, pattern = ".*fasta\\.stats", recursive = T)

# find reads-to-transcriptome counts
r2t_counts_path <- list.files(inpath, pattern = ".*r2t\\.read_counts\\.txt", recursive = T)

# find transcripts-to-genome counts
t2g_counts_path <- list.files(inpath, pattern = ".*t2g\\.read_counts\\.txt", recursive = T)

# find Busco reports
busco_path <- list.files(inpath, pattern = "short_summary_.*\\.busco\\.mammalia.txt", recursive = T)
