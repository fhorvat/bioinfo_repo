...
colnames(rpkm_new) <- c(paste0("GV KO", 1:3), 
                        paste0("GV WT", 1:3), 
                        paste0("MII KO", 1:3), 
                        paste0("MII WT", 1:3),
                        paste0("1C KO", 1:3), 
...
pheatmap_method <- function(rpkm_new, method){
  pheatmap(rpkm_new,
           clustering_method = method,
           main = paste("log2 RPKM all genes", method, "method"),
           col = hmcols, 
           breaks = bk,
           cluster_row = T, 
           cluster_cols = F,
           ...
           treeheight_col	= 100,
           show_rownames = F)
  savepng(paste0("pheatmap_log2_RPKM_all_samples_", method), width = 1000)  
}

lapply(c("ward.D", "ward.D2", "complete", "average"), pheatmap_method, rpkm_new = rpkm_new)
