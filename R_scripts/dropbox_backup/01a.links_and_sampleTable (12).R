...
sample_table_tidy <- 
  sample_table_raw %>% 
  dplyr::select(sample_name = sample, genotype = `genotype Mov10l`, age, barcode = `Barcode seq`, bc = `Barcode number`)
...
sample_tb %>% 
  dplyr::select(sample_id, stage, genotype, age, replicate, bc, barcode, sample_name, sample_file_name) %>% 
  dplyr::distinct(.) %>% 
