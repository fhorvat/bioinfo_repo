...
rpkm_tb_path <- list.files(rpkm_tb_path, pattern = ".*\\.8.5dpp.RPKM_mean\\.csv$", full.names = T)
...
######################################################## MAIN CODE
# set name
table_name <- "MesAur1.1k_windows.8.5dpp"
...
clusters_8.5dpp <- 
  ...
  readr::write_csv(., file.path(outpath, "MesAur1.1k_windows.rpkm_cutoff.1.8.5dpp.merged_clusters_and_superclusters.RPKM_and_RPM.csv"))
