...
meth_sites <- 
  ...
  dplyr::filter(Mov10l1_HET >= 9 & Mov10l1_KO >= 9)
...
purrr::map(names(plot_list), function(name){
  ...
  ggsave(plot = violin_plot,
         filename = file.path(outpath, str_c("violin", "methylation_percentage", name, "500bp_prom", "png", sep = ".")),
         width = 12, height = 10)
