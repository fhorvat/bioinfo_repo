...
getClusters <- function(tiles_tb, super_clusters = T, merge_width = 2000){
  ...
  clusters_rpkm <- 
    ...
    as_tibble(.) %>% 
    dplyr::mutate(cluster_id = subjectHits(overlaps)) %>% 
    dplyr::select(cluster_id, WT_13dpp, KO_13dpp, norm_width) %>% 
...
### 21 dpp
# read RPM table
rpm_tb_21dpp <- readr::read_csv(rpm_tb_path_21dpp)

# read RPKM table
rpkm_tb_21dpp <- readr::read_csv(rpkm_tb_path_21dpp)


### 13 dpp
# read RPM table
rpm_tb_13dpp <- readr::read_csv(rpm_tb_path_13dpp)

# read RPKM table
rpkm_tb_13dpp <- readr::read_csv(rpkm_tb_path_13dpp)
...
tiles_fc <-
  rpkm_tb %>%
  set_colnames(., str_remove(colnames(.), "Mov10l_")) %>%
  dplyr::select(gene_id, coordinates, WT_13dpp, KO_13dpp, WT_21dpp, KO_21dpp) %>%
...
# prepachytene clusters = merge if log2FC 13dpp KO/WT < -2, merge again if clusters are at most 2kb apart
clusters_13dpp <- 
  tiles_fc %>% 
  dplyr::filter(log2FC_13dpp <= -2, log2FC_21dpp <= -2) %>% 
  getClusters(tiles_tb = ., super_clusters = T, merge_width = 2001) %>% 
  dplyr::filter(rpkm_log2FC_13dpp <= -2, rpkm_log2FC_21dpp <= -2) %>% 
  tidyr::separate(coordinates, c("seqnames", "start", "end"), sep = " ") %>% 
  GRanges(.) %>%  
  as_tibble(.) %>% 
  select(-strand) %>% 
  ...
  readr::write_csv(., file.path(outpath, "MesAur1.1k_windows.rpkm_cutoff.1.prepachytene.merged_clusters_and_superclusters.RPKM_and_RPM.csv"))

# pachytene clusters = merge if log2FC 21D KO/WT <-2, log2FC 13D KO/WT >= -2, merge again if clusters are at most 1kb appart
clusters_21dpp <- 
  tiles_fc %>% 
  dplyr::filter(log2FC_13dpp > -2, log2FC_21dpp <= -2) %>% 
  ...
  dplyr::select(coordinates:width_without_N, contains("rpkm"), contains("rpm")) %>%  
  dplyr::arrange(-width_without_N) %T>% 
  readr::write_csv(., file.path(outpath, "MesAur1.1k_windows.rpkm_cutoff.1.pachytene.merged_clusters_and_superclusters.RPKM_and_RPM.csv"))
