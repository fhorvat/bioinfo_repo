...
ltr_tb_path <- file.path(inpath, "LTRs.random_200_per_subfamily.80_percent_of_length.csv")
...
ltr_seq_list <- 
  ...
  set_names(., ltr_seq_path %>% basename(.) %>% str_remove(., "\\.80_percent_of_length\\.single\\.fasta") %>% str_remove(., ".*\\."))
...
sub_rate_tb <- purrr::map(names(sub_rate), function(ltr_name){
  ...
  bind_rows(.) %>% 
  dplyr::mutate(repSubfamily = factor(repSubfamily, levels = rev(unique(ltr_tb$repSubfamily)))) %>% 
...
ggsave(plot = sub_rate_boxplot, 
       filename = file.path(outpath, str_c("LTRs.random_200_per_subfamily", "substitution_rate", "boxplot.png", sep = ".")), 
       width = 20, 
       height = 10)
