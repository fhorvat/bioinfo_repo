...
### sample 50 LTRs of full length
# sample LTRs - 50 in each repeat subfamily
set.seed(1234)
ltr_sample_tb <- 
  ltrs_filt %>%
...
# save table
readr::write_csv(ltr_sample_tb, file.path(outpath, "LTRs.random_50_per_repName.raw_rmsk.csv"))
...
purrr::map(names(ltr_sample_seq_list), function(ltr_name){
  ...
  ltr_sample_seq_list[[ltr_name]] %T>% 
    Biostrings::writeXStringSet(., file.path(outpath, str_c("LTRs.random_50_per_repName", ltr_name, "raw_rmsk.single.fasta", sep = ".")))
