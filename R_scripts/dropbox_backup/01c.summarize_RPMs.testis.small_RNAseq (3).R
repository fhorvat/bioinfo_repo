...
rpm_tb <- purrr::map(rpm_paths, function(path){
  ...
  tidyr::pivot_wider(., id_cols = subset, names_from = c(sense, genotype_age), values_from = RPM.24to31nt, names_prefix = "RPM.24to31nt.") %T>%
  readr::write_csv(., file.path(outpath, "LINE1s_subsets.testis.small_RNAseq.RPM.complete.csv"))
