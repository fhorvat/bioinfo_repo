...
#PBS -m n
#PBS -N pbs.07b.annotate_genome.GeMoMa
#PBS -J 0-1
#PBS -j oe
cd $PBS_O_WORKDIR
...
# tblastn results
INPUT_DIR=.
IN_TBLASTN=($(find $INPUT_DIR -type d -name "mmseqs.GCF_*"))
FILE_TBLASTN=${IN_TBLASTN[$PBS_ARRAY_INDEX]}
BASE_TBLASTN=${FILE_TBLASTN#${INPUT_DIR}/}
BASE_TBLASTN=${BASE_TBLASTN#mmseqs.}
...
outdir=gene_models.mmseqs.${BASE_TBLASTN}
