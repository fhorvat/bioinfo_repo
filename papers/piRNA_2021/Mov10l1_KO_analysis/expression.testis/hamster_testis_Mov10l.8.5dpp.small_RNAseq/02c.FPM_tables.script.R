...
######################################################## FUNCTIONS
# calculate standard error of the mean FPM value
...
fpm_wide_tb <-
  fpm_long_tb %>%
  dcast(., gene_id ~ sample_id, value.var = "fpm") %>%
...
fpm_mean_tb <-
  fpm_long_tb %>%
  .[, .(fpm_mean = round(mean(fpm), 3)), by = c("gene_id", grouping_variables)] %>%
  .[, grouped_variables := do.call(str_c, c(.SD, sep = "_")), .SDcols = grouping_variables] %>%
  dcast(., gene_id ~ grouped_variables, value.var = "fpm_mean") %>%
  .[genes_info, on = "gene_id", `:=`(gene_name = i.gene_name,
                                     coordinates = str_c(i.seqnames, i.start, i.end, sep = " "),
                                     strand = i.strand,
                                     gene_biotype = i.gene_biotype,
