...
genes_info_paths <- genes_info_paths[!str_detect(genes_info_paths, 
                                                 "ensembl.99.MesAur1.0.20200415.UCSCseqnames.geneInfo.csv|ensembl.99.MesAur1.0.20200415.UCSCseqnames.NCBI_PIWIL3.geneInfo.csv|ensembl.99.Rnor_6.0.20200415.UCSCseqnames.geneInfo.csv")]

######################################################## READ DATA
# read genes info
genes_info <- 
  purrr::map(genes_info_paths, readr::read_csv) %>% 
  set_names(., str_extract(genes_info_paths, "cow|mouse|rat|golden_hamster|human"))
...
# create table
pirna_genes_tb <- tibble(gene_name = c("Henmt1", "Mael", "Mov10l1", 
                                       "Piwil1", "Piwil2", "Piwil3", "Piwil4", 
                                       "Pld6", 
                                       "Rnf17", "Tdrd1", "Tdrd12", "Tdrd6", "Tdrd9", "Tdrkh", 
                                       "Spocd1", "Tex15"),
                         other_name = c("HENMT1", "MAEL/Maelstorm", "MOV10L1", 
                                        "MIWI", "MILI", "PIWIL3", "MIWI2", 
                                        "PLD6/MITOPOLD/Zucchini", 
                                        "TDRD4/RNF17", "TDRD1", "TDRD12", "TDRD6", "TDRD9", "TDRD2/TDRKH", 
                                        "SPOCD1", "TEX15"))

# filter gene names
gene_names_list <- purrr::map(names(genes_info), function(animal){
  
  # filter
  gene_names_tb <- genes_info[[animal]]
  
  # if cow then uppercase gene names
  if(animal %in% c("cow", "human")){
...
gene_names_tb <- 
  ...
  dplyr::select(gene_name, gene_id.mouse, gene_id.rat, gene_id.golden_hamster, gene_id.cow, gene_id.human)

# save
readr::write_csv(gene_names_tb, file = file.path(outpath, "piRNA_proteins.gene_ids.csv"))
