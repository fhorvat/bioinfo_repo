...
refSeqToEnsemblStyleGtf <- function(gene_name){
  ...
  if(length(refseq_gtf_filt) > 0){
    ...
    # change seqnames
    new_seqname <- scaffold_names[scaffold_names$refSeq_accn == as.character(seqnames(refseq_gtf_filt)[1]), ]$genBank_accn
...
refSeqToGeneInfo <- function(gene_name){
  ...
  if(length(refseq_gtf_filt) > 0){
    ...
    refseq_geneInfo <- tibble(gene_id = mcols(refseq_gtf_gene)$gene, 
                              seqnames = scaffold_names[scaffold_names$refSeq_accn == as.character(seqnames(refseq_gtf_gene)), ]$genBank_accn,
...
# ensembl gtf path
ensembl_gtf_path <- file.path(inpath, "..", "ensembl.99.MesAur1.0.20200415.UCSCseqnames.gtf")
...
# scaffold names path
scaffold_names_path <- file.path(inpath, "..", "GCF_000349665.1_MesAur1.0_assembly_report.txt")
