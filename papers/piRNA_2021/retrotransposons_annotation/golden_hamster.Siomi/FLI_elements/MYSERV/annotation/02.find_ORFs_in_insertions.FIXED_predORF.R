...
insertions_path <- file.path(inpath, "MYSERV.ltr-1_family-16.2.7_to_3k.csv")

# fasta files path
fasta_files_path <- file.path(inpath, "MYSERV.ltr-1_family-16.2.7_to_3k.fasta")
...
# filter by combination of ORFs and save
insertions_tb_full %>% 
  # dplyr::filter(longest_orf_1 == 312, longest_orf_2 %in% c(264)) %T>% 
  readr::write_csv(., file.path(outpath, "MYSERV.ltr-1_family-16.2.7_to_3k.ORFs.csv"))
