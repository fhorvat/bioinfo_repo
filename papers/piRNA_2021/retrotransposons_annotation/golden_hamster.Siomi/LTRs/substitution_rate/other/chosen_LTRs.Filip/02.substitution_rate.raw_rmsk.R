...
ltr_tb_path <- file.path(inpath, "LTRs.random_200_per_subfamily.80_percent_of_length.raw_rmsk.csv")
...
ltr_seq_list <- 
  ...
  set_names(., ltr_seq_path %>% basename(.) %>% str_remove(., "\\.80_percent_of_length.*") %>% str_remove(., ".*\\."))
...
ggsave(plot = sub_rate_boxplot, 
       filename = file.path(outpath, str_c("LTRs.random_200_per_subfamily", "substitution_rate", "raw_rmsk", "boxplot.png", sep = ".")), 
       width = 20, 
       height = 10)
