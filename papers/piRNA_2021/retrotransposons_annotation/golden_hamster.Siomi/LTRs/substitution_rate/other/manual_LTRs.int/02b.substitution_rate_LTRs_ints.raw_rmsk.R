...
######################################################## WORKING DIRECTORY
# set working dir
setwd("/common/WORK/fhorvat/Projekti/Svoboda/piRNA.Zuzka/Analysis/2020_paper/retrotransposons_annotation/golden_hamster.Siomi_assembly/substitution_rate/random_sampled_LTRs.manual_classes.ints")
...
ltr_tb_path <- file.path(inpath, "LTRs.random_200_per_subfamily.raw_rmsk.all_LTR_classes.200729.csv")

# list subfamily fasta files
ltr_seq_path <- list.files(inpath, pattern = ".*\\.single\\.fasta", full.names = T)

# chosen LTR classes path
classes_tb_path <- file.path(inpath, "..", "all_LTR_classes 200729.xlsx")

# classes summary payh
classes_sum_path <- file.path(inpath, "..", "all_LTR_classes.csv")
...
ltr_seq_list <- 
  ...
  set_names(., ltr_seq_path %>% basename(.) %>% str_remove_all(., "^LTRs\\.random_200_per_subfamily\\.|\\.raw_rmsk\\.all.*"))
...
# read table with classes summary
classes_sum <- readr::read_csv(classes_sum_path)

######################################################## MAIN CODE
# add category to classes summary table
classes_categories <- 
  classes_tb %>% 
  as_tibble(.) %>% 
  dplyr::select(repName, repSubfamily = categories.I) %>% 
  dplyr::left_join(., classes_sum, by = "repName") %>% 
  dplyr::select(repName, repSubfamily, repFamily) %>% 
  dplyr::mutate(repFamily = replace(repFamily, is.na(repFamily), "other")) %>% 
  dplyr::count(., repSubfamily, repFamily) %>% 
...
# saveRDS(sub_rate, file = file.path(outpath, "LTRs.random_200_per_subfamily.raw_rmsk.all_LTR_classes.200729.align.RDS"))

# load file from .RDS
sub_rate <- readRDS(file = file.path(outpath, "LTRs.random_200_per_subfamily.raw_rmsk.all_LTR_classes.200729.align.RDS"))
...
sub_rate_tb <- purrr::map(names(sub_rate), function(ltr_name){
  ...
  bind_rows(.) %>% 
  dplyr::left_join(., classes_categories, by = "repSubfamily") %>%
  dplyr::mutate(repFamily = factor(repFamily, levels = c("ERV1", "ERVK", "ERVL", "ERVL-MaLR", "Gypsy", "other"))) %>% 
  dplyr::arrange(repFamily, repSubfamily) %>% 
...
ggsave(plot = sub_rate_boxplot, 
       filename = file.path(outpath, str_c("LTRs.random_200_per_subfamily.raw_rmsk.all_LTR_classes.int.200729", "boxplot.png", sep = ".")), 
       width = 20, 
       height = 10)
