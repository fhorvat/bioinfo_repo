...
######################################################## MAIN CODE
### filter classes table - get those which are not -int
# filter
classes_tb %<>% 
  as_tibble(.) %>% 
...
ltrs_raw <- 
  ...
  dplyr::select(seqnames:strand, repName, repSubfamily = categories.I, repeatStart:repeatLeft, repeatWidth, genomeWidth, consensusWidth, rmsk_id) 
...
ltr_sample_tb <- 
  ...
  dplyr::ungroup(.) %>% 
  dplyr::mutate(repSubfamily = factor(repSubfamily, levels = unique(classes_tb$categories.I))) %>% 
  dplyr::arrange(repSubfamily)

# save table
readr::write_csv(ltr_sample_tb, file.path(outpath, "LTRs.random_200_per_subfamily.raw_rmsk.all_LTR_classes.200729.csv"))
...
purrr::map(names(ltr_sample_seq_list), function(ltr_name){
  ...
  ltr_sample_seq_list[[ltr_name]] %T>% 
    Biostrings::writeXStringSet(., file.path(outpath, str_c("LTRs.random_200_per_subfamily", ltr_name, "raw_rmsk.all_LTR_classes.200729.single.fasta", sep = ".")))
