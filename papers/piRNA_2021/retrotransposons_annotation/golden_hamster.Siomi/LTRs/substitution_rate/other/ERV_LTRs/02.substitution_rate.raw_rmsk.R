...
ltr_tb_path <- file.path(inpath, "LTRs.random_50_per_repName.ERV.raw_rmsk.csv")
...
ltr_seq_list <- 
  ...
  set_names(., ltr_seq_path %>% basename(.) %>% str_remove(., "LTRs\\.random_50_per_repName\\.") %>% str_remove(., "\\.ERV\\.raw_.*"))
...
ggsave(plot = sub_rate_boxplot, 
       filename = file.path(outpath, str_c("LTRs.random_50_per_repName", 'ERV', "substitution_rate", "raw_rmsk", "boxplot.png", sep = ".")), 
       width = 20, 
       height = 10)
