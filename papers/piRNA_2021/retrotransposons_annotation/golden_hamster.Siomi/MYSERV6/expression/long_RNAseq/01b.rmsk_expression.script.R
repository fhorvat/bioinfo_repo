...
# summarize expression per rmsk_id and coordinates
overlaps_tb <-
  tibble(read_name = names(bam_hits),
         rmsk_id = mcols(rmsk_hits)$name, 
         rmsk_coords = str_c(seqnames(rmsk_hits), start(rmsk_hits), end(rmsk_hits), sep = " ")) %>%
  # dplyr::group_by(read_name) %>%
  # dplyr::sample_n(1) %>%
  # dplyr::ungroup(.) %>%
  dplyr::group_by(rmsk_coords, rmsk_id) %>%
