...
# get LTRs of interest
ltr_hits <- c("MYSERV6-int", "RLTR31B2", "IAPLTR3", "IAPLTR4", "Lx5", "Lx6")
...
rmod_class_tb <- 
  ...
  dplyr::filter(rmsk_repName %in% ltr_hits) %>% 
  ...
  dplyr::select(rmsk_repName, rmod_repName) %T>%
  readr::write_csv(., file = file.path(outpath, "repeatMasker_repeatModeler.repNames.individual_elements.csv"))
