...
######################################################## WORKING DIRECTORY
setwd("/common/WORK/fhorvat/Projekti/Svoboda/piRNA.Zuzka/Analysis/2020_paper/retrotransposons_annotation/MesAur1.LINE1_ORF1/expand_200b_in_ORF1")
...
blast_seq <- 
  blast_results[[1]] %>% 
  dplyr::filter(alignment_length > 175) %>%
  dplyr::filter(identity_perc > 75) %>% 
...
# get sequences, take only those starting with ATG
line1_seq <- getSeq(BSgenome.Maur.UCSC.MesAur1, line1_gr)
line1_seq <- line1_seq[str_detect(as.character(line1_seq), "^ATG")]

# filter ranges, expand, get sequences
line1_gr <- line1_gr[names(line1_gr) %in% names(line1_seq)]
line1_gr <- expandRange(line1_gr, upstream = 150, downstream = 0)
...
# save sequence
line1_seq_save <- line1_seq
names(line1_seq_save) <- str_replace_all(names(line1_seq_save), " ", "_")
Biostrings::writeXStringSet(line1_seq_save, file = file.path(outpath, str_c("LINE1.CriGriPICR.ORF1_expand_200.150_up.L1-1_CGr.consensus", "BLAST_hits_in_MesAur1", "fasta", sep = ".")))
...
pdf(file = file.path(outpath, 
                     str_c("LINE1.CriGriPICR.ORF1_expand_200.150_up.L1-1_CGr.consensus", "BLAST_hits_in_MesAur1", "ClustalO.msa", "phylo_tree", "pdf", sep = ".")), 
    width = 20, height = 120)
...
# table
tibble(coordinates = msa_tree$tip.label) %>% 
  dplyr::left_join(., line1_gr %>% 
                     as_tibble(.) %>% 
                     dplyr::select(-width) %>% 
                     tidyr::unite(col = coordinates, "seqnames", "start", "end", sep = " "), 
                   by = "coordinates") %T>% 
  readr::write_csv(., path = file.path(outpath, str_c("LINE1.CriGriPICR.ORF1_expand_200.150_up.L1-1_CGr.consensus", "BLAST_hits_in_MesAur1", "csv", sep = ".")))
