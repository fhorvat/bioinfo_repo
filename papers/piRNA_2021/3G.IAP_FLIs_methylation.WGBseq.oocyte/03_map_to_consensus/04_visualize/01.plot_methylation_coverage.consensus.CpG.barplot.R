...
######################################################## WORKING DIRECTORY
setwd("/common/WORK/fhorvat/Projekti/Svoboda/piRNA.Zuzka/Analysis/2020_paper/retrotransposons_annotation/golden_hamster.Siomi_assembly/FLI_elements/IAP/methylation/mapped/Nov_2020/FLI_consensus.IAPLTR3")
...
# sequences path
seq_path <- "../../../bismark_index/FLI_consensus.IAPLTR3"
...
methylation_out <- 
  methylation_tb %>% 
  tidyr::pivot_wider(id_cols = c(pos, genotype, rank), names_from = meth, values_from = count, names_prefix = "perc_") %>% 
  dplyr::left_join(., meth_tb %>% dplyr::select(pos = start, count_meth, count_unmeth, genotype), by = c("pos", "genotype")) %>% 
  dplyr::mutate(read_count = count_meth + count_unmeth) %>% 
  dplyr::select(position = rank, genotype, perc_meth, perc_unmeth, read_count) %>% 
  dplyr::mutate(position = str_remove_all(position, "r_|\\.pos.*")) %>% 
  tidyr::pivot_wider(id_cols = position, names_from = genotype, values_from = c(perc_meth, perc_unmeth, read_count)) %>% 
  dplyr::select(position, contains("Mov10l1_HET"), contains("Mov10l1_KO")) %T>%
  readr::write_csv(., file.path(outpath, str_c("IAPLTR3.FLI_consensus.methylation.barplot", "4_reads_filt", "CpG", "csv", sep = ".")))
...
ggsave(plot = baseplot, 
       filename = file.path(outpath, str_c("IAPLTR3.FLI_consensus.methylation.barplot", "4_reads_filt", "CpG", "png", sep = ".")), 
       width = 15, height = 7.5)
...
baseplot <- 
  ...
  scale_x_continuous(limits = c(1, max(methylation_tb$pos) + 10), 
                     breaks = seq(10, max(methylation_tb$pos) + 10, 10), 
                     labels = seq(10, max(methylation_tb$pos) + 10, 10)) + 
  facet_grid(rows = vars(genotype)) +
  xlab("") +
  ylab("") +
  theme_bw(base_size = 10) +
...
# save plot
ggsave(plot = baseplot, 
       filename = file.path(outpath, str_c("IAPLTR3.FLI_consensus.methylation.position_barplot", "4_reads_filt", "CpG", "png", sep = ".")), 
       width = 15, height = 7.5)
