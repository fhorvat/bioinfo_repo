...
# base path
base_path <- "/common/WORK/fhorvat/Projekti/Svoboda/hamster_KO/datasets/mouse_testis.small_RNAseq"
...
# library size path
library_size_path <- file.path(base_path, "Data/Mapped/STAR_mm10.new") 
...
counts2FPM <- function(counts_tb){
  ...
  counts_tb %>% 
    ...
    dplyr::mutate(sample_id = str_remove(sample_id, "_B[1,2].*\\.SE\\.24to31nt\\.bam$")) %>% 
