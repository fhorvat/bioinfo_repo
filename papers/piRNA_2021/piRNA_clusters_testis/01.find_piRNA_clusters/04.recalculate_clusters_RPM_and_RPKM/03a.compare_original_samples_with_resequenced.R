...
clusters_reseq_path <- file.path(inpath, "MesAur1.1k_pachytene_clusters.200730.recalculated.reseq.csv")
...
joined_tb <- 
  ...
  dplyr::left_join(., reseq_tb %>% dplyr::select(coordinates, 
                                                 rpkm_WT_13dpp.reseq = rpkm_WT_13dpp, rpkm_KO_13dpp.reseq = rpkm_KO_13dpp, 
                                                 rpkm_log2FC_13dpp.reseq = rpkm_log2FC_13dpp, 
                                                 rpm_WT_13dpp.reseq = rpm_WT_13dpp, rpm_KO_13dpp.reseq = rpm_KO_13dpp, 
                                                 rpm_log2FC_13dpp.reseq = rpm_log2FC_13dpp), 
                   by = "coordinates")
...
scatter_plot <- 
  ...
  xlab("original") + 
  ylab("resequencing") +
  ggtitle(str_c("13dpp", genotype, counts, sep = " ")) +
  theme_bw() + 
  ...
  ggsave(filename = file.path(outpath, 
                              str_c("MesAur1.1k_pachytene_clusters.200730", "scatter_plot.compare_with_reseq", counts, genotype, "png", sep = ".")), 
         width = 12, height = 12)
...
plot_tb <- 
  joined_tb %>% 
  dplyr::select(original = rpkm_log2FC_13dpp, resequecing = rpkm_log2FC_13dpp.reseq)
...
crosshair_plot <-
  ...
  xlab(str_c("log2 (13dpp Mov10l1 KO / WT) RPKM - original sequencing")) +
  ylab(str_c("log2 (13dpp Mov10l1 KO / WT) RPKM - repeated sequencing")) +
  theme_bw() +
...
ggsave(filename = file.path(outpath, str_c("MesAur1.1k_pachytene_clusters.200730", 
                                           "crosshair_plot.compare_with_reseq", 
                                           "log2_ratio", "png", sep = ".")),
       plot = crosshair_plot,
       width = 10, height = 10)
