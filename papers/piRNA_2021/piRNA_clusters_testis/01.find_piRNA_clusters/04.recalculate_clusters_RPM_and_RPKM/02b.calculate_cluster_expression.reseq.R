...
clusters_rpkm <- 
  rpm_gr %>% 
  as_tibble(.) %>% 
  dplyr::select(coordinates, WT_13dpp, KO_13dpp, KO_21dpp, norm_width) %>% 
...
# save 
readr::write_csv(clusters_tb_sorted, file.path(outpath, "MesAur1.1k_pachytene_clusters.200730.recalculated.reseq.csv"))
