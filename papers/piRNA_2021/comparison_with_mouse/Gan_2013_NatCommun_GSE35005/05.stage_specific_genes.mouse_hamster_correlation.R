...
ortho_path <- file.path(inpath, "Gan_2013_NatCommun_GSE35005.stage_specific_genes.hamster_gene_id.csv")
...
mouse_ortho_fpkm <- 
  ortho_tb %>% 
  dplyr::left_join(., mouse_tb, by = c("mouse_gene_id" = "gene_id")) %>% 
  dplyr::select(mouse_gene_id, SG_A:round_ST) %>% 
  dplyr::filter(!is.na(mouse_gene_id)) %>% 
  magrittr::set_colnames(., c("mouse_gene_id", str_c("mouse.", colnames(.)[2:ncol(.)])))

# hamster
hamster_ortho_fpkm <- 
  ortho_tb %>% 
  dplyr::left_join(., hamster_tb, by = c("hamster_gene_id" = "gene_id")) %>% 
  dplyr::select(mouse_gene_id, Mov10l_WT_13dpp:Mov10l_WT_adult, Mov10l_KO_13dpp:Mov10l_WT_adult) %>% 
  dplyr::filter(!is.na(mouse_gene_id)) %>% 
  magrittr::set_colnames(., str_remove(colnames(.), "Mov10l_")) %>% 
  magrittr::set_colnames(., c("mouse_gene_id", str_c("hamster.", colnames(.)[2:ncol(.)], "_testis")))

# joined
ortho_fpkm <- 
  left_join(mouse_ortho_fpkm, hamster_ortho_fpkm, by = "mouse_gene_id") %>% 
  dplyr::rename(gene_name = "mouse_gene_id") %>% 
...
ggsave(plot = heat_plot, 
       filename = file.path(outpath, str_c("ensembl", ensembl_version, "hamster_vs_mouse.stage_specific_genes.FPKM.correlation.png", sep = ".")), 
       width = 10, height = 10)