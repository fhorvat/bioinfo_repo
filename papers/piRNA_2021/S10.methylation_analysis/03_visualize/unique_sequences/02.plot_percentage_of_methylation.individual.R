...
meth_sites_genotype <- purrr::map(c("Mov10l1_WT", "Mov10l1_HET", "Mov10l1_KO"), function(one_genotype){
  ...
  meth_sites_hits <- 
    ...
    dplyr::right_join(., meth_sites_tb, by = c("hit_coordinate" = "coordinates")) %>% 
...
# split table to list 
meth_sites_list <- list(repeats = meth_sites_genotype %>% dplyr::filter(hit_class %in% c("LINE", "LTR", "SINE", "rmsk_other")), 
                        non_repetitive = meth_sites_genotype %>% dplyr::filter(!(hit_class %in% c("LINE", "LTR", "SINE", "rmsk_other"))))
...
purrr::map(names(meth_sites_list), function(genomic_region){
  ...
  # # plot table - mean percentage of methylation
  # plot_tb <-
  #   meth_sites_list[[genomic_region]] %>%
  #   dplyr::group_by(hit_class, genotype) %>%
  #   dplyr::summarise(meth_percentage = mean(meth_percentage)) %>%
  #   dplyr::ungroup(.) %>%
  #   dplyr::mutate(hit_class = factor(hit_class, levels = c(hierch_tb$hit_class, "intergenic")))
  
  # save as table
  readr::write_csv(plot_tb, file.path(outpath, str_c("methylation_percentage", genomic_region, "genomic_regions", "individual_coverage_filtering", "csv", sep = ".")))
  ...
  ggsave(plot = bar_plot,
         filename = file.path(outpath, str_c("barplot", "methylation_percentage", genomic_region, "genomic_regions", "individual_coverage_filtering", "png", sep = ".")),
         width = 12, height = 10)
  ...
  #        filename = file.path(outpath, str_c("violin", "methylation_percentage", genomic_region, "genomic_regions", "individual_coverage_filtering", "png", sep = ".")),
  #        width = 12, height = 10)
