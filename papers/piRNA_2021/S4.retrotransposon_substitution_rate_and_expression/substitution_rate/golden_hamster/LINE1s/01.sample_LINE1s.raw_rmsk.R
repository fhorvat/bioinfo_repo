...
# check random guy
line1_filt %>% 
  dplyr::filter(repName == "L1ME3C") %>% 
  dplyr::select(seqnames:strand, width = genomeWidth, repName, rmsk_id) %>% 
  dplyr::arrange(desc(width)) %T>%
  readr::write_csv(., file.path(outpath, "rmsk.Siomi.20200701.L1ME3C.csv"))
...
# save table
readr::write_csv(line1_sample_tb, file.path(outpath, "LINE1s.random_200_per_repName.200805.csv"))
...
# get sequences 
line1_sample_seq <- Biostrings::getSeq(BSgenome.Maur.UCSC.Siomi, line1_sample_gr)
...
purrr::map(names(line1_sample_seq_list), function(rep_name){
  ...
  line1_sample_seq_list[[rep_name]] %T>% 
    Biostrings::writeXStringSet(., file.path(outpath, "fasta_files", str_c("LINE1s.random_200_per_repName", rep_name, "200805.single.fasta", sep = ".")))
