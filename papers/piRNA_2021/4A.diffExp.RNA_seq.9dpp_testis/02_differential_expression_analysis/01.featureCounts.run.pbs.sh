...
# files path
IN_SEQ=($(find $MAPPED_PATH -maxdepth 1 \( -name "*.bam" -not -name "s_testis_Mov10l1_WT_8.5dpp_So820-M12_r3.SE.bam" \)))
IN_SEQ=`printf "%s " ${IN_SEQ[@]}`

# script parameters - single or paired end
if [ $SINGLE_END == TRUE ]
then
   SCRIPT_PARAMS="-T $THREADS -F GTF -M -O --fraction -s 2"
else
   SCRIPT_PARAMS="-T $THREADS -F GTF -M -O --fraction -p -s 2"
fi
