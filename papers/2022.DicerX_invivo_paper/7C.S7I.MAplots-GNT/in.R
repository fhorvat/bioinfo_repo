...
exp.df  <- data.frame( CON=as.factor( sub("^x.*_","",sub("[.]frac$","",colnames(exp.mat))) ),WHO=as.factor("VB"),row.names=colnames(exp.mat) )
...
GNT.exp.dt <- merge( mirAnnot.pepType.dt[,c("ID","miRNA.STR","det.STR","mirtron","Name","pep.type"),with=F],as.data.table(counts( FC.dds,normalized=T ),keep.rownames="ID"),by="ID",sort=F,all=T )
GNT.exp.dt <- merge( GNT.exp.dt,KO2WT.dt[,c("ID","log2FoldChange","padj"),with=F],by="ID",sort=F,all=T )
colnames(GNT.exp.dt)[ colnames(GNT.exp.dt)=="log2FoldChange" ] <- "log2FoldChange.GNT"
colnames(GNT.exp.dt)[ colnames(GNT.exp.dt)=="padj"           ] <- "padj.GNT"
save( GNT.exp.dt,file="GNT.exp.dt.rda" )
...
plot(
 ...
 xlab="PC1 (45.2% variance)",ylab="PC2 (21.1% variance)",main=OFNAME
)
