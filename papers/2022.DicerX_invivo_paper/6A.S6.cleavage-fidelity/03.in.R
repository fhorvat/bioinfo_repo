...
# >> order from biggest changes (abs.values) to smallest at "0" position ( == decreasing )
 ...
 xGAPS      <- rep.int( c(0,11,22),c(10,10,10) )
 RANLFC     <- range( all.DE.dt[ ID %in% all.mirnas , paste0("log2FoldChange.",c("EMB","ESC","TAR")) , with=F ] )
...
#(1)
 ...
 main.5p    <- cbind( tmp.esc,tmp.emb,tmp.tar )
 ...
 pheatmap(
  ...
  labels_row=rLAB[rownames(main.5p)],main="ESC / EMB / TAR : miRNA-5p\n", gaps_col=xGAPS,annotation_row=main.5p.df,annotation_colors=my_colors,
  filename=paste("compare-fidelity-5p-",ONAME,"-",pepDate(),".pdf",sep=""),width=060,height=060,fontsize=50
 )
 ...
 main.3p    <- cbind( tmp.esc,tmp.emb,tmp.tar )
 ...
 pheatmap(
  ...
  labels_row=rLAB[rownames(main.3p)],main="ESC / EMB / TAR : miRNA-3p\n", gaps_col=xGAPS,annotation_row=main.3p.df,annotation_colors=my_colors,
  filename=paste("compare-fidelity-3p-",ONAME,"-",pepDate(),".pdf",sep=""),width=060,height=070,fontsize=50
 )
