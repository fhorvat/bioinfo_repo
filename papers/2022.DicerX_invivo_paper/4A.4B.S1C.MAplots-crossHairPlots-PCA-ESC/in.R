...
exp.df  <- data.frame( CON=as.factor(rep.int(c("KO","WT"),c(3,3))),WHO=as.factor("ES"),row.names=colnames(exp.mat) )
...
ESC.exp.dt <- merge( mirAnnot.pepType.dt[,c("ID","miRNA.STR","det.STR","mirtron","Name","pep.type"),with=F],as.data.table(counts( FC.dds,normalized=T ),keep.rownames="ID"),by="ID",sort=F,all=T )
ESC.exp.dt <- merge( ESC.exp.dt,KO2WT.dt[,c("ID","log2FoldChange","padj"),with=F],by="ID",sort=F,all=T )
colnames(ESC.exp.dt)[ colnames(ESC.exp.dt)=="log2FoldChange" ] <- "log2FoldChange.ESC"
colnames(ESC.exp.dt)[ colnames(ESC.exp.dt)=="padj"           ] <- "padj.ESC"
save( ESC.exp.dt,file="ESC.exp.dt.rda" )
...
plot(
 ...
 xlab="PC1 (85.5% variance)",ylab="PC2 ( 5.1% variance)",main=OFNAME
)
