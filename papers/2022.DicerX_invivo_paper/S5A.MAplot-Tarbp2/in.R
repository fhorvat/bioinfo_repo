...
OFNAME="EMB15.5-Pullagura-smallRNAseq"
eMIN=100
SHRINKLFC=TRUE
...
exp.df  <-
 data.frame(
  CON=as.factor( sub("Mut", "KO",sub("^embryo_.*_","",sub("_r[0-9][.]frac$","",colnames(exp.mat)))) ),
  GNT=as.factor( sub("_.*$","",  sub("^embryo_",   "",sub("_r[0-9][.]frac$","",colnames(exp.mat)))) ),
  WHO=as.factor("Pullagura"),
  row.names=colnames(exp.mat)
 )

TAR.dds      <-
 DESeqDataSetFromMatrix(
  countData  = round( exp.mat[,grepl("Tarbp2",colnames(exp.mat))] ),
  colData    = exp.df[grepl("Tarbp2",rownames(exp.df)),],
  design     = ~ CON
 )
TAR.dds$CON  <- relevel( x=TAR.dds$CON,ref=xREF )
TAR.dds      <- DESeq( TAR.dds )

TAR2WT.dt <- dds2res( DDS=TAR.dds,CON="CON",A="KO",B=xREF,as.DT=T,GIDCOL="ID",shrink=SHRINKLFC,shrink.type="normal" )
TAR2WT.dt <- merge( TAR2WT.dt,mirAnnot.dt,by="ID",all.x=T,sort=T )

TAR.exp.dt <- merge( mirAnnot.pepType.dt[,c("ID","miRNA.STR","det.STR","mirtron","Name","pep.type"),with=F],as.data.table(counts( TAR.dds,normalized=T ),keep.rownames="ID"),by="ID",sort=F,all=T )
TAR.exp.dt <- merge( TAR.exp.dt,TAR2WT.dt[,c("ID","log2FoldChange","padj"),with=F],by="ID",sort=F,all=T )
colnames(TAR.exp.dt)[ colnames(TAR.exp.dt)=="log2FoldChange" ] <- "log2FoldChange.TAR"
colnames(TAR.exp.dt)[ colnames(TAR.exp.dt)=="padj"           ] <- "padj.TAR"
save( TAR.exp.dt,file="TAR.exp.dt.rda" )

assign(  x=paste0("EMB15.5-Pullagura-smallRNAseq.TAR",".de.dt"),value=TAR2WT.dt )
save( list=paste0("EMB15.5-Pullagura-smallRNAseq.TAR",".de.dt"), file=paste0("EMB15.5-Pullagura-smallRNAseq.TAR",SHROF,".de.dt.rda") )

print( yLFCLIM )
print( range(TAR2WT.dt[["log2FoldChange"]]) )
if (T) {

if (T) {
pdf( file=paste(OFNAME,SHROF,".DESeq2.MAplot-",pepDate(),".pdf",sep=""),         width=50,height=20,pointsize=25 )
par( bg="white",mfrow=c(1,2) )
pep.mirMAplot( MIRDT=TAR2WT.dt,mirtron=F,xTYPE="miRNA", lfcLIM=yLFCLIM,xLIM=xEXPLIM,TIT=paste0(OFNAME,"-main")      )
pep.mirMAplot( MIRDT=TAR2WT.dt,mirtron=F,xTYPE="miRNA*",lfcLIM=yLFCLIM,xLIM=xEXPLIM,TIT=paste0(OFNAME,"-passenger") )
dev.off()

pdf( file=paste(OFNAME,SHROF,".DESeq2.MAplot-mirtron-",pepDate(),".pdf",sep=""), width=50,height=20,pointsize=25 )
par( bg="white",mfrow=c(1,2) )
pep.mirMAplot( MIRDT=TAR2WT.dt,mirtron=T,xTYPE="miRNA", lfcLIM=yLFCLIM,xLIM=xEXPLIM,TIT=paste0(OFNAME,"-main")      )
pep.mirMAplot( MIRDT=TAR2WT.dt,mirtron=T,xTYPE="miRNA*",lfcLIM=yLFCLIM,xLIM=xEXPLIM,TIT=paste0(OFNAME,"-passenger") )
dev.off()
}

pdf( file=paste(OFNAME,SHROF,".DESeq2.MAplot-all-",pepDate(),".pdf",sep=""),     width=25,height=20,pointsize=25 )
par( bg="white",mfrow=c(1,1) )
pep.mirMAplot( MIRDT=TAR2WT.dt,mirtron=T,xTYPE="all",   lfcLIM=yLFCLIM,xLIM=xEXPLIM,TIT=paste0(OFNAME,"-all")      )
dev.off()

ddd <- TAR2WT.dt[ ,c("ID","Name","baseMean","log2FoldChange","lfcSE","stat","pvalue","padj","pep.type","mirtron"),with=F ]
ddd <- ddd[ order(padj) ]
rrr <- merge( as.data.table(fpm( object=FC.dds,robust=F ),keep.rownames="ID"),TAR2WT.dt[,c("ID","padj","log2FoldChange","Name","pep.type","mirtron"),with=F],by="ID",sort=F,all=T )
rrr <- rrr[ order(padj) ]
save( list=c("ddd","rrr"),file=paste0("CSVs",SHROF,".rda") )

assign( x=paste0(OFNAME,".de.dt"),value=TAR2WT.dt )
...
par( bg="white",mar=c(5,5,5,13),xpd=T )
plot(
 xPCA.df[,c("PC1","PC2")],pch=rep.int(c(16,17),c(5,6)),col=rep.int( c("black","magenta","gray50","red"),c(3,2,3,3) ),cex=xCEX,
# xPCA.df[,c("PC1","PC2")],pch=16,col=c("gray50","red")[ as.integer(xPCA.df$CON=="KO")+1 ],cex=xCEX,
 xlab="PC1 (36.5% variance)",ylab="PC2 (29.0% variance)",main=OFNAME
)
legend(
 x=max(xPCA.df$PC1)*1.1,y=max(xPCA.df$PC2)*0.9,legend=c("Pkr-wt","Pkr-KO","Tarbp2-wt","Tarbp2-KO"),bty="n",
 pch=rep.int(c(16,17),c(2,2)),cex=1.75,pt.cex=xCEX,col=c("black","magenta","gray50","red")
)
dev.off()
...
qqq <- mrgByPreMiRNA( idt=TAR2WT.dt,LIMS=yLFCLIM,SELE=sele.miRNAs )
dev.off()
