...
breeding_tidy <- 
  breeding_tb %>% 
  as_tibble(.) %>% 
  dplyr::select(male, female, litter_size = Total, mother_age = "Age.of.mother.in.days", dead = Dead) %>% 
  dplyr::filter(!((dead > 0) & (dead == litter_size))) %>% 
...
# breeding_test <-
#   breeding_tidy %>%
#   dplyr::filter(mother_age == "16 - 32 weeks")
# 
# t.test(breeding_test %>% dplyr::filter(genotype == "WT") %$% litter_size,
#        breeding_test %>% dplyr::filter(genotype == "Het") %$% litter_size,
#        alternative = "greater")
# 
# wilcox.test(breeding_test %>% dplyr::filter(genotype == "WT") %$% litter_size,
#             breeding_test %>% dplyr::filter(genotype == "Het") %$% litter_size,
#             alternative = "greater")
...
breeding_plot <- 
  ...
  ggsave(filename = file.path(outpath, "litter_size.female_genotype.mating_age.without_dead.plot.png"), width = 10, height = 10)
