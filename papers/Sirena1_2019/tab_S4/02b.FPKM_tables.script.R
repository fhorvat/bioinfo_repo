...
original_tb_path <- "../2019 Ganesh Table S4 Elob pseudogenes.xlsx"
...
original_tb %>% 
  dplyr::left_join(., fpkm_mean_tb %>% dplyr::rename(Fugaku_GV.WE_FPKM = GV.WE), by = c("ID" = "gene_id")) %T>%
  openxlsx::write.xlsx(., file.path(outpath, original_tb_path %>% basename(.) %>% str_replace(., "\\.xlsx", ".with_Fugaku_FPKM.xlsx")))
