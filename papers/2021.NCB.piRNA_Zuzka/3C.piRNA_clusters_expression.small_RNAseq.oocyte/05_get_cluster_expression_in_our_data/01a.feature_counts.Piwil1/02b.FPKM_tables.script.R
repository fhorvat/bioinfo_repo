...
# reads stats path
reads_stats_path <- file.path(documentation_path, "../Mapped/STAR_mesAur1/4_library_size/library_sizes.txt")
...
reads_stats <- reads_stats[str_detect(sample_id, "\\.18to32nt")]
...
sample_table[, sample_id := str_c(sample_id, "18to32nt", sep = ".")]
sample_table[reads_stats, on = "sample_id", `:=`(library_size = library_size)]
sample_table <- sample_table[!is.na(library_size), ]
...
fpm_long_tb <-
  ...
  melt(.,
       ...
       value.name = "counts") %>%
  .[, read_length := str_extract(sample_id, "18to20nt|21to23nt|24to27nt|28to31nt")] %>%
  .[, sample_id := str_remove_all(sample_id, "\\.18to20nt|\\.21to23nt|\\.24to27nt|\\.28to31nt|\\.bam")] %>%
  ...
  .[, c("gene_id", "sample_id", "read_length", "counts", "fpm", "fpkm", grouping_variables, "coordinates"), with = F] %>%
  .[!is.na(gene_id)]
...
fpm_wide_tb <-
  fpm_long_tb %>%
  dcast(., gene_id + coordinates ~ sample_id + read_length, value.var = "fpm", sep = ".") %T>%
...
fpm_mean_tb <-
  ...
  .[, .(fpm_mean = round(mean(fpm), 3),
        coordinates = unique(coordinates)), by = c("gene_id", "genotype", "read_length")] %>%
  .[, grouped_variables := do.call(str_c, c(.SD, sep = ".")), .SDcols = c("genotype", "read_length")] %>%
...
fpm_stats <-
  ...
  .[, list(avg_fpm = mean(fpm),
    ...
    by = c("gene_id", c("genotype", "read_length"))] %T>%
