...
######################################################## WORKING DIRECTORY
setwd("/common/WORK/fhorvat/Projekti/Svoboda/piRNA.Zuzka/Analysis/2020_paper/piRNA_clusters.oocyte/recalculate_cluster_expression")
...
# RPM table path
rpm_tb_path <- list.files(inpath, ".*\\.FPM_mean\\.csv$", full.names = T, recursive = T)

# normalized width path
norm_width_path <- "../count_N_in_genome"
norm_width_path <- list.files(norm_width_path, pattern = "piRNA_clusters.*\\.norm_width\\.csv", full.names = T)
...
purrr::map(c("PIWIL1", "PIWIL3"), function(piwil){
  ...
  norm_width_tb <- readr::read_csv(norm_width_path[str_detect(norm_width_path, piwil)])
