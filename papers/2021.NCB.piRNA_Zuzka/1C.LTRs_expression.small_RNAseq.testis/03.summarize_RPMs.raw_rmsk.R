...
dataset_name_list <- c("hamster_testis_Mov10l.8.5dpp.small_RNAseq", 
                       ...
                       "hamster_Siomi_WT.Piwil1_Piwil3_IP.small_RNAseq", 
                       "hamster_oocyte_Mov10l.deduplicated.smallRNAseq")
...
purrr::map(names(read_lengths), function(rlen){
  ...
  rpm_long <- 
    ...
    dplyr::ungroup(.) %>% 
    dplyr::mutate(age = str_extract(sample_id, "13dpp|21dpp|8.5_dpp|8.5dpp|adult|2C|oocyte(?=_PIWIL)|ovary(?=_PIWIL1)|testis(?=_PIWIL1)"), 
                  genotype = str_extract(sample_id, "Mov10l_WT|Mov10l_HET|Mov10l_KO|Mov10l1_KO|Mov10l1_WT|PIWIL1_IP|PIWIL3_IP"), 
                  reseq = ifelse((str_detect(sample_id, "reseq")) | (sample_id %in% c("s_testis_Mov10l1_KO_8.5dpp_So820-M10_half_r2.SE", 
                                                                                      "s_testis_Mov10l1_KO_8.5dpp_So820-M10_r1.SE", 
                                                                                      "s_testis_Mov10l1_WT_8.5dpp_So802-M1_r1.SE", 
                                                                                      "s_testis_Mov10l1_WT_8.5dpp_So802-M3_r2.SE",
                                                                                      "s_testis_Mov10l1_WT_8.5dpp_So820-M12_r3.SE")), 
                                 T, F),
                  genotype = str_replace(genotype, "Mov10l_", "Mov10l1_")) %>%
    dplyr::mutate(age = replace(age, age == "8.5_dpp", "8.5dpp"),
                  age = factor(age, levels = c("8.5dpp", "13dpp", "21dpp", "adult", "oocyte", "2C", "ovary", "testis")),
                  genotype = ifelse(reseq, str_c(genotype, ".reseq"), genotype),
                  genotype = factor(genotype, levels = c("Mov10l1_WT", "Mov10l1_WT.reseq", "Mov10l1_HET", "Mov10l1_KO", "Mov10l1_KO.reseq", 
                                                         "PIWIL1_IP", "PIWIL3_IP"))) %>% 
...
saveWorkbook(wb_individual, file.path(outpath, str_c("all_LTR_classes", "small_RNAseq", "all_samples_RPM", "20210505", "xlsx", sep = ".")), overwrite = TRUE)
saveWorkbook(wb_mean, file.path(outpath, str_c("all_LTR_classes", "small_RNAseq", "mean_RPM", "20210505", "xlsx", sep = ".")), overwrite = TRUE)
